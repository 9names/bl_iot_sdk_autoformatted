...
static int cb_tx(void *usrdata, uint8_t *audiodata, int len, int is_overflow) {
  ...
  if (((bl602_dac_device_env_t *)(device->env))->cb_tx) {
    return ((bl602_dac_device_env_t *)(device->env))
        ->cb_tx(device, usrdata, audiodata, len, is_overflow);
  }
...
static int cb_rx(void *usrdata, uint8_t *audiodata, int len, int is_overflow) {
  ...
  if (((bl602_dac_device_env_t *)(device->env))->cb_rx) {
    return ((bl602_dac_device_env_t *)(device->env))
        ->cb_rx(device, usrdata, audiodata, len, is_overflow);
  }
...
static int bl602_dac_pause(audio_framework_device_t *device) { return 0; }
...
static int bl602_dac_set_format(audio_framework_device_t *device,
                                AUDIO_FRAMEWORK_FORMAT_T format) {
  /*16bit*/
  switch (format) {
...
static int bl602_dac_set_samplerate(audio_framework_device_t *device,
                                    AUDIO_FRAMEWORK_SAMPLERATE_T samplerate) {
  bl602_dac_device_env_t *p_env = (bl602_dac_device_env_t *)device->env;
...
static int bl602_dac_set_rxcallback(audio_framework_device_t *device,
                                    audio_framework_callback_rx cb_rx) {
...
static int bl602_dac_set_txcallback(audio_framework_device_t *device,
                                    audio_framework_callback_tx cb_tx) {
  ((bl602_dac_device_env_t *)(device->env))->cb_tx = cb_tx;

  return 0;
}

static int bl602_dac_set_buffersize_rx(audio_framework_device_t *device,
                                       int size, int *size_result) {
...
static int bl602_dac_set_buffersize_tx(audio_framework_device_t *device,
                                       int size, int *size_result) {
...
static int bl602_dac_reset_samplerate(audio_framework_device_t *device,
                                      AUDIO_FRAMEWORK_SAMPLERATE_T samplerate) {
...
const static audio_framework_device_t __g_bl602_dac_device
    ATTR_AUDIO_DEVICE_TABLE = {
        .name = "bl602_dac",
        .init = bl602_dac_init,
...
int bl602_dac_device_fake(void) { return 0; }
