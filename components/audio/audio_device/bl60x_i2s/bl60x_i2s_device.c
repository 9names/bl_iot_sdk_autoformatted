...
static int cb_tx(void *usrdata, uint8_t *audiodata, int len, int is_overflow) {
  ...
  if (((bl60x_i2s_device_env_t *)(device->env))->cb_tx) {
    ((bl60x_i2s_device_env_t *)(device->env))
...
static int bl60x_i2s_pause(audio_framework_device_t *device) { return 0; }

static int bl60x_i2s_unint(audio_framework_device_t *device) { return 0; }

static int bl60x_i2s_set_format(audio_framework_device_t *device,
                                AUDIO_FRAMEWORK_FORMAT_T format) {
  /*16bit*/
  switch (format) {
  case AUDIO_FRAMEWORK_FORMAT_DATA16BIT_WIDTH16BIT_MONO:
...
static int bl60x_i2s_set_samplerate(audio_framework_device_t *device,
                                    AUDIO_FRAMEWORK_SAMPLERATE_T samplerate) {
  switch (samplerate) {
  case AUDIO_FRAMEWORK_SAMPLERATE_8K: {
    bl_i2s_sample_rate_config(BL_I2S_SAMPLE_RATE_8K);
...
static int bl60x_i2s_set_rxcallback(audio_framework_device_t *device,
                                    audio_framework_callback_rx cb_rx) {
...
static int bl60x_i2s_set_txcallback(audio_framework_device_t *device,
...
static int bl60x_i2s_set_buffersize_rx(audio_framework_device_t *device,
...
static int bl60x_i2s_set_buffersize_tx(audio_framework_device_t *device,
                                       int size, int *size_result) {
...
static int bl60x_i2s_reset_samplerate(audio_framework_device_t *device,
                                      AUDIO_FRAMEWORK_SAMPLERATE_T samplerate) {
  switch (samplerate) {
  case AUDIO_FRAMEWORK_SAMPLERATE_8K: {
    bl_i2s_sample_rate_config(BL_I2S_SAMPLE_RATE_8K);
...
int bl60x_i2s_device_fake(void) { return 0; }
