...
typedef int (*audio_framework_callback_rx)(audio_framework_device_t *device,
                                           void *uerdata, uint8_t *audiodata,
                                           int len, int is_overflow);
typedef int (*audio_framework_callback_tx)(audio_framework_device_t *device,
                                           void *uerdata, uint8_t *audiodata,
                                           int len, int is_underrun);
int audio_framework_factory_build(const char *name,
...
struct audio_framework_device {
  ...
  int (*set_format)(audio_framework_device_t *device,
                    AUDIO_FRAMEWORK_FORMAT_T format);
  /*configure audio sample rate*/
  int (*set_samplerate)(audio_framework_device_t *device,
                        AUDIO_FRAMEWORK_SAMPLERATE_T samplerate);
  /*configure TX callback*/
  int (*set_rxcallback)(audio_framework_device_t *device,
                        audio_framework_callback_rx cb_rx);
  /*configure RX callback*/
  int (*set_txcallback)(audio_framework_device_t *device,
                        audio_framework_callback_tx cb_tx);
  /*configure rx buffer*/
  int (*set_buffersize_rx)(audio_framework_device_t *device, int size,
                           int *size_result);
  /*configure tx buffer*/
  int (*set_buffersize_tx)(audio_framework_device_t *device, int size,
                           int *size_result);

  /*reset audio sample rate*/
  int (*reset_samplerate)(audio_framework_device_t *device,
                          AUDIO_FRAMEWORK_SAMPLERATE_T samplerate);
  /*set audio mclk */
...
#define ATTR_AUDIO_DEVICE_TABLE                                                \
  __attribute__((section(".framework.audio_device"))) __attribute__((used))
#endif
