...
int32_t bflb_crypt_init_do(bflb_crypt_handle_t *crypt_handle, uint8_t type) {
  ...
  case BFLB_CRYPT_TYPE_CHACHAPOLY1305:
    memset((ChaChaPoly1305 *)&crypt_handle->crypt_ctx, 0,
           sizeof(ChaChaPoly1305));
    break;
  default:
...
int32_t bflb_crypt_setkey_do(bflb_crypt_handle_t *crypt_handle,
  ...
  memset(crypt_handle->crypt_cfg.iv_nonce, 0,
         sizeof(crypt_handle->crypt_cfg.iv_nonce));
  memset(crypt_handle->crypt_cfg.stream_block, 0,
         sizeof(crypt_handle->crypt_cfg.stream_block));
  ...
  case BFLB_CRYPT_TYPE_AES_CBC:
    if (dir == BFLB_CRYPT_DIR_ENCRYPT) {
      ret = mbedtls_aes_setkey_enc(
          (mbedtls_aes_context *)&crypt_handle->crypt_ctx, key, key_len * 8);
    } else {
  ...
  case BFLB_CRYPT_TYPE_CHACHA:
    ret |=
        Chacha_SetIV((ChaCha *)&crypt_handle->crypt_ctx, nonce, nonce_len, 0);
...
int32_t bflb_crypt_setadd_do(bflb_crypt_handle_t *crypt_handle,
                             const uint8_t *add, uint8_t len, uint8_t dir) {
  int32_t ret = BFLB_CRYPT_OK;

  switch (crypt_handle->crypt_cfg.type) {
  case BFLB_CRYPT_TYPE_AES_GCM:
    ret =
        mbedtls_gcm_starts((mbedtls_gcm_context *)&crypt_handle->crypt_ctx,
                           dir == BFLB_CRYPT_DIR_ENCRYPT ? MBEDTLS_GCM_ENCRYPT
                                                         : MBEDTLS_GCM_DECRYPT,
                           crypt_handle->crypt_cfg.iv_nonce,
                           crypt_handle->crypt_cfg.nonce_len, add, len);
    break;
  default:
    bflb_crypt_printe("unsupported type\r\n");
    return BFLB_CRYPT_ERROR;
...
int32_t bflb_crypt_encrypt_do(bflb_crypt_handle_t *crypt_handle,
                              const uint8_t *in, uint32_t len, size_t offset,
                              uint8_t *out) {
  int result;
  ...
  case BFLB_CRYPT_TYPE_AES_CBC:
    ...
    result = mbedtls_aes_crypt_cbc(
        (mbedtls_aes_context *)&crypt_handle->crypt_ctx, MBEDTLS_AES_ENCRYPT,
        len, crypt_handle->crypt_cfg.iv_nonce, in, out);
    break;
  case BFLB_CRYPT_TYPE_AES_CTR:
    result =
        mbedtls_aes_crypt_ctr((mbedtls_aes_context *)&crypt_handle->crypt_ctx,
                              len, &offset2, crypt_handle->crypt_cfg.iv_nonce,
                              crypt_handle->crypt_cfg.stream_block, in, out);
    break;
  case BFLB_CRYPT_TYPE_CHACHA:
...
int32_t bflb_crypt_encrypt_tag_do(bflb_crypt_handle_t *crypt_handle,
                                  const uint8_t *in, uint32_t in_len,
                                  const uint8_t *add, uint32_t add_len,
                                  size_t offset, uint8_t *out, uint8_t *tag,
                                  uint8_t tag_len) {
  int result;
  ...
  case BFLB_CRYPT_TYPE_AES_CCM:
    result = mbedtls_ccm_encrypt_and_tag(
        (mbedtls_ccm_context *)&crypt_handle->crypt_ctx, in_len,
        crypt_handle->crypt_cfg.iv_nonce, crypt_handle->crypt_cfg.nonce_len,
        add, add_len, in, out, tag, tag_len);
    break;
  case BFLB_CRYPT_TYPE_AES_GCM:
    result = mbedtls_gcm_crypt_and_tag(
        (mbedtls_gcm_context *)&crypt_handle->crypt_ctx, MBEDTLS_GCM_ENCRYPT,
        in_len, crypt_handle->crypt_cfg.iv_nonce,
        crypt_handle->crypt_cfg.nonce_len, add, add_len, in, out, tag_len, tag);
    break;
  case BFLB_CRYPT_TYPE_CHACHAPOLY1305:
    result = ChaCha20Poly1305_Encrypt(
        (ChaChaPoly1305 *)&crypt_handle->crypt_ctx, crypt_handle->crypt_cfg.key,
        crypt_handle->crypt_cfg.key_len, crypt_handle->crypt_cfg.iv_nonce,
        crypt_handle->crypt_cfg.nonce_len, add, add_len, in, in_len, out, tag,
        tag_len);
    break;
  default:
    bflb_crypt_printe("unsupported type\r\n");
...
int32_t bflb_crypt_update_do(bflb_crypt_handle_t *crypt_handle,
...
int32_t bflb_crypt_decrypt_do(bflb_crypt_handle_t *crypt_handle,
                              const uint8_t *in, uint32_t len, size_t offset,
                              uint8_t *out) {
  int result;
  size_t offset2 = offset;
  ...
  case BFLB_CRYPT_TYPE_AES_CBC:
    ...
    result = mbedtls_aes_crypt_cbc(
        (mbedtls_aes_context *)&crypt_handle->crypt_ctx, MBEDTLS_AES_DECRYPT,
        len, crypt_handle->crypt_cfg.iv_nonce, in, out);
    break;
  case BFLB_CRYPT_TYPE_AES_CTR:
    result =
...
int32_t bflb_crypt_auth_decrypt_do(bflb_crypt_handle_t *crypt_handle,
                                   const uint8_t *in, uint32_t in_len,
                                   const uint8_t *add, uint32_t add_len,
  ...
  case BFLB_CRYPT_TYPE_AES_GCM:
    result = mbedtls_gcm_auth_decrypt(
        (mbedtls_gcm_context *)&crypt_handle->crypt_ctx, in_len,
        (const uint8_t *)crypt_handle->crypt_cfg.iv_nonce,
        crypt_handle->crypt_cfg.nonce_len, add, add_len, tag, tag_len, in, out);
    break;
  case BFLB_CRYPT_TYPE_CHACHAPOLY1305:
    result = ChaCha20Poly1305_Decrypt(
        (ChaChaPoly1305 *)&crypt_handle->crypt_ctx, crypt_handle->crypt_cfg.key,
        crypt_handle->crypt_cfg.key_len, crypt_handle->crypt_cfg.iv_nonce,
        crypt_handle->crypt_cfg.nonce_len, add, add_len, in, in_len, tag,
        tag_len, out);
    break;
  default:
    bflb_crypt_printe("unsupported type\r\n");
...
int32_t bflb_crypt_finish_do(bflb_crypt_handle_t *crypt_handle, uint8_t *tag,
                             uint32_t len) {
  int result;
...
int32_t bflb_crypt_setkey_do(bflb_crypt_handle_t *crypt_handle,
                             const uint8_t *key, uint8_t keytype,
                             uint8_t key_len, const uint8_t *nonce,
                             uint8_t nonce_len, uint8_t dir) {
  int32_t ret = BFLB_CRYPT_OK;

  switch (crypt_handle->crypt_cfg.type) {
  case BFLB_CRYPT_TYPE_AES_CBC:
    Sec_Eng_AES_Enable_BE(SEC_ENG_AES_ID0);
    Sec_Eng_AES_Init(&aesCtx, SEC_ENG_AES_ID0, SEC_ENG_AES_CBC,
                     (SEC_ENG_AES_Key_Type)keytype,
                     BFLB_CRYPT_DIR_ENCRYPT == dir ? SEC_ENG_AES_ENCRYPTION
                                                   : SEC_ENG_AES_DECRYPTION);
    break;
  case BFLB_CRYPT_TYPE_AES_CTR:
    Sec_Eng_AES_Enable_BE(SEC_ENG_AES_ID0);
    Sec_Eng_AES_Init(&aesCtx, SEC_ENG_AES_ID0, SEC_ENG_AES_CTR,
                     (SEC_ENG_AES_Key_Type)keytype,
                     BFLB_CRYPT_DIR_ENCRYPT == dir ? SEC_ENG_AES_ENCRYPTION
                                                   : SEC_ENG_AES_DECRYPTION);
    break;
...
int32_t bflb_crypt_setkey(bflb_crypt_handle_t *crypt_handle, const uint8_t *key,
                          uint8_t keytype, uint8_t key_len,
                          const uint8_t *nonce, uint8_t nonce_len,
                          uint8_t dir) {
  int32_t result = BFLB_CRYPT_OK;

  result = bflb_crypt_setkey_do(crypt_handle, key, keytype, key_len, nonce,
                                nonce_len, dir);
...
int32_t bflb_crypt_setadd(bflb_crypt_handle_t *crypt_handle, const uint8_t *add,
...
int32_t bflb_crypt_encrypt(bflb_crypt_handle_t *crypt_handle, const uint8_t *in,
...
int32_t bflb_crypt_encrypt_tag(bflb_crypt_handle_t *crypt_handle,
                               const uint8_t *in, uint32_t in_len,
                               const uint8_t *add, uint32_t add_len,
                               size_t offset, uint8_t *out, uint8_t *tag,
                               uint8_t tag_len) {
  ...
  result = bflb_crypt_encrypt_tag_do(crypt_handle, in, in_len, add, add_len,
                                     offset, out, tag, tag_len);
...
int32_t bflb_crypt_update(bflb_crypt_handle_t *crypt_handle, const uint8_t *in,
...
int32_t bflb_crypt_decrypt(bflb_crypt_handle_t *crypt_handle, const uint8_t *in,
                           uint32_t len, size_t offset, uint8_t *out) {
  int32_t result;
...
int32_t bflb_crypt_auth_decrypt(bflb_crypt_handle_t *crypt_handle,
                                const uint8_t *in, uint32_t in_len,
                                const uint8_t *add, uint32_t add_len,
                                size_t offset, uint8_t *out, const uint8_t *tag,
                                uint8_t tag_len) {
  int32_t result;

  result = bflb_crypt_auth_decrypt_do(crypt_handle, in, in_len, add, add_len,
                                      offset, out, tag, tag_len);
...
int32_t bflb_crypt_finish(bflb_crypt_handle_t *crypt_handle, uint8_t *tag,
                          uint32_t len) {
  int32_t result;
