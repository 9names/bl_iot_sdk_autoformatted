...
return number
*/
int32_t bflb_dsa_mexp_binary(uint32_t size, uint32_t *a, uint32_t *b,
  ...
  /* number = 1 */
  Sec_Eng_PKA_Write_Data(nregType, 5, (uint32_t *)oneBuf, sizeof(oneBuf) / 4,
                         0);
...
/*r=a^b%c*/
int32_t bflb_dsa_mexp_mont(uint32_t size, uint32_t *a, uint32_t *b, uint32_t *c,
                           uint32_t *invR_c, uint32_t *primeN_c, uint32_t *r) {
  SEC_ENG_PKA_REG_SIZE_Type nregType = bflb_dsa_get_reg_size(size);
  SEC_ENG_PKA_REG_SIZE_Type lregType = bflb_dsa_get_reg_size(size * 2);
  uint32_t dataSize = (size >> 3) >> 2;
  ...
  Sec_Eng_PKA_CREG(nregType, 11, dataSize, 0);
  /*index 2 is invertR*/
  Sec_Eng_PKA_Mont2GF(nregType, 7, nregType, 6, nregType, 2, lregType, 5,
                      nregType, 0);
...
int32_t bflb_dsa_decrypt_crt(uint32_t size, uint32_t *c,
                             bflb_dsa_crt_cfg *crtCfg, uint32_t *d,
                             uint32_t *r) {
  /*
   * m1 = pow(c, dP, p)
   * m2 = pow(c, dQ, q)
   ...
   * 10&11:qInv*(m1-m2)
   */
  bflb_dsa_mexp_mont(size, c, crtCfg->dP, crtCfg->p, crtCfg->invR_p,
                     crtCfg->primeN_p, m1);
  bflb_dsa_mexp_mont(size, c, crtCfg->dQ, crtCfg->q, crtCfg->invR_q,
                     crtCfg->primeN_q, m2);
...
int32_t bflb_dsa_init(uint32_t size) { return 0; }
