...
int32_t bflb_rsa_init(bflb_rsa_handle_t *rsa_handle, int padding,
                      int hashtype) {
  int32_t ret = BFLB_RSA_OK;
  memset(rsa_handle, 0, sizeof(bflb_rsa_handle_t));
  mbedtls_rsa_init((mbedtls_rsa_context *)&rsa_handle->rsa_ctx, padding,
                   bflb_hash_get_type(hashtype));
  return ret;
}

int32_t bflb_rsa_set_parameter(bflb_rsa_handle_t *rsa_handle, int type,
...
int32_t bflb_rsa_sign(bflb_rsa_handle_t *rsa_handle, const uint8_t *hash,
                      uint8_t hashlen, uint8_t hashtype, uint8_t *sig,
                      uint32_t *slen) {
  int32_t ret = BFLB_RSA_OK;
  mbedtls_rsa_context *rsa = (mbedtls_rsa_context *)&rsa_handle->rsa_ctx;
...
int32_t bflb_rsa_verify(bflb_rsa_handle_t *rsa_handle, const uint8_t *hash,
                        uint8_t hashlen, uint8_t hashtype, const uint8_t *sig,
                        uint32_t slen) {
  int32_t ret = BFLB_RSA_OK;
  mbedtls_rsa_context *rsa = (mbedtls_rsa_context *)&rsa_handle->rsa_ctx;
  ...
  ret = mbedtls_rsa_pkcs1_verify(
      ...
      (mbedtls_md_type_t)bflb_hash_get_type(hashtype), hashlen, hash, sig);
  if (ret != 0) {
