...
void SystemInit(void) {
  ...
  BMX_Cfg_Type bmxCfg = {
      .timeoutEn = 0, .errEn = DISABLE, .arbMod = BMX_ARB_FIX};
  ...
  *p = 0x5A5AA5A5;
  /*diable BMX error incase Sbooten=0xf,while user send vector(core) reset and
  CPU read deadbeef,
  ...
  // AON_Power_On_SOC_11();
  /* Record LDO18 pu flag before power up. This maybe not neccessary but copy
   * from 606*/
...
void System_NVIC_SetPriority(IRQn_Type IRQn, uint32_t PreemptPriority,
