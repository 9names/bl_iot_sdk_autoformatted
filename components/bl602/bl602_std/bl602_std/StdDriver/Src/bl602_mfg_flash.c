...
static BL_Err_Type PtTable_Flash_Read(uint32_t addr, uint8_t *data,
                                      uint32_t len) {
...
int8_t mfg_flash_init(SPI_Flash_Cfg_Type *flashCfg) {
  ...
  if (pFlashCfg != NULL) {
    ...
    ret = PtTable_Get_Active_Entries_By_Name(
        &ptTableStuff[activeID], (uint8_t *)RF_PARA_PART_NAME, &ptEntry);
...
static int8_t mfg_flash_program(void) {
  ...
  __disable_irq();
  ret = XIP_SFlash_Write_Need_Lock(pFlashCfg, rf_para_addr, (uint8_t *)&rf_para,
                                   sizeof(rf_para));
  __enable_irq();
  if (ret != SUCCESS) {
...
static int8_t mfg_flash_read(void) {
  ...
  __disable_irq();
  ret = XIP_SFlash_Read_Need_Lock(pFlashCfg, rf_para_addr, (uint8_t *)&rf_para,
                                  sizeof(rf_para));
  __enable_irq();
  if (ret != SUCCESS) {
    mfg_print("Flash write error\r\n");
...
int8_t mfg_flash_read_xtal_capcode(uint8_t *capcode, uint8_t reload) {
  ...
  if (rf_para.magic == RF_PARA_MAGIC_FLAG) {
    if (rf_para.crc32 ==
        (BFLB_Soft_CRC32(&rf_para.capcode_valid, sizeof(rf_para) - 8))) {
