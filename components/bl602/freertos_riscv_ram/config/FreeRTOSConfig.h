...
/* Creating idle task */
/* -1  -> prvInitialiseNewTask: (subtract 1 to get top of stack) pxTopOfStack =
 * &( pxNewTCB->pxStack[ ulStackDepth - ( uint32_t ) 1 ] ); */
/* -1  -> prvInitialiseNewTask: (subtract 1 in case not matching 8 bytes
 * alignment) pxTopOfStack = ( StackType_t * ) ( ( ( portPOINTER_SIZE_TYPE )
...
/* -34 -> portasmSAVE_ADDITIONAL_REGISTERS: addi sp, sp,
 * -(portasmADDITIONAL_CONTEXT_SIZE * portWORD_SIZE) */
/* Checking for stack overflow */
/*  4  -> taskCHECK_FOR_STACK_OVERFLOW: if( ( pulStack[ 0 ] != ulCheckValue ) ||
 * ( pulStack[ 1 ] != ulCheckValue ) || ( pulStack[ 2 ] != ulCheckValue ) || (
 * pulStack[ 3 ] != ulCheckValue ) ) */
#define configMINIMAL_STACK_SIZE                                               \
  ((unsigned short)98) /* SIZE-1-1-12-16-30-34>=4 */
