...
#define GET_F32_REG(insn, pos, regs)                                           \
  ({                                                                           \
    ...
    uintptr_t tmp;                                                             \
    asm("1: auipc %0, %%pcrel_hi(get_f32_reg); add %0, %0, %1; jalr t0, %0, "  \
        "%%pcrel_lo(1b)"                                                       \
        : "=&r"(tmp), "+&r"(value)::"t0");                                     \
    value;                                                                     \
  })
#define SET_F32_REG(insn, pos, regs, val)                                      \
  ({                                                                           \
    ...
    uintptr_t tmp;                                                             \
    asm volatile("1: auipc %0, %%pcrel_hi(put_f32_reg); add %0, %0, %2; jalr " \
                 "t0, %0, %%pcrel_lo(1b)"                                      \
                 : "=&r"(tmp)                                                  \
                 : "r"(value), "r"(offset)                                     \
                 : "t0");                                                      \
...
#define GET_F64_REG(insn, pos, regs)                                           \
  ({                                                                           \
    ...
    uintptr_t tmp;                                                             \
    asm("1: auipc %0, %%pcrel_hi(get_f64_reg); add %0, %0, %1; jalr t0, %0, "  \
        "%%pcrel_lo(1b)"                                                       \
        : "=&r"(tmp), "+&r"(value)::"t0");                                     \
...
#define SET_F64_REG(insn, pos, regs, val)                                      \
  ({                                                                           \
    ...
    uintptr_t offset = SHIFT_RIGHT(insn, (pos)-3) & 0xf8;                      \
    uintptr_t tmp;                                                             \
    asm volatile("1: auipc %0, %%pcrel_hi(put_f64_reg); add %0, %0, %2; jalr " \
                 "t0, %0, %%pcrel_lo(1b)"                                      \
                 : "=&r"(tmp)                                                  \
                 : "r"(value), "r"(offset)                                     \
                 : "t0");                                                      \
...
#else
#define GET_F64_REG(insn, pos, regs)                                           \
  (*(int64_t *)((void *)((regs) + 32) + (SHIFT_RIGHT(insn, (pos)-3) & 0xf8)))
