...
#define DECLARE_UNPRIVILEGED_STORE_FUNCTION(type, insn)                        \
  static inline void store_##type(type *addr, type val, uintptr_t mepc) {      \
    ...
    asm volatile("csrrs %0, mstatus, %3\n" #insn " %1, %2\n"                   \
                 "csrw mstatus, %0"                                            \
                 : "+&r"(__mstatus)                                            \
                 : "r"(val), "m"(*addr), "r"(__mstatus_adjust), "r"(__mepc));  \
  }
...
static inline void store_uint64_t(uint64_t *addr, uint64_t val,
                                  uintptr_t mepc) {
...
static uintptr_t inline __attribute__((always_inline))
