...
int backtrace_now(int (*print_func)(const char *fmt, ...), uintptr_t *regs)
    __attribute__((weak, alias("backtrace_riscv")));
...
static inline void backtrace_stack_app(int (*print_func)(const char *fmt, ...),
                                       unsigned long *fp, int depth) {
  uintptr_t *pc;

  while (depth--) {
    if ((((uintptr_t)fp & 0xff000000ul) != VALID_PC_START_XIP) &&
        (((uintptr_t)fp & 0xff000000ul) != VALID_FP_START_XIP)) {
      print_func("!!");
      return;
    }
    ...
    if ((((uintptr_t)pc & 0xff000000ul) != VALID_PC_START_XIP) &&
        (((uintptr_t)pc & 0xff000000ul) != VALID_FP_START_XIP)) {
      print_func("!!");
      return;
    }
...
BaseType_t xPortIsInsideInterrupt(void) { return TrapNetCounter ? 1 : 0; }
