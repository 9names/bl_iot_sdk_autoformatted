...
static ssize_t vfs_adc_read(file_t *fp, void *buf, size_t nbytes) {
  ...
  if ((fp != NULL) && (fp->node != NULL)) {
    ...
    if (adc_dev != NULL) {
      ...
      if (0 == ret) {
        ret = nbytes;
        memcpy(buf, &adc_value,
               sizeof(adc_value)); // we use memcpy since the user may pass
                                   // unaligned pointer to us
      } else {
...
static int vfs_adc_poll(file_t *fp, bool setup, poll_notify_t notify,
...
static int vfs_adc_ioctl(file_t *fp, int cmd, unsigned long arg) {
  ...
  /*we only support arch which have the same size between pointer and unsigned
   * long*/
