...
/* gpio driver struct */
const struct file_ops gpio_ops = {.open = vfs_gpio_open,
...
int vfs_gpio_ioctl(file_t *fp, int cmd, unsigned long arg) {
  ...
  case IOCTL_GPIO_PULL_UP:
    ret =
        hal_gpio_pulltype_set(gpio_dev, GPIO_CONFIG_PULL_UP); /* gpio pullup */
    break;
  case IOCTL_GPIO_PULL_DOWN:
    ret = hal_gpio_pulltype_set(gpio_dev,
                                GPIO_CONFIG_PULL_DOWN); /* gpio pulldown */
    break;
  case IOCTL_GPIO_PULL_NONE:
    ret = hal_gpio_pulltype_set(gpio_dev,
                                GPIO_CONFIG_PULL_NONE); /* gpio pullnone */
    break;
  default:
    ret = -EINVAL;
    break;
