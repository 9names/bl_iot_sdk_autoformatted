...
/* i2c driver struct */
const struct file_ops i2c_ops = {.open = vfs_i2c_open,
                                 .close = vfs_i2c_close,
                                 .read = vfs_i2c_read,
                                 .write = vfs_i2c_write};
...
ssize_t vfs_i2c_read(file_t *fp, void *buf, size_t nbytes) {
  ...
  if ((fp != NULL) && (fp->node != NULL)) {
    ...
    if (dev_addr != NULL) {
      ...
      /* get data from i2c. */
      ret = hal_i2c_master_recv(i2c_dev, dev_addr, (uint8_t *)buf, nbytes,
                                HAL_WAIT_FOREVER);
...
ssize_t vfs_i2c_write(file_t *fp, const void *buf, size_t nbytes) {
  ...
  if ((fp != NULL) && (fp->node != NULL)) {
    ...
    if (dev_addr != NULL) {
      ...
      /* send data from i2c. */
      ret = hal_i2c_master_send(i2c_dev, dev_addr, (const uint8_t *)buf, nbytes,
                                HAL_WAIT_FOREVER);
