...
ssize_t vfs_spi_read(file_t *fp, void *buf, size_t nbytes) {
  ...
  if ((fp != NULL) && (fp->node != NULL)) {
    ...
    if (spi_dev != NULL) {
      ...
      /* get data from spi. */
      ret =
          hal_spi_recv(spi_dev, (unsigned char *)buf, nbytes, HAL_WAIT_FOREVER);
...
ssize_t vfs_spi_write(file_t *fp, const void *buf, size_t nbytes) {
  ...
  if ((fp != NULL) && (fp->node != NULL)) {
    ...
    if (spi_dev != NULL) {
      ...
      /* send data from spi. */
      ret =
          hal_spi_send(spi_dev, (const uint8_t *)buf, nbytes, HAL_WAIT_FOREVER);
...
/* spi driver struct */
const struct file_ops spi_ops = {.open = vfs_spi_open,
                                 .close = vfs_spi_close,
                                 .read = vfs_spi_read,
                                 .write = vfs_spi_write,
                                 .ioctl = vfs_spi_ioctl};
