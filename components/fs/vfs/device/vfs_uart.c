...
static void __uart_rx_irq(void *p_arg) {
  ...
  if (length) {
    xStreamBufferSendFromISR(uart->rx_ringbuf_handle, tmp_buf, length,
...
int vfs_uart_open(inode_t *inode, file_t *fp) {
  ...
  if ((fp != NULL) && (fp->node != NULL)) {
    ...
    if (fp->node->refs == 1) {
      ...
      uart_dev->rx_ringbuf_handle =
          xStreamBufferCreate(uart_dev->rx_buf_size, 1);
      uart_dev->tx_ringbuf_handle =
          xStreamBufferCreate(uart_dev->tx_buf_size, 1);
      if (uart_dev->rx_ringbuf_handle == NULL ||
          uart_dev->tx_ringbuf_handle == NULL) {
        return -EINVAL;
      }
...
ssize_t vfs_uart_read(file_t *fp, void *buf, size_t nbytes) {
  ...
  if ((fp != NULL) && (fp->node != NULL)) {
    ...
    if ((nbytes > 0) && (uart_dev != NULL)) {
      ...
      /* block */
      timeout = (UART_READ_CFG_BLOCK == uart_dev->read_block_flag)
                    ? AOS_WAIT_FOREVER
                    : 0;

      while (1) {
        ret += xStreamBufferReceive(uart_dev->rx_ringbuf_handle, buf + ret,
                                    nbytes - ret, timeout);
...
int vfs_uart_poll(file_t *fp, bool setup, poll_notify_t notify,
...
int uart_ioctl_cmd_waimode(uart_dev_t *uart_dev, int cmd, unsigned long arg) {
  ...
  while (1) {
    ...
    ret += xStreamBufferReceive(uart_dev->rx_ringbuf_handle,
                                (uint8_t *)waitr_arg->buf + ret, nbytes - ret,
                                timeout);
    if ((ret == nbytes) || (timeout == 0)) {
      break;
    }
