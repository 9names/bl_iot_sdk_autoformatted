 ...
 *
 * @return  The number of bytes read, 0 at end of file, negative error on
 * failure.
 */
 ...
 * @param[in]  arg  Argument to the command, interpreted according to the
 * command.
 *
 ...
 * For every file descriptor provided, poll() examines it for any events
 * registered for that particular file descriptor.
 *
 * @param[in]  fds      a point to the array of pollfd struct carrying a file
 * descriptor and bitmasks of events.
 * @param[in]  nfhs     number of file descriptors.
 * @param[in]  timeout  timer value to timeout or -1 for loop forever.
 *
 * @return  number of file descriptors selected (for which revents is non-zero).
 * 0 if timed out with nothing selected. -1 for error.
 */
...
/**
 * Obtain the current location associated with the directory stream specified by
 * dirp.
 *
 * @param[in]  dir  the handle of the directory.
 *
