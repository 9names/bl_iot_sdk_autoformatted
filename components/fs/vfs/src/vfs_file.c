...
static int file_cmp_cb(struct rb_tree *self, struct rb_node *node_a,
...
file_t *new_file(inode_t *node) {
  ...
  if (NULL == files_handle_dev ||
      MAX_FILE_NUM <= rb_tree_size(files_handle_dev->file_rb_tree)) {
    goto exit;
  }
...
int get_fd(file_t *file) { return file->fd; }
