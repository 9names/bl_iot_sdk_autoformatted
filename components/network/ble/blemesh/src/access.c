...
static void mod_publish(struct k_work *work) {
  ...
  if (pub->count) {
    ...
    if (err) {
      ...
      if (period_ms) {
        k_delayed_work_submit(&pub->timer, K_MSEC(period_ms));
      }
    }
...
u16_t bt_mesh_primary_addr(void) { return dev_primary_addr; }
...
u16_t *bt_mesh_model_find_group(struct bt_mesh_model **mod, u16_t addr) {
  ...
  bt_mesh_model_tree_walk(bt_mesh_model_root(*mod), find_group_mod_visitor,
                          &ctx);
...
u8_t bt_mesh_elem_count(void) { return dev_comp->elem_count; }
