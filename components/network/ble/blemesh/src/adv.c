...
void bt_mesh_adv_update(void) {
  ...
  // in order to post the adv thread to make it handle the enqueued msg, enqueue
...
struct net_buf *bt_mesh_adv_create(enum bt_mesh_adv_type type, u8_t xmit,
                                   s32_t timeout) {
  return bt_mesh_adv_create_from_pool(&adv_buf_pool, adv_alloc, type, xmit,
                                      timeout);
}
...
static void bt_mesh_scan_cb(const bt_addr_le_t *addr, s8_t rssi, u8_t adv_type,
...
void bt_mesh_adv_init(void) {
  ...
  k_thread_create(&adv_thread_data, adv_thread_stack,
                  K_THREAD_STACK_SIZEOF(adv_thread_stack), adv_thread, NULL,
