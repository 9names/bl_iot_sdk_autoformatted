...
static int secure_beacon_send(void) {
  ...
  for (i = 0; i < ARRAY_SIZE(bt_mesh.sub); i++) {
    ...
    if (time_diff < K_SECONDS(600) && time_diff < BEACON_THRESHOLD(sub)) {
      continue;
    }
    ...
    BT_PTS("[PTS] Sending secure network beacon (Flags = 0x%02X, IV Index = "
           "0x%08X) %u",
           bt_mesh_net_flags(sub), bt_mesh.iv_index, ++pts_cnt);
#endif
