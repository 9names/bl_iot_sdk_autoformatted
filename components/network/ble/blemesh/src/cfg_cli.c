...
static void comp_data_status(struct bt_mesh_model *model,
  ...
  BT_DBG("net_idx 0x%04x app_idx 0x%04x src 0x%04x len %u: %s", ctx->net_idx,
         ctx->app_idx, ctx->addr, buf->len, bt_hex(buf->data, buf->len));
...
static void state_status_u8(struct bt_mesh_model *model,
                            struct bt_mesh_msg_ctx *ctx,
                            struct net_buf_simple *buf, u32_t expect_status) {
  u8_t *status;
...
static void lpn_poll_timeout_status(struct bt_mesh_model *model,
  ...
  *param->poll_timeout =
      net_buf_simple_pull_le16(buf) + (net_buf_simple_pull_u8(buf) << 16);
...
/* Add by bouffalo */
int bt_mesh_cfg_krp_get(u16_t net_idx, u16_t addr, u8_t *status,
                        struct bt_mesh_cfg_krp *krp) {
...
/* Add by bouffalo */
int bt_mesh_cfg_krp_set(u16_t net_idx, u16_t addr, u8_t *status,
                        struct bt_mesh_cfg_krp *krp) {
...
int bt_mesh_cfg_net_key_del(u16_t net_idx, u16_t addr, u16_t key_net_idx,
                            u8_t *status) {
...
int bt_mesh_cfg_mod_sub_del_all(u16_t net_idx, u16_t addr, u16_t elem_addr,
  ...
  return mod_sub_del(OP_MOD_SUB_DEL_ALL, net_idx, addr, elem_addr, mod_id,
                     CID_NVAL, status);
}
...
int bt_mesh_cfg_mod_sub_del_all_vnd(u16_t net_idx, u16_t addr, u16_t elem_addr,
  ...
  return mod_sub_del(OP_MOD_SUB_DEL, net_idx, addr, elem_addr, mod_id, cid,
                     status);
}
...
int bt_mesh_cfg_mod_sub_va_overwrite(u16_t net_idx, u16_t addr, u16_t elem_addr,
                                     const u8_t label[16], u16_t mod_id,
                                     u16_t *virt_addr, u8_t *status) {
...
static int mod_pub_get(u16_t net_idx, u16_t addr, u16_t elem_addr, u16_t mod_id,
                       u16_t cid, struct bt_mesh_cfg_mod_pub *pub,
                       u8_t *status) {
...
int bt_mesh_cfg_mod_pub_va_set(u16_t net_idx, u16_t addr, u16_t elem_addr,
                               const u8_t label[16], u16_t mod_id,
                               struct bt_mesh_cfg_mod_pub *pub, u8_t *status) {
  return mod_pub_va_set(net_idx, addr, elem_addr, label, mod_id, CID_NVAL, pub,
                        status);
}
...
int bt_mesh_cfg_mod_pub_va_set_vnd(u16_t net_idx, u16_t addr, u16_t elem_addr,
  ...
  return mod_pub_va_set(net_idx, addr, elem_addr, label, mod_id, cid, pub,
                        status);
}
...
s32_t bt_mesh_cfg_cli_timeout_get(void) { return msg_timeout; }

void bt_mesh_cfg_cli_timeout_set(s32_t timeout) { msg_timeout = timeout; }

/* Added by bouffalo */
int bt_mesh_cfg_node_identity_set(u16_t net_idx, u16_t addr, u8_t *status,
                                  u16_t *NetKeyIndex, u8_t *Identity) {
...
/* Added by bouffalo */
int bt_mesh_cfg_node_identity_get(u16_t net_idx, u16_t addr, u8_t *status,
                                  u16_t *NetKeyIndex, u8_t *Identity) {
...
/* Added by bouffalo */
int bt_mesh_cfg_network_transmit_set(u16_t net_idx, u16_t addr, u8_t *count,
                                     u8_t *interval) {
...
/* Added by bouffalo */
int bt_mesh_cfg_network_transmit_get(u16_t net_idx, u16_t addr, u8_t *count,
                                     u8_t *interval) {
...
/* Added by bouffalo */
int bt_mesh_cfg_lpn_poll_timeout_get(u16_t net_idx, u16_t addr, u16_t *lpn_addr,
                                     u32_t *poll_timeout) {
