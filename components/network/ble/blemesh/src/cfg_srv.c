...
static u8_t _mod_pub_set(struct bt_mesh_model *model, u16_t pub_addr,
  ...
  if (model->pub->update) {
    ...
    if (period_ms > 0) {
      k_delayed_work_submit(&model->pub->timer, K_MSEC(period_ms));
    } else {
      k_delayed_work_cancel(&model->pub->timer);
    }
  }
...
static void relay_set(struct bt_mesh_model *model, struct bt_mesh_msg_ctx *ctx,
  ...
  if (buf->data[0] == 0x00 || buf->data[0] == 0x01) {
    ...
    BT_DBG("Relay 0x%02x (%s) xmit 0x%02x (count %u interval %u)", cfg->relay,
           change ? "changed" : "not changed", cfg->relay_retransmit,
...
static void send_mod_pub_status(struct bt_mesh_model *cfg_mod,
                                struct bt_mesh_msg_ctx *ctx, u16_t elem_addr,
                                u16_t pub_addr, bool vnd,
                                struct bt_mesh_model *mod, u8_t status,
                                u8_t *mod_id) {
...
static struct label *va_find(const u8_t *label_uuid, struct label **free_slot) {
...
static void mod_sub_del_all(struct bt_mesh_model *model,
  ...
  bt_mesh_model_tree_walk(bt_mesh_model_root(mod), mod_sub_clear_visitor, NULL);
...
send_status:
  send_mod_sub_status(model, ctx, status, elem_addr, BT_MESH_ADDR_UNASSIGNED,
                      mod_id, vnd);
}
...
static void send_net_key_status(struct bt_mesh_model *model,
                                struct bt_mesh_msg_ctx *ctx, u16_t idx,
                                u8_t status) {
...
static void create_mod_app_status(struct net_buf_simple *msg,
                                  ...
                                  u16_t elem_addr, u16_t app_idx, u8_t status,
                                  u8_t *mod_id) {
...
static void mod_app_get(struct bt_mesh_model *model,
  ...
  NET_BUF_SIMPLE_DEFINE(
      msg, MAX(BT_MESH_MODEL_BUF_LEN(OP_VND_MOD_APP_LIST, 9 + KEY_LIST_LEN),
...
#if defined(BFLB_BLE)
  /*Fix issue:for proxy node, OP_NODE_RESET_STATUS cannot be sent out because le
   *link is disconnected before sending OP_NODE_RESET_STATUS. delay the task to
   *make node reset later to make sure OP_NODE_RESET_STATUS can be sent out.
   */
  vTaskDelay(500);
#endif
  if (!IS_ENABLED(CONFIG_BT_MESH_GATT_PROXY)) {
...
static void send_krp_status(struct bt_mesh_model *model,
                            struct bt_mesh_msg_ctx *ctx, u16_t idx, u8_t phase,
                            u8_t status) {
...
void bt_mesh_heartbeat(u16_t src, u16_t dst, u8_t hops, u16_t feat) {
  ...
  BT_DBG("src 0x%04x dst 0x%04x hops %u min %u max %u count %u", src, dst, hops,
...
void bt_mesh_hb_pub_disable(void) { hb_pub_disable(conf); }
