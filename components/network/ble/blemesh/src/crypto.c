...
int bt_mesh_aes_cmac(const u8_t key[16], struct bt_mesh_sg *sg, size_t sg_len,
                     u8_t mac[16]) {
...
int bt_mesh_k2(const u8_t n[16], const u8_t *p, size_t p_len, u8_t net_id[1],
               u8_t enc_key[16], u8_t priv_key[16]) {
...
static int bt_mesh_ccm_decrypt(const u8_t key[16], u8_t nonce[13],
                               ...
                               const u8_t *aad, size_t aad_len, u8_t *out_msg,
                               size_t mic_size) {
...
static int bt_mesh_ccm_encrypt(const u8_t key[16], u8_t nonce[13],
                               const u8_t *msg, size_t msg_len, const u8_t *aad,
                               size_t aad_len, u8_t *out_msg, size_t mic_size) {
  ...
  for (j = 0; j < blk_cnt; j++) {
    if (j + 1 == blk_cnt) {
      ...
      /* Encrypted = Payload[0-15] ^ C_1 */
      for (i = 0; i < last_blk; i++) {
        out_msg[(j * 16) + i] = msg[(j * 16) + i] ^ cmsg[i];
      }
    } else {
      /* X_1 = e(AppKey, X_0 ^ Payload[0-15]) */
      for (i = 0; i < 16; i++) {
...
static void create_net_nonce(u8_t nonce[13], const u8_t *pdu, u32_t iv_index) {
  /* Nonce Type */
  nonce[0] = 0x00;
...
int bt_mesh_net_encrypt(const u8_t key[16], struct net_buf_simple *buf,
  ...
  BT_DBG("IVIndex %u EncKey %s mic_len %u", iv_index, bt_hex(key, 16), mic_len);
...
int bt_mesh_net_decrypt(const u8_t key[16], struct net_buf_simple *buf,
  ...
  BT_DBG("iv_index %u, key %s mic_len %u", iv_index, bt_hex(key, 16), mic_len);
...
int bt_mesh_app_encrypt(const u8_t key[16], bool dev_key, u8_t aszmic,
                        struct net_buf_simple *buf, const u8_t *ad, u16_t src,
  ...
  err = bt_mesh_ccm_encrypt(key, nonce, buf->data, buf->len, ad, ad ? 16 : 0,
...
int bt_mesh_app_decrypt(const u8_t key[16], bool dev_key, u8_t aszmic,
  ...
  err = bt_mesh_ccm_decrypt(key, nonce, buf->data, buf->len, ad, ad ? 16 : 0,
