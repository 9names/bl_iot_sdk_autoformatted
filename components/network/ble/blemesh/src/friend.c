...
#define FRIEND_BUF_COUNT                                                       \
  ((CONFIG_BT_MESH_FRIEND_QUEUE_SIZE + 1) * CONFIG_BT_MESH_FRIEND_LPN_COUNT)
...
int bt_mesh_friend_clear(struct bt_mesh_net_rx *rx,
...
static int unseg_app_sdu_decrypt(struct bt_mesh_friend *frnd,
  ...
  return bt_mesh_app_decrypt(
      meta->key, meta->is_dev_key, 0, &sdu, &sdu, meta->ad, meta->net.ctx.addr,
      meta->net.ctx.recv_dst, meta->net.seq, BT_MESH_NET_IVI_TX);
}
...
static int unseg_app_sdu_encrypt(struct bt_mesh_friend *frnd,
  ...
  return bt_mesh_app_encrypt(meta->key, meta->is_dev_key, 0, &sdu, meta->ad,
                             meta->net.ctx.addr, meta->net.ctx.recv_dst,
...
static void enqueue_friend_pdu(struct bt_mesh_friend *frnd,
                               enum bt_mesh_friend_pdu_type type, u16_t src,
  ...
  u16_t seq_zero =
      (((buf->data[10] << 8 | buf->data[11]) >> 2) & TRANS_SEQ_ZERO_MASK);
