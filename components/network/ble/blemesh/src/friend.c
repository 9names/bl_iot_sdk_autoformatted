...
static struct bt_mesh_adv *adv_alloc(int id) { return &adv_pool[id].adv; }
...
static void send_friend_clear(struct bt_mesh_friend *frnd) {
  ...
  bt_mesh_ctl_send(&tx, TRANS_CTL_OP_FRIEND_CLEAR, &req, sizeof(req), NULL,
                   &clear_sent_cb, frnd);
}
...
int bt_mesh_friend_req(struct bt_mesh_net_rx *rx, struct net_buf_simple *buf) {
  ...
  poll_to = (((u32_t)msg->poll_to[0] << 16) | ((u32_t)msg->poll_to[1] << 8) |
             ((u32_t)msg->poll_to[2]));
