...
int bt_mesh_local_model_bind(u16_t net_idx, u16_t mod_app_idx) {
  ...
  for (i = 0; i < dev_comp->elem_count; i++) {
    ...
    for (j = 0; j < elem->model_count; j++) {
      ...
      err = bt_mesh_cfg_mod_app_bind(net_idx, addr, addr + i, mod_app_idx,
                                     model->id, &status);
    }
    ...
    for (j = 0; j < elem->vnd_model_count; j++) {
      ...
      err = bt_mesh_cfg_mod_app_bind_vnd(net_idx, addr, addr + i, mod_app_idx,
                                         model->vnd.id, model->vnd.company,
                                         &status);
    }
  }
  return err;
