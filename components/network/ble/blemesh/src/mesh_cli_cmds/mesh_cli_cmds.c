...
u8_t dev_uuid[16] = {0xA8, 0x01, 0x71, 0x5e, 0x1c, 0x00, 0x00, 0xe4,
                     0x46, 0x46, 0x63, 0xa7, 0xf8, 0x02, 0x00, 0x00};
u8_t auth_value[16] = {0x78, 0x8A, 0xE3, 0xEE, 0x0F, 0x2A, 0x7E, 0xFA,
                       0xD3, 0x67, 0x35, 0x81, 0x41, 0xFE, 0x1B, 0x06};
#else
u8_t dev_uuid[16] = {0xA8, 0x01, 0x71, 0xe0, 0x1a, 0x00, 0x00, 0x0f,
                     0x7e, 0x35, 0x63, 0xa7, 0xf8, 0x02, 0x00, 0x00};
u8_t auth_value[16] = {0x7f, 0x80, 0x1a, 0xf4, 0xa0, 0x8c, 0x50, 0x39,
                       0xae, 0x7d, 0x7b, 0x44, 0xa0, 0x92, 0xd9, 0xc2};
#endif
...
#else
#define vOutputString(...)                                                     \
  bl_print(SYSTEM_UART_ID, PRINT_MODULE_CLI, __VA_ARGS__)
#endif
...
static int fault_get_reg(struct bt_mesh_model *model, u16_t cid, u8_t *test_id,
...
static void blemesh_init(char *pcWriteBuffer, int xWriteBufferLen, int argc,
                         char **argv);
#if defined(CONFIG_BT_MESH_PROVISIONER)
static void blemesh_pvnr_init(char *pcWriteBuffer, int xWriteBufferLen,
                              int argc, char **argv);
#endif
static void blemesh_set_dev_uuid(char *pcWriteBuffer, int xWriteBufferLen,
                                 int argc, char **argv);
static void blemesh_input_num(char *pcWriteBuffer, int xWriteBufferLen,
                              int argc, char **argv);
static void blemesh_input_str(char *pcWriteBuffer, int xWriteBufferLen,
                              int argc, char **argv);
#if defined(CONFIG_BT_MESH_MODEL)
#if defined(CONFIG_BT_MESH_MODEL_GEN_CLI)
static void blemesh_gen_oo_cli(char *pcWriteBuffer, int xWriteBufferLen,
                               int argc, char **argv);
#endif
#if defined(CONFIG_BT_MESH_MODEL_LIGHT_CLI)
static void blemesh_light_lgn_cli(char *pcWriteBuffer, int xWriteBufferLen,
                                  int argc, char **argv);
static void blemesh_light_ctl_cli(char *pcWriteBuffer, int xWriteBufferLen,
                                  int argc, char **argv);
static void blemesh_light_hsl_cli(char *pcWriteBuffer, int xWriteBufferLen,
                                  int argc, char **argv);
#endif
#if defined(CONFIG_BT_MESH_MODEL_VENDOR_CLI)
static void blemesh_vendor_cli(char *pcWriteBuffer, int xWriteBufferLen,
                               int argc, char **argv);
#endif
#endif /* CONFIG_BT_MESH_MODEL */
static void blemesh_pb(char *pcWriteBuffer, int xWriteBufferLen, int argc,
                       char **argv);
#if defined(CONFIG_BT_MESH_PB_ADV)
#if defined(CONFIG_BT_MESH_PROVISIONER)
static void blemesh_provision_adv(char *pcWriteBuffer, int xWriteBufferLen,
                                  int argc, char **argv);
#endif /* CONFIG_BT_MESH_PROVISIONER */
#endif /* CONFIG_BT_MESH_PB_ADV */
static void blemesh_reset(char *pcWriteBuffer, int xWriteBufferLen, int argc,
                          char **argv);
static void blemesh_net_send(char *pcWriteBuffer, int xWriteBufferLen, int argc,
                             char **argv);
static void blemesh_seg_send(char *pcWriteBuffer, int xWriteBufferLen, int argc,
                             char **argv);
static void blemesh_rpl_clr(char *pcWriteBuffer, int xWriteBufferLen, int argc,
                            char **argv);
static void blemesh_ivu_test(char *pcWriteBuffer, int xWriteBufferLen, int argc,
                             char **argv);
static void blemesh_iv_update(char *pcWriteBuffer, int xWriteBufferLen,
                              int argc, char **argv);
static void blemesh_fault_set(char *pcWriteBuffer, int xWriteBufferLen,
                              int argc, char **argv);

#if defined(CONFIG_BT_MESH_LOW_POWER)
static void blemesh_lpn_set(char *pcWriteBuffer, int xWriteBufferLen, int argc,
                            char **argv);
#endif

#if defined(CONFIG_BT_MESH_CDB)
static void blemesh_cdb_create(char *pcWriteBuffer, int xWriteBufferLen,
                               int argc, char *argv[]);
static void blemesh_cdb_clear(char *pcWriteBuffer, int xWriteBufferLen,
                              int argc, char *argv[]);
static void blemesh_cdb_show(char *pcWriteBuffer, int xWriteBufferLen, int argc,
                             char *argv[]);
static void blemesh_cdb_node_add(char *pcWriteBuffer, int xWriteBufferLen,
                                 int argc, char *argv[]);
static void blemesh_cdb_node_del(char *pcWriteBuffer, int xWriteBufferLen,
                                 int argc, char *argv[]);
static void blemesh_cdb_subnet_add(char *pcWriteBuffer, int xWriteBufferLen,
                                   int argc, char *argv[]);
static void blemesh_cdb_subnet_del(char *pcWriteBuffer, int xWriteBufferLen,
                                   int argc, char *argv[]);
static void blemesh_cdb_app_key_add(char *pcWriteBuffer, int xWriteBufferLen,
                                    int argc, char *argv[]);
static void blemesh_cdb_app_key_del(char *pcWriteBuffer, int xWriteBufferLen,
                                    int argc, char *argv[]);
#endif /* CONFIG_BT_MESH_CDB */
#if defined(CONFIG_BT_MESH_PROVISIONER)
static void blemesh_beacon_listen(char *pcWriteBuffer, int xWriteBufferLen,
                                  int argc, char *argv[]);
static void blemesh_provision(char *pcWriteBuffer, int xWriteBufferLen,
                              int argc, char *argv[]);
static void blemesh_node_cfg(char *pcWriteBuffer, int xWriteBufferLen, int argc,
                             char *argv[]);
#endif /* CONFIG_BT_MESH_PROVISIONER */
#if defined(CONFIG_BT_MESH_PROVISIONER)
static void blemesh_get_comp(char *pcWriteBuffer, int xWriteBufferLen, int argc,
                             char *argv[]);
static void blemesh_set_dst(char *pcWriteBuffer, int xWriteBufferLen, int argc,
                            char *argv[]);
static void blemesh_krp_get(char *pcWriteBuffer, int xWriteBufferLen, int argc,
                            char *argv[]);
static void blemesh_krp_set(char *pcWriteBuffer, int xWriteBufferLen, int argc,
                            char *argv[]);
static void blemesh_cfg_bcn_get(char *pcWriteBuffer, int xWriteBufferLen,
                                int argc, char *argv[]);
static void blemesh_cfg_bcn_set(char *pcWriteBuffer, int xWriteBufferLen,
                                int argc, char *argv[]);
static void blemesh_cfg_dttl_get(char *pcWriteBuffer, int xWriteBufferLen,
                                 int argc, char *argv[]);
static void blemesh_cfg_dttl_set(char *pcWriteBuffer, int xWriteBufferLen,
                                 int argc, char *argv[]);
static void blemesh_cfg_gpxy_get(char *pcWriteBuffer, int xWriteBufferLen,
                                 int argc, char *argv[]);
static void blemesh_cfg_gpxy_set(char *pcWriteBuffer, int xWriteBufferLen,
                                 int argc, char *argv[]);
static void blemesh_friend(char *pcWriteBuffer, int xWriteBufferLen, int argc,
                           char *argv[]);
static void blemesh_relay(char *pcWriteBuffer, int xWriteBufferLen, int argc,
                          char *argv[]);
static void blemesh_node_identify(char *pcWriteBuffer, int xWriteBufferLen,
                                  int argc, char *argv[]);
static void blemesh_node_reset(char *pcWriteBuffer, int xWriteBufferLen,
                               int argc, char *argv[]);
static void blemesh_network_trans(char *pcWriteBuffer, int xWriteBufferLen,
                                  int argc, char *argv[]);
static void blemesh_lpn_timeout_get(char *pcWriteBuffer, int xWriteBufferLen,
                                    int argc, char *argv[]);
static void blemesh_net_key_add(char *pcWriteBuffer, int xWriteBufferLen,
                                int argc, char *argv[]);
/* Added by bouffalo */
static void blemesh_net_key_update(char *pcWriteBuffer, int xWriteBufferLen,
                                   int argc, char *argv[]);
static void blemesh_net_key_get(char *pcWriteBuffer, int xWriteBufferLen,
                                int argc, char *argv[]);
static void blemesh_net_key_del(char *pcWriteBuffer, int xWriteBufferLen,
                                int argc, char *argv[]);
static void blemesh_app_key_add(char *pcWriteBuffer, int xWriteBufferLen,
                                int argc, char *argv[]);
/* Added by bouffalo */
static void blemesh_app_key_update(char *pcWriteBuffer, int xWriteBufferLen,
                                   int argc, char *argv[]);
static void blemesh_app_key_get(char *pcWriteBuffer, int xWriteBufferLen,
                                int argc, char *argv[]);
static void blemesh_app_key_del(char *pcWriteBuffer, int xWriteBufferLen,
                                int argc, char *argv[]);
/* Added by bouffalo */
static void blemesh_kr_update(char *pcWriteBuffer, int xWriteBufferLen,
                              int argc, char *argv[]);
static void blemesh_mod_app_bind(char *pcWriteBuffer, int xWriteBufferLen,
                                 int argc, char *argv[]);
static void blemesh_mod_app_unbind(char *pcWriteBuffer, int xWriteBufferLen,
                                   int argc, char *argv[]);
static void blemesh_mod_app_get(char *pcWriteBuffer, int xWriteBufferLen,
                                int argc, char *argv[]);
static void blemesh_mod_sub_add(char *pcWriteBuffer, int xWriteBufferLen,
                                int argc, char *argv[]);
static void blemesh_mod_sub_ow(char *pcWriteBuffer, int xWriteBufferLen,
                               int argc, char *argv[]);
static void blemesh_mod_sub_del(char *pcWriteBuffer, int xWriteBufferLen,
                                int argc, char *argv[]);
static void blemesh_mod_sub_del_all(char *pcWriteBuffer, int xWriteBufferLen,
                                    int argc, char *argv[]);
static void blemesh_mod_sub_add_va(char *pcWriteBuffer, int xWriteBufferLen,
                                   int argc, char *argv[]);
static void blemesh_mod_sub_ow_va(char *pcWriteBuffer, int xWriteBufferLen,
                                  int argc, char *argv[]);
static void blemesh_mod_sub_del_va(char *pcWriteBuffer, int xWriteBufferLen,
                                   int argc, char *argv[]);
static void blemesh_mod_sub_get(char *pcWriteBuffer, int xWriteBufferLen,
                                int argc, char *argv[]);
static void blemesh_mod_pub(char *pcWriteBuffer, int xWriteBufferLen, int argc,
                            char *argv[]);
/* Added by bouffalo */
static void blemesh_mod_pub_va(char *pcWriteBuffer, int xWriteBufferLen,
                               int argc, char *argv[]);
static void blemesh_hb_sub(char *pcWriteBuffer, int xWriteBufferLen, int argc,
                           char *argv[]);
static void blemesh_hb_pub(char *pcWriteBuffer, int xWriteBufferLen, int argc,
                           char *argv[]);

static void blemesh_clhm_fault(char *pcWriteBuffer, int xWriteBufferLen,
                               int argc, char *argv[]);
static void blemesh_clhm_period(char *pcWriteBuffer, int xWriteBufferLen,
                                int argc, char *argv[]);
static void blemesh_clhm_ats(char *pcWriteBuffer, int xWriteBufferLen, int argc,
                             char *argv[]);
#endif
#if defined(CFG_NODE_SEND_CFGCLI_MSG) && defined(CONFIG_BT_MESH_CDB)
static void blemesh_pvnr_devkey(char *pcWriteBuffer, int xWriteBufferLen,
                                int argc, char *argv[]);
#endif
#if defined(CONFIG_BT_MESH_SYNC)
static void blemesh_sync_start(char *pcWriteBuffer, int xWriteBufferLen,
                               int argc, char *argv[]);
static void blemesh_sync_stop(char *pcWriteBuffer, int xWriteBufferLen,
                              int argc, char *argv[]);
#endif

#if defined(CONFIG_BT_MESH_PROVISIONER)
static void print_node_added(u16_t net_idx, u8_t uuid[16], u16_t addr,
                             u8_t num_elem);
#endif
...
#endif
#ifdef CONFIG_BT_MESH_PTS
    .input_actions = (BT_MESH_PUSH | BT_MESH_TWIST | BT_MESH_ENTER_NUMBER |
...
#endif
#if defined(CONFIG_BT_MESH_MODEL_LIGHT_CLI)
    BFL_BLE_MESH_MODEL_LIGHT_LIGHTNESS_CLI(&lightness_cli_pub,
                                           &lightness_client),
#endif
#if defined(CONFIG_BT_MESH_MODEL_LIGHT_SRV)
...
static struct bt_mesh_model vendor_models[] = {
    BT_MESH_MODEL_VND(BL_COMP_ID, BT_MESH_VND_MODEL_ID_DATA_SRV, vendor_data_op,
                      NULL, &vendor_data),
#if defined(CONFIG_BT_MESH_MODEL_VENDOR_CLI)
...
#if defined(CONFIG_BT_MESH_MODEL_GEN_CLI)
    {"blemesh_gen_oo_cli",
     "\r\blemesh_gen_oo_cli:[cmd op app_idx opcode msg_role addr net_idx op_en_t onoff tid trans_time delay]\r\n\
     []\r\n",
     blemesh_gen_oo_cli},
#endif
#if defined(CONFIG_BT_MESH_MODEL_LIGHT_CLI)
    {"blemesh_light_lgn_cli",
     "\r\blemesh_light_lgn_cli:[cmd op app_idx opcode msg_role addr net_idx op_en_t lightness tid trans_time delay]\r\n\
     []\r\n",
     blemesh_light_lgn_cli},
    {"blemesh_light_ctl_cli",
     "\r\blemesh_light_ctl_cli:[cmd op app_idx opcode msg_role addr net_idx op_en_t ctl_lightness ctl_temperatrue ctl_delta_uv tid trans_time delay]\r\n\
     []\r\n",
     blemesh_light_ctl_cli},
    {"blemesh_light_hsl_cli",
     "\r\blemesh_light_hsl_cli:[cmd op app_idx opcode msg_role addr net_idx op_en lightness hue saturation tid trans_time delay]\r\n\
     []\r\n",
     blemesh_light_hsl_cli},
#endif
#if defined(CONFIG_BT_MESH_MODEL_VENDOR_CLI)
    {"blemesh_vendor_cli",
     "\r\blemesh_vendor_cli:[cmd op app_idx opcode msg_role addr net_idx]\r\n\
		[]\r\n",
     blemesh_vendor_cli},
#endif
#endif /* CONFIG_BT_MESH_MODEL */
...
static void blemesh_pvnr_init(char *pcWriteBuffer, int xWriteBufferLen,
  ...
  err =
      bt_mesh_provision(net_key, BT_MESH_NET_PRIMARY, 0, 0, self_addr, dev_key);
  if (err == -EALREADY) {
    ...
    if (IS_ENABLED(CONFIG_BT_MESH_CDB)) {
      ...
      u8_t status;
      err = bt_mesh_cfg_app_key_add(net_idx, self_addr, net_idx, app_idx,
                                    app_key->keys[0].app_key, &status);
      if (err) {
...
static u8_t capabilities(prov_caps_t *prv_caps, prov_start_t *prv_start) {
  ...
  vOutputString(
      ">>>Provisionee pub_key_type[%x], Provisioner pub_key_type[%x]\n",
  ...
  if (prv_start->pub_key) {
    ...
    const u8_t prvee_key[] = {
        0xF4, 0x65, 0xE4, 0x3F, 0xF2, 0x3D, 0x3F, 0x1B, 0x9D, 0xC7, 0xDF,
        0xC0, 0x4D, 0xA8, 0x75, 0x81, 0x84, 0xDB, 0xC9, 0x66, 0x20, 0x47,
        0x96, 0xEC, 0xCF, 0x0D, 0x6C, 0xF5, 0xE1, 0x65, 0x00, 0xCC, 0x02,
        0x01, 0xD0, 0x48, 0xBC, 0xBB, 0xD8, 0x99, 0xEE, 0xEF, 0xC4, 0x24,
        0x16, 0x4E, 0x33, 0xC2, 0x01, 0xC2, 0xB0, 0x10, 0xCA, 0x6B, 0x4D,
        0x43, 0xA8, 0xA1, 0x55, 0xCA, 0xD8, 0xEC, 0xB2, 0x79};
    memcpy(&prv_start->pub_key_data, prvee_key,
           sizeof(prv_start->pub_key_data));
    vOutputString(
        ">>>Recved Provisionee's public key:%s\n",
  ...
  if (prv_caps->op_oob_size == 0 && prv_caps->ip_oob_size == 0) {
...
static void prov_input_complete(void) { vOutputString("Input complete\r\n"); }
...
#if defined(CONFIG_BT_MESH_MODEL)
#if defined(CONFIG_BT_MESH_MODEL_GEN_CLI)
void ble_mesh_generic_onoff_client_model_cb(
...
static void blemesh_gen_oo_cli(char *pcWriteBuffer, int xWriteBufferLen,
  ...
  if (strcmp(argv[1], "get") == 0) {
    ...
    err = bfl_ble_mesh_register_generic_client_callback(
        ble_mesh_generic_onoff_client_model_cb);
    if (err == BFL_OK) {
...
#if defined(CONFIG_BT_MESH_MODEL_LIGHT_CLI)
void ble_mesh_light_client_model_cb(
    bfl_ble_mesh_light_client_cb_event_t event,
  ...
  case BFL_BLE_MESH_LIGHT_CLIENT_GET_STATE_EVT: {
    ...
    case BFL_BLE_MESH_MODEL_OP_LIGHT_CTL_GET:
      if (param->error_code == BFL_OK) {
        vOutputString(
            "Light ctl Client:GetStatus,OK,ln[%x]ln_t[%x] tp[%x]tp_t[%x]\n",
  ...
  case BFL_BLE_MESH_LIGHT_CLIENT_SET_STATE_EVT: {
    ...
    case BFL_BLE_MESH_MODEL_OP_LIGHT_CTL_SET:
      if (param->error_code == BFL_OK) {
        vOutputString(
            "Light ctl Client:SetStatus,OK, ln[%x]ln_t[%x] tp[%x]tp_t[%x]\n",
            param->status_cb.ctl_status.present_ctl_lightness,
            param->status_cb.ctl_status.target_ctl_lightness,
            param->status_cb.ctl_status.present_ctl_temperature,
            param->status_cb.ctl_status.target_ctl_temperature);
...
static void blemesh_light_lgn_cli(char *pcWriteBuffer, int xWriteBufferLen,
  ...
  if (argc > 11) {
    ...
    /* trans_time */
    get_uint8_from_string(&argv[10],
                          &light_client_set.lightness_set.trans_time);
    /* delay */
  ...
  if (strcmp(argv[1], "get") == 0) {
    ...
    err = bfl_ble_mesh_register_light_client_callback(
        ble_mesh_light_client_model_cb);
    if (err == BFL_OK) {
...
static void blemesh_light_ctl_cli(char *pcWriteBuffer, int xWriteBufferLen,
  ...
  if (argc > 13) {
    ...
    get_uint16_from_string(&argv[10],
                           (uint16_t *)&light_client_set.ctl_set.ctl_delta_uv);
    /* tid */
...
static void blemesh_vendor_cli(char *pcWriteBuffer, int xWriteBufferLen,
  ...
  model_t = bt_mesh_model_find_vnd(elements, BL_COMP_ID,
                                   BT_MESH_VND_MODEL_ID_DATA_CLI);
  if (model_t == NULL) {
...
blemesh_ident(char *pcWriteBuffer, int xWriteBufferLen, int argc, char **argv) {
  int err;

  err = bt_mesh_proxy_identity_enable();
  if (err) {
    vOutputString("Failed advertise using Node Identity (err "
                  "%d)\r\n",
                  err);
  }
}
...
static void cdb_print_app_keys(void) {
  ...
  for (i = 0; i < ARRAY_SIZE(bt_mesh_cdb.app_keys); ++i) {
    ...
    vOutputString("0x%03x   0x%03x   %s\n", app_key->net_idx, app_key->app_idx,
                  key_hex_str);
  }
...
static void configure_node(struct bt_mesh_cdb_node *node) {
  ...
  err = bt_mesh_cfg_mod_pub_set(net_idx, node->addr, node->addr,
                                BT_MESH_MODEL_ID_HEALTH_SRV, &pub, &status);
  if (err < 0) {
...
static void blemesh_get_comp(char *pcWriteBuffer, int xWriteBufferLen, int argc,
  ...
  int err = bt_mesh_cfg_comp_data_get(net_idx, dst, page, &status, &comp_buf);
  if (err) {
...
static void blemesh_net_key_add(char *pcWriteBuffer, int xWriteBufferLen,
  ...
  if (IS_ENABLED(CONFIG_BT_MESH_CDB)) {
    ...
    if (subnet) {
      if (has_key_val) {
        vOutputString("Subnet 0x%03x already has a value", key_net_idx);
        return;
      }
  ...
  if (status) {
    vOutputString("NetKeyAdd failed with status 0x%02x", status);
  } else {
    vOutputString("NetKey added with NetKey Index 0x%03x", key_net_idx);
  }
...
static void blemesh_net_key_update(char *pcWriteBuffer, int xWriteBufferLen,
  ...
  if (status) {
    vOutputString("NetKeyUpdate failed with status 0x%02x", status);
  } else {
    vOutputString("NetKey updated with NetKey Index 0x%03x", key_net_idx);
  }
  return;
}
...
static void blemesh_net_key_del(char *pcWriteBuffer, int xWriteBufferLen,
  ...
  if (status) {
    vOutputString("NetKeyDel failed with status 0x%02x", status);
  } else {
...
static void blemesh_app_key_add(char *pcWriteBuffer, int xWriteBufferLen,
  ...
  if (IS_ENABLED(CONFIG_BT_MESH_CDB)) {
    ...
    if (app_key) {
      if (has_key_val) {
        vOutputString("App key 0x%03x already has a value", key_app_idx);
        return;
      }
  ...
  err = bt_mesh_cfg_app_key_add(net_idx, dst, key_net_idx, key_app_idx, key_val,
                                &status);
  if (err) {
    vOutputString("Unable to send App Key Add (err %d)", err);
    return;
  ...
  if (status) {
    vOutputString("AppKeyAdd failed with status 0x%02x", status);
  } else {
...
static void blemesh_app_key_update(char *pcWriteBuffer, int xWriteBufferLen,
  ...
  err = bt_mesh_cfg_app_key_update(net_idx, dst, key_net_idx, key_app_idx,
                                   key_val, &status);
  if (err) {
    vOutputString("Unable to send App Key Add (err %d)", err);
    return;
...
static void blemesh_app_key_get(char *pcWriteBuffer, int xWriteBufferLen,
  ...
  err = bt_mesh_cfg_app_key_get(net_idx, dst, key_net_idx, &status, keys, &cnt);
  if (err) {
  ...
  if (status) {
    vOutputString("AppKeyGet failed with status 0x%02x", status);
    return;
  }

  vOutputString("AppKeys for NetKey 0x%03x known by 0x%04x:", net_idx, dst);
  for (i = 0; i < cnt; i++) {
    vOutputString("\t0x%03x", keys[i]);
  }
...
static void blemesh_app_key_del(char *pcWriteBuffer, int xWriteBufferLen,
  ...
  err =
      bt_mesh_cfg_app_key_del(net_idx, dst, key_net_idx, key_app_idx, &status);
  if (err) {
  ...
  if (status) {
    vOutputString("AppKeyDel failed with status 0x%02x", status);
  } else {
...
static void blemesh_mod_app_bind(char *pcWriteBuffer, int xWriteBufferLen,
  ...
  if (argc > 6) {
    ...
    err = bt_mesh_cfg_mod_app_bind_vnd(net_idx, dst, elem_addr, mod_app_idx,
                                       mod_id, cid, &status);
  } else {
    err = bt_mesh_cfg_mod_app_bind(net_idx, dst, elem_addr, mod_app_idx, mod_id,
                                   &status);
  }

  if (err) {
    vOutputString("Unable to send Model App Bind (err %d)", err);
    return;
  }

  if (status) {
    vOutputString("Model App Bind failed with status 0x%02x", status);
  } else {
...
static void blemesh_mod_app_unbind(char *pcWriteBuffer, int xWriteBufferLen,
  ...
  if (argc > 6) {
    get_uint16_from_string(&argv[6], &cid);
    err = bt_mesh_cfg_mod_app_unbind_vnd(net_idx, dst, elem_addr, mod_app_idx,
                                         mod_id, cid, &status);
  } else {
    err = bt_mesh_cfg_mod_app_unbind(net_idx, dst, elem_addr, mod_app_idx,
                                     mod_id, &status);
  }

  if (err) {
    vOutputString("Unable to send Model App Unbind (err %d)", err);
    return;
  }

  if (status) {
    vOutputString("Model App Unbind failed with status 0x%02x", status);
  } else {
...
static void blemesh_mod_app_get(char *pcWriteBuffer, int xWriteBufferLen,
  ...
  if (argc > 5) {
    ...
    err = bt_mesh_cfg_mod_app_get_vnd(net_idx, dst, elem_addr, mod_id, cid,
                                      &status, apps, &cnt);
  } else {
    err = bt_mesh_cfg_mod_app_get(net_idx, dst, elem_addr, mod_id, &status,
                                  apps, &cnt);
  }
...
static void blemesh_mod_sub_add(char *pcWriteBuffer, int xWriteBufferLen,
  ...
  if (argc > 6) {
    get_uint16_from_string(&argv[6], &cid);
    err = bt_mesh_cfg_mod_sub_add_vnd(net_idx, dst, elem_addr, sub_addr, mod_id,
                                      cid, &status);
  } else {
    err = bt_mesh_cfg_mod_sub_add(net_idx, dst, elem_addr, sub_addr, mod_id,
                                  &status);
  }
...
static void blemesh_mod_sub_ow(char *pcWriteBuffer, int xWriteBufferLen,
  ...
  if (argc > 6) {
    get_uint16_from_string(&argv[6], &cid);
    err = bt_mesh_cfg_mod_sub_overwrite_vnd(net_idx, dst, elem_addr, sub_addr,
                                            mod_id, cid, &status);
  } else {
    err = bt_mesh_cfg_mod_sub_overwrite(net_idx, dst, elem_addr, sub_addr,
                                        mod_id, &status);
  }
...
static void blemesh_mod_sub_del(char *pcWriteBuffer, int xWriteBufferLen,
  ...
  if (argc > 6) {
    get_uint16_from_string(&argv[6], &cid);
    err = bt_mesh_cfg_mod_sub_del_vnd(net_idx, dst, elem_addr, sub_addr, mod_id,
                                      cid, &status);
  } else {
    err = bt_mesh_cfg_mod_sub_del(net_idx, dst, elem_addr, sub_addr, mod_id,
                                  &status);
  }
...
static void blemesh_mod_sub_del_all(char *pcWriteBuffer, int xWriteBufferLen,
  ...
  if (argc > 6) {
    get_uint16_from_string(&argv[6], &cid);
    err = bt_mesh_cfg_mod_sub_del_all_vnd(net_idx, dst, elem_addr, mod_id, cid,
                                          &status);
  } else {
...
static void blemesh_mod_sub_add_va(char *pcWriteBuffer, int xWriteBufferLen,
  ...
  if (argc > 6) {
    get_uint16_from_string(&argv[6], &cid);
    err = bt_mesh_cfg_mod_sub_va_add_vnd(net_idx, dst, elem_addr, label, mod_id,
                                         cid, &sub_addr, &status);
  } else {
    err = bt_mesh_cfg_mod_sub_va_add(net_idx, dst, elem_addr, label, mod_id,
                                     &sub_addr, &status);
  }
...
static void blemesh_mod_sub_ow_va(char *pcWriteBuffer, int xWriteBufferLen,
  ...
  if (argc > 6) {
    get_uint16_from_string(&argv[6], &cid);
    err = bt_mesh_cfg_mod_sub_va_overwrite_vnd(
        net_idx, dst, elem_addr, label, mod_id, cid, &virt_addr, &status);
  } else {
    err = bt_mesh_cfg_mod_sub_va_overwrite(net_idx, dst, elem_addr, label,
                                           mod_id, &virt_addr, &status);
  }
...
static void blemesh_mod_sub_del_va(char *pcWriteBuffer, int xWriteBufferLen,
  ...
  if (argc > 6) {
    get_uint16_from_string(&argv[6], &cid);
    err = bt_mesh_cfg_mod_sub_va_del_vnd(net_idx, dst, elem_addr, label, mod_id,
                                         cid, &sub_addr, &status);
  } else {
    err = bt_mesh_cfg_mod_sub_va_del(net_idx, dst, elem_addr, label, mod_id,
                                     &sub_addr, &status);
  }
...
static void blemesh_mod_sub_get(char *pcWriteBuffer, int xWriteBufferLen,
  ...
  if (argc > 5) {
    get_uint16_from_string(&argv[5], &cid);
    err = bt_mesh_cfg_mod_sub_get_vnd(net_idx, dst, elem_addr, mod_id, cid,
                                      &status, subs, &cnt);
  } else {
    err = bt_mesh_cfg_mod_sub_get(net_idx, dst, elem_addr, mod_id, &status,
                                  subs, &cnt);
  }
...
static void mod_pub_get(u16_t addr, u16_t mod_id, u16_t cid) {
  struct bt_mesh_cfg_mod_pub pub;
  u8_t status;
  int err;
...
static void mod_pub_set(u16_t addr, u16_t mod_id, u16_t cid, int argc,
                        char *argv[]) {
  struct bt_mesh_cfg_mod_pub pub;
  u8_t status, count;
  ...
  if (cid == CID_NVAL) {
    err = bt_mesh_cfg_mod_pub_set(net_idx, dst, addr, mod_id, &pub, &status);
  } else {
    err = bt_mesh_cfg_mod_pub_set_vnd(net_idx, dst, addr, mod_id, cid, &pub,
                                      &status);
  }
...
static void mod_pub_va_set(u16_t addr, u16_t mod_id, u16_t cid, int argc,
                           char *argv[]) {
  struct bt_mesh_cfg_mod_pub pub;
  u8_t status, count;
  u16_t interval, net_idx, dst;
  ...
  if (cid == CID_NVAL) {
    err = bt_mesh_cfg_mod_pub_va_set(net_idx, dst, addr, label, mod_id, &pub,
                                     &status);
  } else {
    err = bt_mesh_cfg_mod_pub_va_set(net_idx, dst, addr, label, mod_id, &pub,
                                     &status);
...
static void blemesh_krp_get(char *pcWriteBuffer, int xWriteBufferLen, int argc,
  ...
  err = bt_mesh_cfg_krp_get(net_idx, dst, &status, &krp_buf);
  if (err) {
...
static void blemesh_krp_set(char *pcWriteBuffer, int xWriteBufferLen, int argc,
  ...
  err = bt_mesh_cfg_krp_set(net_idx, dst, &status, &krp_buf);
  if (err) {
    vOutputString("Getting Key Refresh Phase failed (err %d)\n", err);
    return;
  }
...
static void blemesh_relay(char *pcWriteBuffer, int xWriteBufferLen, int argc,
  ...
  if (argc < 4) {
    ...
    err = bt_mesh_cfg_relay_set(net_idx, dst, val, new_transmit, &relay,
                                &transmit);
  }

  if (err) {
    vOutputString("Unable to send Relay Get/Set (err %d)", err);
    return;
  }
...
static void blemesh_node_identify(char *pcWriteBuffer, int xWriteBufferLen,
  ...
  if (argc == 5) {
    ...
    err = bt_mesh_cfg_node_identity_set(net_idx, dst, &status, &netkeyindex,
                                        &identity);
  } else if (argc == 4) {
    get_uint16_from_string(&argv[3], &netkeyindex);
    err = bt_mesh_cfg_node_identity_get(net_idx, dst, &status, &netkeyindex,
                                        &identity);
  }

  if (err) {
    vOutputString("Unable to node identity Get/Set (err %d)", err);
    return;
  }

  vOutputString("netkeyindex is %x, identity %x", netkeyindex, identity);
}
...
static void blemesh_network_trans(char *pcWriteBuffer, int xWriteBufferLen,
  ...
  if (argc == 5) {
    ...
    err = bt_mesh_cfg_network_transmit_set(net_idx, dst, &count, &interval);
  } else if (argc == 1) {
    err = bt_mesh_cfg_network_transmit_get(net_idx, dst, &count, &interval);
  }

  if (err) {
    vOutputString("Unable to Network Transmit Get/Set (err %d)", err);
    return;
  }

  vOutputString("count is %x, interval %x", count, interval);
}
...
static void blemesh_lpn_timeout_get(char *pcWriteBuffer, int xWriteBufferLen,
  ...
  err =
      bt_mesh_cfg_lpn_poll_timeout_get(net_idx, dst, &lpn_addr, &poll_timeout);

  if (err) {
    vOutputString("Unable to LPN Poll time out Get/Set (err %d)", err);
    return;
  }

  vOutputString("lpn_addr is %x, poll_timeout %x", lpn_addr, poll_timeout);
}
...
static void blemesh_clhm_fault(char *pcWriteBuffer, int xWriteBufferLen,
  ...
  if (0 == strcmp(argv[1], "test")) {
    ...
    if (argc == 6)
      /* Health Fault Test */
      err = bt_mesh_health_fault_test(dst, app_idx, cid, test_id, faults,
                                      &fault_count);
    else
      /* Health Fault Test Unacknowledged */
      err = bt_mesh_health_fault_test(dst, app_idx, cid, test_id, NULL,
                                      &fault_count);
  } else if (0 == strcmp(argv[1], "get")) {
    get_uint16_from_string(&argv[2], &dst);
    get_uint16_from_string(&argv[3], &app_idx);
    get_uint16_from_string(&argv[4], &cid);
    err = bt_mesh_health_fault_get(dst, app_idx, cid, &test_id, faults,
                                   &fault_count);
    ...
    if (argc == 5)
      /* Health Fault Clear */
      err = bt_mesh_health_fault_clear(dst, app_idx, cid, &test_id, faults,
                                       &fault_count);
    else
      /* Health Fault Clear Unacknowledged */
      err = bt_mesh_health_fault_clear(dst, app_idx, cid, NULL, faults,
                                       &fault_count);
  }

  if (err) {
    vOutputString("Unable to health fault Get/Set (err %d)", err);
    return;
  }
}
...
static void blemesh_clhm_period(char *pcWriteBuffer, int xWriteBufferLen,
  ...
  if (0 == strcmp(argv[1], "get")) {
    ...
    if (argc == 5)
      /* Acknowledged*/
      err = bt_mesh_health_period_set(dst, app_idx, divisor, &updated_divisor);
    else
      /* Unacknowledged */
      err = bt_mesh_health_period_set(dst, app_idx, divisor, NULL);
  }

  if (err) {
    vOutputString("Unable to health period Get/Set (err %d)", err);
    return;
  }
}
...
static void blemesh_clhm_ats(char *pcWriteBuffer, int xWriteBufferLen, int argc,
  ...
  if (0 == strcmp(argv[1], "get")) {
    ...
    if (argc == 5)
      ...
      /* Unacknowledged */
      err = bt_mesh_health_attention_set(dst, app_idx, attention, NULL);
  }

  if (err) {
    vOutputString("Unable to health attention Get/Set (err %d)", err);
    return;
  }
}
#endif
...
int blemesh_cli_register(void) {
  // static command(s) do NOT need to call aos_cli_register_command(s) to
  // register. However, calling aos_cli_register_command(s) here is OK but is of
  // no effect as cmds_user are included in cmds list.
  // XXX NOTE: Calling this *empty* function is necessary to make cmds_user in
  // this file to be kept in the final link.
  // aos_cli_register_commands(btStackCmdSet,
  // sizeof(btMeshCmdSet)/sizeof(btMeshCmdSet[0]));
  return 0;
}