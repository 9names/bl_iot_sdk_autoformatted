...
void bt_mesh_srv_update_last_rcvd_msg(struct bt_mesh_last_rcvd_msg *last_msg,
                                      u8_t tid, u16_t src_addr, u16_t dst_addr,
                                      int64_t rcvd_time) {
  last_msg->tid = tid;
...
bool bt_mesh_srv_check_rcvd_msg_with_last(
    u8_t tid, u16_t src_addr, u16_t dst_addr,

    int64_t *rcvd_time, struct bt_mesh_last_rcvd_msg *last_msg) {
  /*In 3.3.1.2.2 of MMDL spec, When a Generic OnOff Server receives a Generic
    OnOff Set message or a Generic OnOff Set Unacknowledged message, it shall
    set the Generic OnOff state to the OnOff field of the message, unless the
    message has the same value for the SRC, DST, and TID fields as the previous
