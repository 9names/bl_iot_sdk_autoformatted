...
static void send_onoff_status(struct bt_mesh_model *model,
                              struct bt_mesh_msg_ctx *ctx) {
  struct bt_mesh_gen_onoff_srv *srv = model->user_data;
  NET_BUF_SIMPLE_DEFINE(msg, MESH_GEN_OPCODE_SIZE_2 +
                                 GEN_ONOFF_STATUS_PAYLOAD_SIZE +
...
static void gen_onoff_set(struct bt_mesh_model *model,
  ...
  if (bt_mesh_srv_check_rcvd_msg_with_last(tid, ctx->addr, ctx->recv_dst,
                                           &recv_time, &srv->last_msg)) {
    return;
  }
  ...
  bt_mesh_srv_update_last_rcvd_msg(&srv->last_msg, tid, ctx->addr,
                                   ctx->recv_dst, recv_time);
}
...
void mesh_gen_srv_callback_register(bt_mesh_model_gen_cb cb) { gen_cb = cb; }
