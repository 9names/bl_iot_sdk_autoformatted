...
#define SEQ(pdu)                                                               \
  (((u32_t)(pdu)[2] << 16) | ((u32_t)(pdu)[3] << 8) | (u32_t)(pdu)[4]);
...
struct bt_mesh_net bt_mesh = {
    ...
    .sub = {[0 ...(CONFIG_BT_MESH_SUBNET_COUNT - 1)] =
                {
...
int friend_cred_set(struct friend_cred *cred, u8_t idx,
...
bool bt_mesh_net_iv_update(u32_t iv_index, bool iv_update) {
  ...
  if (atomic_test_bit(bt_mesh.flags, BT_MESH_IVU_IN_PROGRESS)) {
    ...
    if (iv_index < bt_mesh.iv_index || iv_index > bt_mesh.iv_index + 42) {
      BT_ERR("IV Index out of sync: 0x%08x != 0x%08x", iv_index,
             bt_mesh.iv_index);
      return false;
    }
...
int bt_mesh_net_send(struct bt_mesh_net_tx *tx, struct net_buf *buf,
  ...
  BT_DBG("src 0x%04x dst 0x%04x len %u headroom %zu tailroom %zu", tx->src,
         tx->ctx->addr, buf->len, net_buf_headroom(buf), net_buf_tailroom(buf));
   ...
   */
  if (IS_ENABLED(CONFIG_BT_MESH_GATT_PROXY) && tx->ctx->send_ttl != 1U) {
...
static bool auth_match(struct bt_mesh_subnet_keys *keys, const u8_t net_id[8],
                       u8_t flags, u32_t iv_index, const u8_t auth[8]) {
  u8_t net_auth[8];
  ...
  if (memcmp(auth, net_auth, 8)) {
    BT_WARN("Authentication Value %s != %s", bt_hex(auth, 8),
            bt_hex(net_auth, 8));
    return false;
  }
...
static int net_decrypt(struct bt_mesh_subnet *sub, const u8_t *enc,
                       const u8_t *priv, const u8_t *data, size_t data_len,
...
static int friend_decrypt(struct bt_mesh_subnet *sub, const u8_t *data,
  ...
  for (i = 0; i < ARRAY_SIZE(friend_cred); i++) {
    ...
    if (NID(data) == cred->cred[0].nid &&
        !net_decrypt(sub, cred->cred[0].enc, cred->cred[0].privacy, data,
                     data_len, rx, buf)) {
      return 0;
    }
    ...
    if (NID(data) == cred->cred[1].nid &&
        !net_decrypt(sub, cred->cred[1].enc, cred->cred[1].privacy, data,
                     data_len, rx, buf)) {
      rx->new_key = 1U;
      return 0;
    }
...
static bool net_find_and_decrypt(const u8_t *data, size_t data_len,
  ...
  for (i = 0; i < ARRAY_SIZE(bt_mesh.sub); i++) {
    ...
    if (NID(data) == sub->keys[0].nid &&
        !net_decrypt(sub, sub->keys[0].enc, sub->keys[0].privacy, data,
                     data_len, rx, buf)) {
      rx->ctx.net_idx = sub->net_idx;
      rx->sub = sub;
      return true;
    ...
    if (NID(data) == sub->keys[1].nid &&
        !net_decrypt(sub, sub->keys[1].enc, sub->keys[1].privacy, data,
                     data_len, rx, buf)) {
      rx->new_key = 1U;
      rx->ctx.net_idx = sub->net_idx;
      rx->sub = sub;
...
int bt_mesh_net_decode(struct net_buf_simple *data, enum bt_mesh_net_if net_if,
  ...
  if (net_if != BT_MESH_NET_IF_LOCAL) {
    ...
    BT_PTS("[PTS] - TransportPDU: [%s]",
           bt_hex(buf->data + BT_MESH_NET_HDR_LEN,
                  buf->len - BT_MESH_NET_HDR_LEN));
  }
#endif
