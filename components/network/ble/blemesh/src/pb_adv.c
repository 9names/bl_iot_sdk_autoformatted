...
static void prov_msg_recv(void) {
  ...
  if (atomic_test_bit(link.flags, LINK_INVALID)) {
    BT_WARN("Unexpected msg 0x%02x on invalidated link", link.rx.buf->data[0]);
...
static int bearer_ctl_send(u8_t op, const void *data, u8_t data_len,
  ...
  buf =
      adv_buf_create(reliable ? RETRANSMITS_RELIABLE : RETRANSMITS_UNRELIABLE);
  if (!buf) {
    return -ENOBUFS;
  }
...
static int prov_send_adv(struct net_buf_simple *msg,
  ...
  for (seg_id = 1U; msg->len > 0; seg_id++) {
    ...
    BT_DBG("seg %u len %u: %s", seg_id, seg_len, bt_hex(msg->data, seg_len));
