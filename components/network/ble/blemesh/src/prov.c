...
/** Add by bouffalo.
 * For provisiooner. Convert provisionee's start parameter to provisioner's
 * information provisionee's Output OOB authentication -> provisiooner's Input
 * OOB authentication.
 *
...
static void prov_capabilities(const u8_t *data) {
  ...
  link.provisioner.node =
      bt_mesh_cdb_node_alloc(link.provisioner.uuid, link.provisioner.addr,
                             data[0], link.provisioner.net_idx);
  if (link.provisioner.node == NULL) {
...
static int prov_auth(u8_t method, u8_t action, u8_t size) {
  ...
  case AUTH_METHOD_STATIC:
    ...
    memcpy(link.auth + 16 - prov->static_val_len, prov->static_val,
           prov->static_val_len);
    (void)memset(link.auth, 0, sizeof(link.auth) - prov->static_val_len);
    return 0;
  ...
  case AUTH_METHOD_OUTPUT:
    ...
    if (output == BT_MESH_DISPLAY_STRING) {
      ...
      (void)memset(link.auth + size, 0, sizeof(link.auth) - size);

      return prov->output_string((char *)str);
    } else {
      u32_t div[8] = {10,     100,     1000,     10000,
...
/** Add by bouffalo.
 * For provisiooner, Get public key sent callback. When provisionee use public
 * oob key. provisiooner needn't get provisonee's public key from provisioning
 * pdu.
 */
...
static void send_pub_key(void) {
  ...
  /** Modified by bouffalo.
   * For provisiooner. When provisionee use public oob key. we should get
   * provisioner had sent public key.
   */
 ...
 */
    ...
    prov_auth(prv_start.auth_method, prv_start.auth_action,
              prv_start.auth_size);
#endif
    /** Modified by bouffalo.
     * For provisioner.
     * Wait input complete pdu, When provisionee use input number/Alphanumeric
     * action.
     */
...
static void send_prov_data(void) {
  ...
  if (sub == NULL) {
    BT_ERR("No subnet with net_idx %u", link.provisioner.node->net_idx);
    prov_fail(PROV_ERR_UNEXP_ERR);
    return;
  }
  ...
  err = bt_mesh_prov_encrypt(session_key, nonce, &pdu.data[1], &pdu.data[1]);
  if (err) {
...
static void prov_complete(const u8_t *data) {
  ...
  if (prov->node_added) {
    prov->node_added(node->net_idx, node->uuid, node->addr, node->num_elem);
  }
#endif
}
...
static void prov_random(const u8_t *data) {
  ...
  if (bt_mesh_prov_salt(link.conf_salt, prov_rand, dev_rand, link.prov_salt)) {
...
static void prov_confirm(const u8_t *data) {
   ...
   * For provisioner. may be need't notify in this place(received Provisioning
   * Confirmation pdu), since provisioner will receive Provisioning Input
   * Complete.
   */
  notify_input_complete();
...
const struct bt_mesh_prov *bt_mesh_prov_get(void) { return prov; }

bool bt_prov_active(void) { return atomic_test_bit(link.flags, LINK_ACTIVE); }
