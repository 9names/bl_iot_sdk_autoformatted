...
static void prov_random(const u8_t *data) {
  ...
  if (bt_mesh_prov_salt(link.conf_salt, data, link.rand, link.prov_salt)) {
...
static void prov_complete(const u8_t *data) { BT_DBG(""); }

static void prov_failed(const u8_t *data) { BT_WARN("Error: 0x%02x", data[0]); }
...
#ifdef CONFIG_BT_MESH_PTS
    BT_PTS("[PTS] Link Open (UUID = %s) received and ignored",
           bt_hex(buf->data, 16));
#endif
...
static void prov_msg_recv(void) {
  ...
  if (1 + prov_handlers[type].len != link.rx.buf->len) {
    BT_ERR("Invalid length %u for type 0x%02x", link.rx.buf->len, type);
    prov_send_fail_msg(PROV_ERR_NVAL_FMT);
    return;
  }
...
static void gen_prov_cont(struct prov_rx *rx, struct net_buf_simple *buf) {
  ...
  if (rx->xact_id != link.rx.id) {
    BT_WARN("Data for unknown transaction (%u != %u)", rx->xact_id, link.rx.id);
    return;
  }
  ...
  if (seg > link.rx.last_seg) {
    ...
    expect_len = (link.rx.buf->len - 20U - ((link.rx.last_seg - 1) * 23U));
...
static void gen_prov_start(struct prov_rx *rx, struct net_buf_simple *buf) {
  ...
  if (link.rx.buf->len > link.rx.buf->size) {
    BT_ERR("Too large provisioning PDU (%u bytes)", link.rx.buf->len);
    prov_send_fail_msg(PROV_ERR_NVAL_FMT);
    return;
  }
