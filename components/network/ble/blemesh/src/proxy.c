...
#else
  err = bt_le_adv_start(&slow_adv_param, net_id_ad, ARRAY_SIZE(net_id_ad), NULL,
                        0);
#endif /*CONFIG_BLE_MULTI_ADV*/
  if (err) {
...
#if defined(CONFIG_BLE_MULTI_ADV)
    if (!bt_le_multi_adv_id_is_vaild(proxy_adv_id) &&
        bt_le_multi_adv_start(param, prov_ad, ARRAY_SIZE(prov_ad), prov_sd,
                              prov_sd_len, &proxy_adv_id) == 0) {
