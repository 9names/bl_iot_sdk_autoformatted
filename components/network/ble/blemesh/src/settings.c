...
static int mod_data_set(struct bt_mesh_model *mod, const char *name,
  ...
  if (mod->cb && mod->cb->settings_set) {
    return mod->cb->settings_set(mod, next, len_rd, read_cb, cb_arg);
  }
...
cdb_node_update_find(u16_t addr, struct node_update **free_slot) {
...
static void store_pending_mod_sub(struct bt_mesh_model *mod, bool vnd) {
  ...
  if (count) {
    /*Fix warnning by bouffalo */
    err = settings_save_one(path, (u8_t *)groups, count * sizeof(groups[0]));
  } else {
    err = settings_delete(path);
  }
...
static void store_pending_va(void) {
  ...
  for (i = 0; (lab = get_label(i)) != NULL; i++) {
    if (!atomic_test_and_clear_bit(lab->flags, BT_MESH_VA_CHANGED)) {
      continue;
    }
    ...
    if (err) {
      BT_ERR("Failed to %s %s value (err %d)",
             IS_VA_DEL(lab) ? "delete" : "store", log_strdup(path), err);
    } else {
      BT_DBG("%s %s value", IS_VA_DEL(lab) ? "Deleted" : "Stored",
             log_strdup(path));
    }
  }
}
...
void bt_mesh_store_label(void) { schedule_store(BT_MESH_VA_PENDING); }
...
void bt_mesh_store_cdb(void) { schedule_cdb_store(BT_MESH_CDB_SUBNET_PENDING); }
...
/* Added by bouffalo */
static ssize_t mesh_settings_read_cb(void *cb_arg, void *data,
                                     size_t data_len) {
...
static bool setting_env_cb(env_node_obj_t env, void *arg1, void *arg2) {
  if (env->crc_is_ok) {
    ...
    if (env->status == ENV_WRITE) {
      BT_WARN("Env[%.*s] Data len[%d]", env->name_len, env->name,
              env->value_len);
#if 1
      ...
      for (int i = 0; i < ARRAY_SIZE(settings); i++) {
        ...
        if (0 == strncmp(pname, settings[i].name, str_len) &&
          ...
          if (*pname != '/') {
            ...
            settings[i].func(NULL, env->name_len, mesh_settings_read_cb,
                             (void *)env);
          } else {
            BT_DBG("current name[%s]", pname + 1);
            settings[i].func(pname + 1, env->name_len, mesh_settings_read_cb,
                             (void *)env);
          }
          break;
        }
