...
static int iv_set(void) {
  ...
  BT_DBG("IV Index 0x%04x (IV Update Flag %u) duration %u hours", iv.iv_index,
         iv.iv_update, iv.iv_duration);
...
static int rpl_set(void) {
  bt_settings_get_bin(NV_MESH_RPL, (u8_t *)bt_mesh.rpl, sizeof(bt_mesh.rpl),
                      NULL);
  return 0;
}
...
static int net_key_set(void) {
  ...
  err = bt_settings_get_bin("bt/mesh/NetKeyIndex", (u8_t *)mesh_netkey_idx,
                            sizeof(mesh_netkey_idx), NULL);
  if (err) {
  ...
  for (i = 0; i < CONFIG_BT_MESH_SUBNET_COUNT; i++) {
    ...
    err = bt_settings_get_bin((const char *)path, (u8_t *)&key, sizeof(key),
                              NULL);
    if (err) {
...
static int app_key_set(void) {
  ...
  err = bt_settings_get_bin("bt/mesh/AppKeyIndex", (u8_t *)mesh_appkey_idx,
                            sizeof(mesh_appkey_idx), NULL);
  if (err) {
...
static int hb_pub_set(void) {
  ...
  err =
      bt_settings_get_bin(NV_MESH_HBPUB, (u8_t *)&hb_val, sizeof(hb_val), NULL);
...
static int cfg_set(void) {
  ...
  err = bt_settings_get_bin(NV_MESH_CFG, (u8_t *)&stored_cfg.cfg,
                            sizeof(stored_cfg.cfg), NULL);
...
static int mod_set_bind(struct bt_mesh_model *mod, bool vnd) {
  ...
  bt_settings_get_bin((const char *)path, (u8_t *)(mod->keys),
                      sizeof(mod->keys), NULL);
...
static int mod_set_sub(struct bt_mesh_model *mod, bool vnd) {
  ...
  bt_settings_get_bin((const char *)path, (u8_t *)(mod->groups),
                      sizeof(mod->groups), NULL);
...
static int mod_set_pub(struct bt_mesh_model *mod, bool vnd) {
  ...
  err =
      bt_settings_get_bin((const char *)path, (u8_t *)&pub, sizeof(pub), NULL);
...
static int rpl_set(const char *name, size_t len_rd, settings_read_cb read_cb,
  ...
  BT_DBG("RPL entry for 0x%04x: Seq 0x%06x old_iv %u", entry->src, entry->seq,
         entry->old_iv);
...
const struct mesh_setting {
  ...
  int (*func)(const char *name, size_t len_rd, settings_read_cb read_cb,
    ...
    {"Net", net_set},      {"IV", iv_set},          {"Seq", seq_set},
    {"RPL", rpl_set},      {"NetKey", net_key_set}, {"AppKey", app_key_set},
    {"HBPub", hb_pub_set}, {"Cfg", cfg_set},        {"s", sig_mod_set},
    {"v", vnd_mod_set},
};
...
void bt_mesh_store_net(void) { schedule_store(BT_MESH_NET_PENDING); }
...
static void clear_app_key(u16_t app_idx) {
  ...
  if (del) {
    ...
    err =
        settings_save_one("bt/mesh/AppKeyIndex", (const u8_t *)mesh_netkey_idx,
                          sizeof(mesh_netkey_idx));
    if (err)
...
static void clear_net_key(u16_t net_idx) {
  ...
  if (del) {
    ...
    err =
        settings_save_one("bt/mesh/NetKeyIndex", (const u8_t *)mesh_netkey_idx,
                          sizeof(mesh_netkey_idx));
    if (err)
...
static void store_app_key(struct bt_mesh_app_key *app) {
  ...
  if (empty_entry != 0xff) {
    ...
    err =
        settings_save_one("bt/mesh/AppKeyIndex", (const u8_t *)mesh_appkey_idx,
                          sizeof(mesh_appkey_idx));
    if (err) {
...
static void store_pending_mod_bind(struct bt_mesh_model *mod, bool vnd) {
  ...
  if (count) {
#if defined(BFLB_BLE)
    err = bt_settings_set_bin((const char *)path, (const u8_t *)keys,
                              sizeof(keys));
#else
...
static void store_pending_mod_sub(struct bt_mesh_model *mod, bool vnd) {
  ...
  if (count) {
#if defined(BFLB_BLE)
    err = bt_settings_set_bin((const char *)path, (const u8_t *)groups,
                              sizeof(groups));
#else
...
#if defined(BFLB_BLE)
    err = bt_settings_set_bin((const char *)path, (const u8_t *)&pub,
                              sizeof(pub));
#else
...
void bt_mesh_store_hb_pub(void) { schedule_store(BT_MESH_HB_PUB_PENDING); }

void bt_mesh_store_cfg(void) { schedule_store(BT_MESH_CFG_PENDING); }
...
void bt_mesh_clear_rpl(void) { schedule_store(BT_MESH_RPL_PENDING); }
