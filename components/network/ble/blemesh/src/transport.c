...
static int send_unseg(struct bt_mesh_net_tx *tx, struct net_buf_simple *sdu,
  ...
  BT_DBG("src 0x%04x dst 0x%04x app_idx 0x%04x sdu_len %u", tx->src,
         tx->ctx->addr, tx->ctx->app_idx, sdu->len);
...
static void seg_tx_send_unacked(struct seg_tx *tx) {
  ...
  for (i = 0; i <= tx->seg_n; i++) {
    ...
    BT_PTS("[PTS] Resending segment %u/%u (SeqZero = 0x%04X)", i, tx->seg_n,
           (u16_t)(tx->seq_auth & 0x1FFF));
#endif
...
static int send_seg(struct bt_mesh_net_tx *net_tx, struct net_buf_simple *sdu,
  ...
  for (seg_o = 0U; sdu->len; seg_o++) {
    ...
    if (IS_ENABLED(CONFIG_BT_MESH_FRIEND)) {
      ...
      if (bt_mesh_friend_enqueue_tx(net_tx, type, &tx->seq_auth, tx->seg_n + 1,
                                    &seg->b) &&
...
#ifdef CONFIG_BT_MESH_PTS
    BT_PTS("[PTS] Sending segment %u/%u (SeqZero = 0x%04X)", seg_o, tx->seg_n,
           seq_zero);
#endif

    BT_DBG("Sending %u/%u", seg_o, tx->seg_n);

    err = bt_mesh_net_send(net_tx, seg, seg_o ? &seg_sent_cb : &first_sent_cb,
                           tx);
    if (err) {
      BT_ERR("Sending segment failed");
      seg_tx_reset(tx);
...
int bt_mesh_ctl_send(struct bt_mesh_net_tx *tx, u8_t ctl_op, void *data,
  ...
  if (IS_ENABLED(CONFIG_BT_MESH_FRIEND)) {
    if (bt_mesh_friend_enqueue_tx(tx, BT_MESH_FRIEND_PDU_SINGLE, seq_auth, 1,
                                  &buf->b) &&
        BT_MESH_ADDR_IS_UNICAST(tx->ctx->addr)) {
      /* PDUs for a specific Friend should only go
       * out through the Friend Queue.
...
found_rx:
  ...
  *pdu_type = BT_MESH_FRIEND_PDU_COMPLETE;
  ...
  if (net_rx->ctl) {
    ...
    err = sdu_recv(net_rx, (rx->seq_auth & 0xffffff), *hdr, ASZMIC(hdr),
                   &rx->buf);
  }
...
void bt_mesh_heartbeat_send(void) {
  ...
  bt_mesh_ctl_send(&tx, TRANS_CTL_OP_HEARTBEAT, &hb, sizeof(hb), NULL, NULL,
                   NULL);
}
