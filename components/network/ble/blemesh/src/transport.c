...
u8_t bt_mesh_get_seg_retrans_num(void) { return SEG_RETRANSMIT_ATTEMPTS; }
...
void bt_mesh_set_hb_sub_dst(u16_t addr) { hb_sub_dst = addr; }
...
static void seg_tx_send_unacked(struct seg_tx *tx) {
  ...
  if (!tx->seg_pending) {
    k_delayed_work_submit(&tx->retransmit, K_MSEC(SEG_RETRANSMIT_TIMEOUT(tx)));
  }
...
int bt_mesh_trans_send(struct bt_mesh_net_tx *tx, struct net_buf_simple *msg,
  ...
  err =
      bt_mesh_app_key_get(tx->sub, tx->ctx->app_idx, tx->ctx->addr, &key, &aid);
  if (err) {
    return err;
  }
...
static void seg_rx_assemble(struct seg_rx *rx, struct net_buf_simple *buf,
  ...
  for (i = 0; i <= rx->seg_n; i++) {
    net_buf_simple_add_mem(
        buf, rx->seg[i],
        MIN(seg_len(rx->ctl), rx->len - (i * seg_len(rx->ctl))));
  }
...
static int app_key_decrypt(struct bt_mesh_net_rx *rx,
  ...
  err =
      bt_mesh_app_decrypt(keys->val, false, aszmic, buf, sdu, ad, rx->ctx.addr,
                          rx->ctx.recv_dst, seq, BT_MESH_NET_IVI_RX(rx));
  if (err) {
...
static int sdu_recv_unseg(struct bt_mesh_net_rx *rx, u8_t hdr,
  ...
  if (!AKF(&hdr)) {
    ...
    net_buf_simple_reset(&sdu);
    err = bt_mesh_app_decrypt(bt_mesh.dev_key, true, 0, buf, &sdu, ad,
  ...
  for (i = 0U; i < ARRAY_SIZE(bt_mesh.app_keys); i++) {
    ...
    net_buf_simple_reset(&sdu);
    err = app_key_decrypt(rx, &bt_mesh.app_keys[i], rx->seq, ad, hdr, 0, buf,
                          &sdu);
...
static int sdu_recv_seg(struct seg_rx *seg, u8_t hdr, u8_t aszmic,
  ...
  if (!AKF(&hdr)) {
    ...
    err = bt_mesh_app_decrypt(bt_mesh.dev_key, true, aszmic, &buf, &sdu, ad,
                              rx->ctx.addr, rx->ctx.recv_dst, seq,
                              BT_MESH_NET_IVI_RX(rx));
    if (err) {
      BT_ERR("Unable to decrypt with local DevKey");
  ...
  for (i = 0U; i < ARRAY_SIZE(bt_mesh.app_keys); i++) {
    ...
    err = app_key_decrypt(rx, &bt_mesh.app_keys[i], seq, ad, hdr, aszmic, &buf,
                          &sdu);
    if (err) {
      continue;
    }
...
static int send_ack(struct bt_mesh_subnet *sub, u16_t src, u16_t dst, u8_t ttl,
                    u64_t *seq_auth, u32_t block, u8_t obo) {
  struct bt_mesh_msg_ctx ctx = {
      .net_idx = sub->net_idx,
...
static int trans_seg(struct net_buf_simple *buf, struct bt_mesh_net_rx *net_rx,
  ...
  if (bt_mesh_rpl_check(net_rx, &rpl)) {
    BT_WARN("Replay: src 0x%04x dst 0x%04x seq 0x%06x", net_rx->ctx.addr,
...
found_rx:
  ...
  if (seg_o == seg_n) {
    ...
    BT_DBG("Target len %u * %u + %u = %u", seg_n, seg_len(rx->ctl), buf->len,
           rx->len);
...
void bt_mesh_trans_init(void) {
   ...
   * Init slab.
   *K_MEM_SLAB_DEFINE(segs, BT_MESH_APP_SEG_SDU_MAX, CONFIG_BT_MESH_SEG_BUFS,
   *4);
   */
  k_mem_slab_init(&segs, _k_mem_slab_buf_segs, BT_MESH_APP_SEG_SDU_MAX,
...
int bt_mesh_app_key_get(const struct bt_mesh_subnet *subnet, u16_t app_idx,
  ...
  if (app_idx == BT_MESH_KEY_DEV_LOCAL ||
      (app_idx == BT_MESH_KEY_DEV_REMOTE && bt_mesh_elem_find(addr) != NULL)) {
    *aid = 0;
    *key = bt_mesh.dev_key;
    return 0;
