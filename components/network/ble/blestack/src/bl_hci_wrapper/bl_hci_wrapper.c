...
static void bl_onchiphci_rx_packet_handler(uint8_t pkt_type, uint16_t src_id,
...
void bl_handle_queued_msg(void) {
  // give a default buf type BT_BUF_ACL_IN, will be set buf type in
  // bl_onchiphci_rx_packet_handler again.
  struct net_buf *buf;
  ...
  bl_onchiphci_rx_packet_handler(msg->pkt_type, msg->src_id, msg->param,
                                 msg->param_len, buf);
  if (msg->param) {
    k_free(msg->param);
  }
...
int bl_onchiphci_send_2_controller(struct net_buf *buf) {
  ...
  case BT_BUF_CMD: {
    ...
    printf("[btsnoop]:opcode =[0x%x],len =[0x%x],data=[%s]\r\n", opcode,
           chdr->param_len, bt_hex(buf->data, chdr->param_len));
    printf("[btsnoop]:Stop\r\n");
#endif
  ...
  case BT_BUF_ACL_OUT: {
    ...
    printf("[btsnoop]:Acl_out_handle =[0x%x],pb_bc_flag =[0x%x],len "
           "=[0x%x],data=[%s]\r\n",
           pkt.p.acl_data.conhdl, pkt.p.acl_data.pb_bc_flag, tlt_len,
           bt_hex(pkt.p.acl_data.buffer, tlt_len));
    printf("[btsnoop]:Stop\r\n");
#endif
...
#endif
{
  ...
  if (!rx_buf) {
    ...
    if (pkt_type == BT_HCI_CMD_CMP_EVT || pkt_type == BT_HCI_CMD_STAT_EVT)
      ...
      do {
        ...
        if ((pkt_type == BT_HCI_LE_EVT &&
             param[0] == BT_HCI_EVT_LE_ADVERTISING_REPORT) &&
            (bt_buf_get_rx_avail_cnt() <= CONFIG_BT_RX_BUF_RSV_COUNT)) {
          break;
        }
        /*not use K_FOREVER, rw main loop thread cannot be blocked here. if
          there is no rx buffer,directly igore. otherwise, if rw main loop
          blocked here, hci command cannot be handled.*/
  ...
  case BT_HCI_CMD_CMP_EVT: {
    ...
    *buf_data++ = nb_h2c_cmd_pkts;
    ...
    printf("[btsnoop]:pkt_type =[0x%x],len =[0x%x],data=[%s]\r\n", pkt_type,
           3 + param_len, bt_hex(tbuf_data - (3 + param_len), 3 + param_len));
    printf("[btsnoop]:Stop\r\n");
#endif
...
#if defined(CONFIG_BTSOONP_PRINT)
    printf("[btsnoop]:pkt_type =[0x%x],len =[0x%x],data=[%s]\r\n", pkt_type,
           param_len, bt_hex(buf_data, param_len));
    printf("[btsnoop]:Stop\r\n");
#endif
  ...
  case BT_HCI_EVT: {
    ...
    *buf_data++ = param_len;
    ...
    if (!prio) {
      printf("[btsnoop]:pkt_type =[0x%x],len =[0x%x],data=[%s]\r\n", pkt_type,
             param_len, bt_hex(buf_data - 2, param_len + 2));
      printf("[btsnoop]:Stop\r\n");
    } else {
  ...
  case BT_HCI_ACL_DATA: {
    ...
    printf("[btsnoop]:Acl_in_handle =[0x%x],pb_bc_flag =[0x%x],len "
           "=[0x%x],data=[%s]\r\n",
           buf_data[0], buf_data[1], tlt_len - 4,
           bt_hex(buf_data + 4, tlt_len - 4));
    printf("[btsnoop]:Stop\r\n");
#endif
