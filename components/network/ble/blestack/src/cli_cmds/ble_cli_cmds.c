...
static void ble_init(char *pcWriteBuffer, int xWriteBufferLen, int argc,
                     char **argv);
#if defined(BL702)
static void ble_set_2M_phy(char *pcWriteBuffer, int xWriteBufferLen, int argc,
                           char **argv);
static void ble_set_default_phy(char *pcWriteBuffer, int xWriteBufferLen,
                                int argc, char **argv);
#endif
static void ble_get_device_name(char *pcWriteBuffer, int xWriteBufferLen,
                                int argc, char **argv);
static void ble_set_device_name(char *pcWriteBuffer, int xWriteBufferLen,
                                int argc, char **argv);
#if defined(CONFIG_BT_OBSERVER)
static void ble_start_scan(char *pcWriteBuffer, int xWriteBufferLen, int argc,
                           char **argv);
static void ble_stop_scan(char *pcWriteBuffer, int xWriteBufferLen, int argc,
                          char **argv);
#endif
static void ble_read_local_address(char *pcWriteBuffer, int xWriteBufferLen,
                                   int argc, char **argv);
#if defined(CONFIG_BT_PERIPHERAL)
static void ble_set_adv_channel(char *pcWriteBuffer, int xWriteBufferLen,
                                int argc, char **argv);
static void ble_start_advertise(char *pcWriteBuffer, int xWriteBufferLen,
                                int argc, char **argv);
static void ble_stop_advertise(char *pcWriteBuffer, int xWriteBufferLen,
                               int argc, char **argv);
#endif
#if defined(CONFIG_BLE_TP_SERVER)
static void ble_tp_start(char *pcWriteBuffer, int xWriteBufferLen, int argc,
                         char **argv);
#endif
#if defined(CONFIG_BT_CONN)
#if defined(CONFIG_BT_CENTRAL)
static void ble_connect(char *pcWriteBuffer, int xWriteBufferLen, int argc,
                        char **argv);
#endif
static void ble_disconnect(char *pcWriteBuffer, int xWriteBufferLen, int argc,
                           char **argv);
static void ble_select_conn(char *pcWriteBuffer, int xWriteBufferLen, int argc,
                            char **argv);
static void ble_conn_update(char *pcWriteBuffer, int xWriteBufferLen, int argc,
                            char **argv);
static void ble_unpair(char *pcWriteBuffer, int xWriteBufferLen, int argc,
                       char **argv);
#endif
#if defined(CONFIG_BT_SMP)
static void ble_security(char *pcWriteBuffer, int xWriteBufferLen, int argc,
                         char **argv);
static void ble_auth(char *pcWriteBuffer, int xWriteBufferLen, int argc,
                     char **argv);
static void ble_auth_cancel(char *pcWriteBuffer, int xWriteBufferLen, int argc,
                            char **argv);
static void ble_auth_passkey_confirm(char *pcWriteBuffer, int xWriteBufferLen,
                                     int argc, char **argv);
static void ble_auth_pairing_confirm(char *pcWriteBuffer, int xWriteBufferLen,
                                     int argc, char **argv);
static void ble_auth_passkey(char *pcWriteBuffer, int xWriteBufferLen, int argc,
                             char **argv);
#endif
static void ble_exchange_mtu(char *pcWriteBuffer, int xWriteBufferLen, int argc,
                             char **argv);
static void ble_discover(char *pcWriteBuffer, int xWriteBufferLen, int argc,
                         char **argv);
static void ble_read(char *pcWriteBuffer, int xWriteBufferLen, int argc,
                     char **argv);
static void ble_write(char *pcWriteBuffer, int xWriteBufferLen, int argc,
                      char **argv);
static void ble_write_without_rsp(char *pcWriteBuffer, int xWriteBufferLen,
                                  int argc, char **argv);
static void ble_subscribe(char *pcWriteBuffer, int xWriteBufferLen, int argc,
                          char **argv);
static void ble_unsubscribe(char *pcWriteBuffer, int xWriteBufferLen, int argc,
                            char **argv);
static void ble_set_data_len(char *pcWriteBuffer, int xWriteBufferLen, int argc,
                             char **argv);
static void ble_get_all_conn_info(char *pcWriteBuffer, int xWriteBufferLen,
                                  int argc, char **argv);
static void ble_disable(char *pcWriteBuffer, int xWriteBufferLen, int argc,
                        char **argv);
#if defined(CONFIG_BLE_MULTI_ADV)
static void ble_start_multi_advertise(char *pcWriteBuffer, int xWriteBufferLen,
                                      int argc, char **argv);
static void ble_stop_multi_advertise(char *pcWriteBuffer, int xWriteBufferLen,
                                     int argc, char **argv);
#endif
#if defined(CONFIG_SET_TX_PWR)
static void ble_set_tx_pwr(char *pcWriteBuffer, int xWriteBufferLen, int argc,
                           char **argv);
#endif
#if defined(CONFIG_HOGP_SERVER)
static void ble_hog_srv_notify(char *pcWriteBuffer, int xWriteBufferLen,
                               int argc, char **argv);
#endif
...
static void ble_start_advertise(char *pcWriteBuffer, int xWriteBufferLen,
  ...
  if (mode == 0 || mode == 1 || mode == 2) {
    ...
    if (mode == 0) {
      struct bt_data gen_disc_data = (struct bt_data)BT_DATA_BYTES(
          BT_DATA_FLAGS, (BT_LE_AD_NO_BREDR | BT_LE_AD_GENERAL));
      ad_discov[0] = gen_disc_data;
    } else if (mode == 1) {
      struct bt_data non_disc_data =
          (struct bt_data)BT_DATA_BYTES(BT_DATA_FLAGS, BT_LE_AD_NO_BREDR);
      ad_discov[0] = non_disc_data;
    } else if (mode == 2) {
      struct bt_data limt_disc_data = (struct bt_data)BT_DATA_BYTES(
          BT_DATA_FLAGS, (BT_LE_AD_NO_BREDR | BT_LE_AD_LIMITED));
    ...
    struct bt_data data =
        (struct bt_data)BT_DATA(BT_DATA_NAME_COMPLETE, name, strlen(name));
    ad_discov[1] = data;
...
#if defined(CONFIG_BLE_MULTI_ADV)
    if (ble_adv_id == 0) {
      err = bt_le_multi_adv_start(&param, ad, ad_len, &ad_discov[0], 1,
                                  &ble_adv_id);
    }
#else
...
static void ble_start_multi_advertise(char *pcWriteBuffer, int xWriteBufferLen,
  ...
  struct bt_data flag_1 = (struct bt_data)BT_DATA_BYTES(
      BT_DATA_FLAGS, (BT_LE_AD_NO_BREDR | BT_LE_AD_GENERAL));
  struct bt_data data_1 =
      (struct bt_data)BT_DATA(BT_DATA_NAME_COMPLETE, name_1, strlen(name_1));
  ad_discov[0] = flag_1;
  ad_discov[0] = data_1;
  ad_1 = ad_discov;
  ...
  err_1 =
      bt_le_multi_adv_start(&param_1, ad_1, ad_len_1, NULL, 0, &instant_id_1);
  if (err_1) {
  ...
  struct bt_data flag_2 = (struct bt_data)BT_DATA_BYTES(
      BT_DATA_FLAGS, (BT_LE_AD_NO_BREDR | BT_LE_AD_GENERAL));
  struct bt_data data_2 =
      (struct bt_data)BT_DATA(BT_DATA_NAME_COMPLETE, name_2, strlen(name_2));
  ad_discov[0] = flag_2;
  ad_discov[0] = data_2;
  ad_2 = ad_discov;
  ...
  err_2 =
      bt_le_multi_adv_start(&param_2, ad_2, ad_len_2, NULL, 0, &instant_id_2);
  if (err_2) {
...
static void ble_connect(char *pcWriteBuffer, int xWriteBufferLen, int argc,
  ...
  /*Get addr type, 0:ADDR_PUBLIC, 1:ADDR_RAND, 2:ADDR_RPA_OR_PUBLIC,
   * 3:ADDR_RPA_OR_RAND*/
  addr.type = type;
...
static void ble_disconnect(char *pcWriteBuffer, int xWriteBufferLen, int argc,
  ...
  /*Notice:Because conn is got via bt_conn_lookup_addr_le in which
    bt_conn_ref(increase conn_ref by 1) this conn, we need to
    bt_conn_unref(decrease conn_ref by 1) this conn.*/
  bt_conn_unref(conn);
}
...
static void ble_security(char *pcWriteBuffer, int xWriteBufferLen, int argc,
  ...
  if (!default_conn) {
    vOutputString(
        "Please firstly choose the connection using ble_select_conn\r\n");
    return;
  }
...
u8_t discover_func(struct bt_conn *conn, const struct bt_gatt_attr *attr,
  ...
  case BT_GATT_DISCOVER_CHARACTERISTIC:
    ...
    vOutputString(
        "Characteristic %s found: attr->handle %x  chrc->handle %x \r\n", str,
        attr->handle, gatt_chrc->value_handle);
  ...
  case BT_GATT_DISCOVER_INCLUDE:
    ...
    vOutputString("Include %s found: handle %x, start %x, end %x\r\n", str,
                  attr->handle, gatt_include->start_handle,
...
static void ble_write_without_rsp(char *pcWriteBuffer, int xWriteBufferLen,
  ...
  err = bt_gatt_write_without_response(default_conn, handle, gatt_write_buf,
                                       len, sign);
...
static void ble_set_tx_pwr(char *pcWriteBuffer, int xWriteBufferLen, int argc,
  ...
  if (power > 21) {
    vOutputString(
        "ble_set_tx_pwr, invalid value, value shall be in [%d - %d]\r\n", 0,
        21);
    return;
  }
#elif defined(BL702)
...
static void ble_disable(char *pcWriteBuffer, int xWriteBufferLen, int argc,
  ...
  if (err) {
    vOutputString(
        "Fail to disable bt, there is existed scan/adv/conn event \r\n");
  } else {
