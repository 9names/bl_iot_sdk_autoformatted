...
u8_t peer_irk[16] = {0x01, 0x23, 0x45, 0x67, 0x89, 0xab, 0xcd, 0xef,
...
const u8_t discover_mode[] = {(BT_LE_AD_LIMITED | BT_LE_AD_NO_BREDR)};

static const u8_t service_uuid[16] = {0x00, 0x01, 0x02, 0x03, 0x04, 0x05,
...
static void pts_ble_add_peer_irk(char *pcWriteBuffer, int xWriteBufferLen,
                                 int argc, char **argv);

static void pts_ble_start_scan(char *pcWriteBuffer, int xWriteBufferLen,
                               int argc, char **argv);
static void pts_ble_start_scan_rpa(char *pcWriteBuffer, int xWriteBufferLen,
                                   int argc, char **argv);
static void pts_ble_start_advertise(char *pcWriteBuffer, int xWriteBufferLen,
                                    int argc, char **argv);
static void pts_ble_connect_le(char *pcWriteBuffer, int xWriteBufferLen,
                               int argc, char **argv);
static void pts_ble_disconnect(char *pcWriteBuffer, int xWriteBufferLen,
                               int argc, char **argv);
static void pts_ble_select_conn(char *pcWriteBuffer, int xWriteBufferLen,
                                int argc, char **argv);
static void pts_ble_conn_update(char *pcWriteBuffer, int xWriteBufferLen,
                                int argc, char **argv);
static void pts_ble_start_scan_timeout(char *pcWriteBuffer, int xWriteBufferLen,
                                       int argc, char **argv);
static void pts_ble_add_dev_to_resolve_list(char *pcWriteBuffer,
                                            int xWriteBufferLen, int argc,
                                            char **argv);
static void pts_ble_address_register(char *pcWriteBuffer, int xWriteBufferLen,
                                     int argc, char **argv);
static void pts_ble_set_flag(char *pcWriteBuffer, int xWriteBufferLen, int argc,
                             char **argv);
static void pts_ble_set_smp_flag(char *pcWriteBuffer, int xWriteBufferLen,
                                 int argc, char **argv);
static void pts_ble_clear_smp_flag(char *pcWriteBuffer, int xWriteBufferLen,
                                   int argc, char **argv);
static void pts_ble_bondable(char *pcWriteBuffer, int xWriteBufferLen, int argc,
                             char **argv);
static void pts_ble_read_uuid(char *pcWriteBuffer, int xWriteBufferLen,
                              int argc, char **argv);
static void pts_ble_mread(char *pcWriteBuffer, int xWriteBufferLen, int argc,
                          char **argv);
static void pts_ble_discover_uuid_128(char *pcWriteBuffer, int xWriteBufferLen,
                                      int argc, char **argv);
static void pts_ble_read_uuid_128(char *pcWriteBuffer, int xWriteBufferLen,
                                  int argc, char **argv);

#if defined(PTS_TEST_CASE_INSUFFICIENT_KEY)
static void pts_set_enc_key_size(char *pcWriteBuffer, int xWriteBufferLen,
                                 int argc, char **argv);
#endif
#if defined(CONFIG_BT_SMP)
static void pts_ble_set_mitm(char *pcWriteBuffer, int xWriteBufferLen, int argc,
                             char **argv);
#endif
#if defined(PTS_GAP_SLAVER_CONFIG_NOTIFY_CHARC)
static void pts_ble_notify(char *pcWriteBuffer, int xWriteBufferLen, int argc,
                           char **argv);
#endif
/*Service change*/
static void pts_ble_sc_indicate(char *pcWriteBuffer, int xWriteBufferLen,
                                int argc, char **argv);
#if defined(PTS_GAP_SLAVER_CONFIG_INDICATE_CHARC)
static void pts_ble_indicate(char *pcWriteBuffer, int xWriteBufferLen, int argc,
                             char **argv);
#endif
static void pts_ble_register_pts_svc(char *pcWriteBuffer, int xWriteBufferLen,
                                     int argc, char **argv);
#if defined(CONFIG_BT_WHITELIST)
static void pts_ble_bt_le_whitelist_add(char *pcWriteBuffer,
                                        int xWriteBufferLen, int argc,
                                        char **argv);
static void pts_ble_wl_connect(char *pcWriteBuffer, int xWriteBufferLen,
                               int argc, char **argv);
#endif
static void pts_ble_prepare_write(char *pcWriteBuffer, int xWriteBufferLen,
                                  int argc, char **argv);

const struct cli_command PtsCmdSet[] STATIC_CLI_CMD_ATTRIBUTE = {
    /*1.The cmd string to type, 2.Cmd description, 3.The function to
       run, 4.Number of parameters*/
...
static u8_t report[] = {0x01, 0x02, 0x03, 0x04, 0x05, 0x06, 0x07, 0x08, 0x09,
                        0x01, 0x02, 0x03, 0x04, 0x05, 0x06, 0x07, 0x08, 0x09,
                        0x01, 0x02, 0x03, 0x04, 0x05, 0x06, 0x07, 0x08, 0x09};

static ssize_t read_pts_long_value(struct bt_conn *conn,
...
static ssize_t read_pts_name(struct bt_conn *conn,
                             const struct bt_gatt_attr *attr, void *buf,
                             u16_t len, u16_t offset) {
...
static ssize_t pts_read_report(struct bt_conn *conn,
                               const struct bt_gatt_attr *attr, void *buf,
                               u16_t len, u16_t offset) {
...
static u8_t short_buf[TEST_SVAL_MAX] = {0x01, 0x02, 0x03, 0x04};
static u8_t long_buf[TEST_LVAL_MAX] = {
    0x01, 0x02, 0x03, 0x04, 0x05, 0x06, 0x07, 0x08, 0x09, 0x0a,
    0x11, 0x12, 0x13, 0x14, 0x15, 0x16, 0x17, 0x18, 0x19, 0x1a,
    0x21, 0x22, 0x23, 0x24, 0x25, 0x26, 0x27, 0x28, 0x29, 0x2a};

static ssize_t pts_write_short_value(struct bt_conn *conn,
...
static ssize_t pts_read_value(struct bt_conn *conn,
                              const struct bt_gatt_attr *attr, void *buf,
                              u16_t len, u16_t offset) {
  u8_t *data = short_buf;
...
static ssize_t pts_read_long_value(struct bt_conn *conn,
                                   const struct bt_gatt_attr *attr, void *buf,
                                   u16_t len, u16_t offset) {
  u8_t *data = long_buf;
...
static ssize_t pts_write_long_value(struct bt_conn *conn,
                                    const struct bt_gatt_attr *bt_attr,
                                    void *buf, u16_t len, u16_t offset,
                                    u8_t flags) {
  u16_t tlen = len;
...
#if defined(PTS_GAP_SLAVER_CONFIG_NOTIFY_CHARC) ||                             \
    defined(PTS_GAP_SLAVER_CONFIG_INDICATE_CHARC)
...
static ssize_t pts_read_value(struct bt_conn *conn,
                              const struct bt_gatt_attr *bt_attr, void *buf,
...
static ssize_t pts_write_value(struct bt_conn *conn,
                               const struct bt_gatt_attr *bt_attr, void *buf,
                               u16_t len, u16_t offset, u8_t flags) {
  return len;
}
...
static ssize_t pts_read_chara_value(struct bt_conn *conn,
                                    const struct bt_gatt_attr *bt_attr,
                                    void *buf, u16_t len, u16_t offset,
                                    u8_t flags) {
...
static ssize_t pts_read_mtu_size(struct bt_conn *conn,
                                 const struct bt_gatt_attr *bt_attr, void *buf,
                                 u16_t len, u16_t offset, u8_t flags) {
...
#endif
#if defined(PTS_TEST_CASE_INSUFFICIENT_KEY)
static ssize_t pts_read_value_with_key(struct bt_conn *conn,
...
static ssize_t pts_write_value_with_key(struct bt_conn *conn,
                                        const struct bt_gatt_attr *attr,
                                        void *buf, u16_t len, u16_t offset,
                                        u8_t flags) {
  const struct gatt_value *value = attr->user_data;
...
static struct bt_gatt_attr pts_attr[] = {
    ...
    BT_GATT_CHARACTERISTIC(BT_UUID_PTS_AUTH_CHAR, BT_GATT_CHRC_READ,
                           BT_GATT_PERM_READ_AUTHEN, pts_read_chara_value, NULL,
                           NULL),

#if defined(PTS_TEST_CASE_INSUFFICIENT_KEY)
    BT_GATT_CHARACTERISTIC(
        BT_UUID_PTS_ENC_KEY, BT_GATT_CHRC_READ | BT_GATT_CHRC_WRITE,
        BT_GATT_PERM_READ | BT_GATT_PERM_READ_ENCRYPT | BT_GATT_PERM_WRITE |
            BT_GATT_PERM_WRITE_ENCRYPT | BT_GATT_PERM_PREPARE_WRITE,
        pts_read_value_with_key, pts_write_value_with_key, &value),
#endif

#if defined(PTS_GAP_SLAVER_CONFIG_WRITE_CHARC)

    /* Case : GATT/SR/GAR/BV-03-C
     * Verify that a Generic Attribute Profile server can support writing a
     * Characteristic Value selected by handle
     */
    BT_GATT_CHARACTERISTIC(BT_UUID_PTS_CHAR_WRITE_VALUE,
                           BT_GATT_CHRC_READ | BT_GATT_CHRC_WRITE,
                           BT_GATT_PERM_READ | BT_GATT_PERM_WRITE,
                           pts_read_value, pts_write_short_value, NULL),
     ...
     */
    BT_GATT_CHARACTERISTIC(BT_UUID_PTS_CHAR_WRITE_NORSP,
     ...
     * Write Characteristic Value Request to a non-writeable Characteristic
     * Value and issue a Write Not Permitted Response.
     */
    BT_GATT_CHARACTERISTIC(BT_UUID_PTS_CHAR_WRITE_AUTHEN,
                           BT_GATT_CHRC_READ | BT_GATT_CHRC_WRITE,
                           BT_GATT_PERM_READ | BT_GATT_PERM_WRITE_AUTHEN,
                           pts_read_long_value, pts_write_long_value, NULL),

    BT_GATT_CHARACTERISTIC(BT_UUID_PTS_CHAR_WRITE_AUTHEN,
                           BT_GATT_CHRC_READ | BT_GATT_CHRC_WRITE,
                           BT_GATT_PERM_READ | BT_GATT_PERM_WRITE_AUTHEN,
                           pts_read_value, pts_write_long_value, NULL),

    /* Case : GATT/SR/GAW/BV-05-C */
    BT_GATT_CHARACTERISTIC(BT_UUID_PTS_CHAR_WRITE_LONGVAL,
                           BT_GATT_CHRC_READ | BT_GATT_CHRC_WRITE,
                           BT_GATT_PERM_READ | BT_GATT_PERM_WRITE,
                           pts_read_long_value, pts_write_long_value, NULL),

    /* Case : GATT/SR/GAW/BV-10-C */
    BT_GATT_CHARACTERISTIC(BT_UUID_PTS_CHAR_WRITE_2LONGVAL,
                           BT_GATT_CHRC_READ | BT_GATT_CHRC_WRITE,
                           BT_GATT_PERM_READ | BT_GATT_PERM_WRITE,
                           pts_read_long_value, pts_write_long_value, NULL),

    BT_GATT_DESCRIPTOR(BT_UUID_PTS_CHAR_WRITE_L_DES,
                       BT_GATT_PERM_READ | BT_GATT_PERM_WRITE,
                       pts_read_long_value, pts_write_long_value, NULL),
     ...
     */
    BT_GATT_CHARACTERISTIC(BT_UUID_PTS_CHAR_READ_AUTHEN, BT_GATT_CHRC_READ,
                           BT_GATT_PERM_READ_AUTHEN, read_pts_name, NULL, NULL),
     ...
     */
    BT_GATT_CHARACTERISTIC(BT_UUID_PTS_CHAR_READ_NOPERM, BT_GATT_CHRC_READ,
                           BT_GATT_PERM_NONE, read_pts_name, NULL, NULL),
     ...
     */
    BT_GATT_CHARACTERISTIC(BT_UUID_PTS_CHAR_READ_LONGVAL, BT_GATT_CHRC_READ,
                           BT_GATT_PERM_READ, read_pts_long_value, NULL, NULL),
     ...
     */
    BT_GATT_DESCRIPTOR(BT_UUID_PTS_CHAR_READ_LVAL_REF, BT_GATT_PERM_READ,
                       pts_read_report, NULL, report),
     ...
     * Value and issue a Read Not Permitted Response.
     */
    BT_GATT_CHARACTERISTIC(BT_UUID_PTS_CHAR_READ_L_NOPERM, BT_GATT_CHRC_READ,
                           BT_GATT_PERM_NONE, read_pts_long_value, NULL, NULL),

#endif /* PTS_GAP_SLAVER_CONFIG_READ_CHARC */

#if defined(PTS_GAP_SLAVER_CONFIG_NOTIFY_CHARC)
    BT_GATT_CHARACTERISTIC(
        BT_UUID_PTS_CHAR_NOTIFY_CHAR, BT_GATT_CHRC_READ | BT_GATT_CHRC_NOTIFY,
        BT_GATT_PERM_READ, pts_read_value, NULL, &battery_level),

    BT_GATT_CCC(notify, BT_GATT_PERM_READ | BT_GATT_PERM_WRITE),
#endif

#if defined(PTS_GAP_SLAVER_CONFIG_INDICATE_CHARC)
    BT_GATT_CHARACTERISTIC(BT_UUID_PTS_CHAR_INDICATE_CHAR,
...
#endif
#if defined(PTS_CHARC_LEN_EQUAL_MTU_SIZE)
    BT_GATT_CHARACTERISTIC(BT_UUID_PTS_READ_MTU_SIZE_CHAR, BT_GATT_CHRC_READ,
                           BT_GATT_PERM_READ, pts_read_mtu_size, NULL, NULL),
#endif
...
static void pts_device_found(const bt_addr_le_t *addr, s8_t rssi, u8_t evtype,
  ...
  if (!memcmp(&pts_addr, addr, sizeof(bt_addr_le_t)) ||
    ...
    if (*ad_flag & 0x01) {
      vOutputString(
          "Advertising data : 'Limited Discovered Mode' flag is set one\r\n");
    } else
      vOutputString(
          "Advertising data : 'Limited Discovered Mode' flag is not set\r\n");

    if (*ad_flag & 0x02) {
      vOutputString(
          "Advertising data : 'General Discovered Mode' flag is set one\r\n");
    } else
      vOutputString(
          "Advertising data : 'General Discovered Mode' flag is not set\r\n");
  }
}
...
static void pts_ble_wl_connect(char *pcWriteBuffer, int xWriteBufferLen,
  ...
  /*Address type, 0:ADDR_PUBLIC, 1:ADDR_RAND, 2:ADDR_RPA_OR_PUBLIC,
   * 3:ADDR_RPA_OR_RAND*/
...
static void pts_ble_start_advertise(char *pcWriteBuffer, int xWriteBufferLen,
  ...
  /*Get adv type, 0:adv_ind,  1:adv_scan_ind, 2:adv_nonconn_ind 3:
   * adv_direct_ind*/
  ...
  if (adv_type == 0) {
    ...
    param.options = (BT_LE_ADV_OPT_CONNECTABLE |
                     BT_LE_ADV_OPT_DIR_MODE_LOW_DUTY | BT_LE_ADV_OPT_ONE_TIME);
  } else {
  ...
  /*Get mode, 0:General discoverable,  1:non discoverable, 2:limit
   * discoverable*/
  ...
  /*upper address type, 0:non-resolvable private address,1:resolvable private
   * address,2:public address*/
...
static void pts_ble_connect_le(char *pcWriteBuffer, int xWriteBufferLen,
  ...
  /*Get addr type,0:ADDR_RAND, 1:ADDR_RPA_OR_PUBLIC,2:ADDR_PUBLIC,
   * 3:ADDR_RPA_OR_RAND*/
  if (type == 0)
...
extern u8_t discover_func(struct bt_conn *conn, const struct bt_gatt_attr *attr,
...
static void pts_ble_prepare_write(char *pcWriteBuffer, int xWriteBufferLen,
  ...
  data_len = write_params.length > sizeof(gatt_write_buf)
                 ? (sizeof(gatt_write_buf))
                 : (write_params.length);
...
int pts_cli_register(void) {
  ...
  // sizeof(btStackCmdSet)/sizeof(btStackCmdSet[0]));
  return 0;
}
