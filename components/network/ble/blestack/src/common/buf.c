...
#if defined(CONFIG_NET_BUF_WARN_ALLOC_INTERVAL) &&                             \
...
void net_buf_init(struct net_buf_pool *buf_pool, u16_t buf_count,
                  size_t data_size, destroy_cb_t destroy) {
  struct net_buf_pool_fixed *buf_fixed;
  buf_pool->alloc = (struct net_buf_data_alloc *)k_malloc(sizeof(void *));
  buf_pool->alloc->alloc_data =
      (struct net_buf_pool_fixed *)k_malloc(sizeof(void *));
  ...
  buf_pool->__bufs =
      (struct net_buf *)k_malloc(buf_count * sizeof(struct net_buf));
...
void net_buf_deinit(struct net_buf_pool *buf_pool) {
  struct net_buf_pool_fixed *buf_fixed =
      (struct net_buf_pool_fixed *)buf_pool->alloc->alloc_data;
...
static u8_t *mem_pool_data_alloc(struct net_buf *buf, size_t *size,
  ...
  if (k_mem_pool_alloc(pool, &block, sizeof(struct k_mem_block_id) + 1 + *size,
                       timeout)) {
    return NULL;
  }
...
#endif
{
  ...
  NET_BUF_DBG("%s():%d: pool %p size %zu timeout %d", func, line, pool, size,
              timeout);
...
#if defined(CONFIG_NET_BUF_POOL_USAGE)
      NET_BUF_WARN("%s():%d: Pool %s blocked for %u secs", func, line,
                   pool->name, (k_uptime_get_32() - ref) / MSEC_PER_SEC);
#else
      NET_BUF_WARN("%s():%d: Pool %p blocked for %u secs", func, line, pool,
...
#endif
{
  ...
  while (buf) {
    ...
    unsigned int key =
        irq_lock(); /* Added by bouffalo lab, to protect ref decrease */
    if (--buf->ref > 0) {
...
#if defined(OPTIMIZE_DATA_EVT_FLOW_FROM_CONTROLLER)
    if (pool == &hci_rx_pool &&
        (buf_type == BT_BUF_ACL_IN || adv_report == true)) {
...
struct net_buf *net_buf_ref(struct net_buf *buf) {
  ...
  unsigned int key =
      irq_lock(); /* Added by bouffalo lab,  to protect ref increase */
  buf->ref++;
  irq_unlock(key); /* Added by bouffalo lab */
  return buf;
}
...
size_t net_buf_append_bytes(struct net_buf *buf, size_t len, const void *value,
                            s32_t timeout, net_buf_allocator_cb allocate_cb,
                            void *user_data) {
