...
#define NET_BUF_POOL_INITIALIZER(_pool, _alloc, _bufs, _count, _destroy)       \
  {                                                                            \
    .alloc = _alloc, .free = _K_LIFO_INITIALIZER(_pool.free), .__bufs = _bufs, \
    .buf_count = _count, .uninit_count = _count, .avail_count = _count,        \
    .destroy = _destroy, .name = STRINGIFY(_pool),                             \
  }
#else
#define NET_BUF_POOL_INITIALIZER(_pool, _alloc, _bufs, _count, _destroy)       \
  {                                                                            \
    .alloc = _alloc, .free = _K_LIFO_INITIALIZER(_pool.free), .__bufs = _bufs, \
    .buf_count = _count, .uninit_count = _count, .destroy = _destroy,          \
  }
#endif /* CONFIG_NET_BUF_POOL_USAGE */
...
#define NET_BUF_POOL_HEAP_DEFINE(_name, _count, _destroy)                      \
  ...
  struct net_buf_pool _name __net_buf_align __in_section(_net_buf_pool,        \
                                                         static, _name) =      \
      NET_BUF_POOL_INITIALIZER(_name, &net_buf_heap_alloc, net_buf_##_name,    \
                               _count, _destroy)
...
#define net_buf_alloc_with_data(_pool, _data_, _size, _timeout)                \
  net_buf_alloc_with_data_debug(_pool, _data_, _size, _timeout, __func__,      \
                                __LINE__)
#else
