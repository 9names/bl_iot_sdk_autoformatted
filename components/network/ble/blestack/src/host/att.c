...
#endif
  return bt_l2cap_send_cb(conn, BT_L2CAP_CID_ATT, buf, cb ? cb : att_cb(buf),
                          user_data);
}
...
static u8_t find_info_cb(const struct bt_gatt_attr *attr, void *user_data) {
  ...
  if (!data->rsp) {
    ...
    data->rsp->format = (attr->uuid->type == BT_UUID_TYPE_16) ? BT_ATT_INFO_16
                                                              : BT_ATT_INFO_128;
  }
...
#if defined(CONFIG_BT_STACK_PTS)
  /*when PTS sends a request to the iut discover all primary services it
   * contains, set event flag to @att_find_by_type_value_ind make it easy for
   * the user to check whether the messages is correct in the console.
   */
  if (event_flag == att_find_by_type_value_ind) {
    u8_t i = 0;
...
static const struct att_handler {
  ...
  u8_t (*func)(struct bt_att *att, struct net_buf *buf);
    ...
    {BT_ATT_OP_FIND_TYPE_RSP, sizeof(struct bt_att_find_type_rsp), ATT_RESPONSE,
    ...
    {BT_ATT_OP_READ_RSP, sizeof(struct bt_att_read_rsp), ATT_RESPONSE,
     att_handle_read_rsp},
    {BT_ATT_OP_READ_BLOB_RSP, sizeof(struct bt_att_read_blob_rsp), ATT_RESPONSE,
     att_handle_read_blob_rsp},
#if defined(CONFIG_BT_GATT_READ_MULTIPLE)
    {BT_ATT_OP_READ_MULT_RSP, sizeof(struct bt_att_read_mult_rsp), ATT_RESPONSE,
