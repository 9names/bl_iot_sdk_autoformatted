...
static void handle_avdtp_discover_cmd(struct bt_avdtp *session,
                                      struct net_buf *buf, uint8_t msg_type);
static void handle_avdtp_get_cap_cmd(struct bt_avdtp *session,
                                     struct net_buf *buf, uint8_t msg_type);
static void handle_avdtp_set_conf_cmd(struct bt_avdtp *session,
                                      struct net_buf *buf, uint8_t msg_type);
static void handle_avdtp_get_conf_cmd(struct bt_avdtp *session,
                                      struct net_buf *buf, uint8_t msg_type);
static void handle_avdtp_reconf_cmd(struct bt_avdtp *session,
                                    struct net_buf *buf, uint8_t msg_type);
static void handle_avdtp_open_cmd(struct bt_avdtp *session, struct net_buf *buf,
                                  uint8_t msg_type);
static void handle_avdtp_start_cmd(struct bt_avdtp *session,
                                   struct net_buf *buf, uint8_t msg_type);
static void handle_avdtp_close_cmd(struct bt_avdtp *session,
                                   struct net_buf *buf, uint8_t msg_type);
static void handle_avdtp_suspend_cmd(struct bt_avdtp *session,
                                     struct net_buf *buf, uint8_t msg_type);
static void handle_avdtp_abort_cmd(struct bt_avdtp *session,
                                   struct net_buf *buf, uint8_t msg_type);
static void handle_avdtp_sec_ctrl_cmd(struct bt_avdtp *session,
                                      struct net_buf *buf, uint8_t msg_type);
static void handle_avdtp_get_all_cap_cmd(struct bt_avdtp *session,
                                         struct net_buf *buf, uint8_t msg_type);
static void handle_avdtp_dly_rpt_cmd(struct bt_avdtp *session,
                                     struct net_buf *buf, uint8_t msg_type);
...
static void handle_avdtp_discover_cmd(struct bt_avdtp *session,
  ...
  rsp_buf = avdtp_create_pdu(BT_AVDTP_ACCEPT, BT_AVDTP_PACKET_TYPE_SINGLE,
                             BT_AVDTP_DISCOVER);
  if (!rsp_buf) {
  ...
  while (disc_sep) {
    acp_endpoint[0] =
        disc_sep->sep.id << 2 | disc_sep->sep.inuse << 1 | disc_sep->sep.rfa0;
    acp_endpoint[1] = disc_sep->sep.media_type << 4 | disc_sep->sep.tsep << 3 |
                      disc_sep->sep.rfa1;
...
static void handle_avdtp_get_cap_cmd(struct bt_avdtp *session,
  ...
  svc_cat_2[5] = sbc_info.block_length << 4 | sbc_info.subbands << 2 |
...
int bt_avdtp_l2cap_recv(struct bt_l2cap_chan *chan, struct net_buf *buf) {
  ...
  BT_DBG("msg_type[0x%02x] sig_id[0x%02x] tid[0x%02x]", msgtype, sigid, tid);
...
int bt_avdtp_l2cap_media_stream_recv(struct bt_l2cap_chan *chan,
                                     struct net_buf *buf) {
#if 0
	BT_DBG("avdtp payload len: %d \n", buf->len);
	for(int i = 0; i < buf->len; i++)
