...
static void blhast_ble_scan_assist_cb(const struct bt_le_scan_param *param,
                                      bt_le_scan_cb_t cb);
static void blhast_ble_adv_assist_cb(const struct bt_le_adv_param *param,
...
static void blhast_ble_get_ad(const struct bt_data *ad, size_t ad_len,
                              uint8_t *output) {
  int i;
  uint8_t data_len = 0;
...
static void blhast_ble_construct_ad(struct blhast_le_adv_data *adv_data,
                                    struct bt_data *output) {
  int i;
...
static void blhast_host_state_restore(void) {
  ...
  if (atomic_test_bit(bt_dev.flags, BT_DEV_EXPLICIT_SCAN)) {
    ...
    bt_le_scan_start((const struct bt_le_scan_param *)&blhast_le_scan_param,
                     blhast_le_scan_cb);
  }
#endif
