...
static void sc_store(struct gatt_sc_cfg *cfg) {
  ...
  BT_DBG("stored SC for %s (%s, 0x%04x-0x%04x)", bt_addr_le_str(&cfg->peer),
         log_strdup(key), cfg->data.start, cfg->data.end);
}
  ...
  if (IS_ENABLED(CONFIG_BT_SETTINGS)) {
    ...
    if (modified && bt_addr_le_is_bonded(cfg->id, &cfg->peer)) {
      ...
      if (err) {
        ...
        BT_DBG("deleted SC for %s (%s)", bt_addr_le_str(&cfg->peer),
               log_strdup(key));
      }
    }
  }
...
static bool update_range(u16_t *start, u16_t *end, u16_t new_start,
                         u16_t new_end) {
  BT_DBG("start 0x%04x end 0x%04x new_start 0x%04x new_end 0x%04x", *start,
...
static bool cf_set_value(struct gatt_cf_cfg *cfg, const u8_t *value,
                         u16_t len) {
  u16_t i;
  u8_t last_byte = 1U;
  u8_t last_bit = 1U;
...
static u8_t gen_hash_m(const struct bt_gatt_attr *attr, void *user_data) {
  ...
  case 0x2900: /* GATT Characteristic Extended Properties */
    ...
    if (tc_cmac_update(&state->state, (uint8_t *)&value, sizeof(u16->val)) ==
        TC_CRYPTO_FAIL) {
      state->err = -EINVAL;
      return BT_GATT_ITER_STOP;
    }
...
BT_GATT_SERVICE_DEFINE(_1_gatt_svc,
    ...
    BT_GATT_CHARACTERISTIC(
        BT_UUID_GATT_CLIENT_FEATURES, BT_GATT_CHRC_READ | BT_GATT_CHRC_WRITE,
        BT_GATT_PERM_READ | BT_GATT_PERM_WRITE, cf_read, cf_write, NULL),
...
#if defined(CONFIG_BT_STACK_PTS)
int service_change_test(struct bt_gatt_indicate_params *params,
                        const struct bt_conn *con) {
  u16_t sc_range[2];
...
void bt_gatt_cancle_sc_work(void) { k_delayed_work_cancel(&gatt_sc.work); }
#endif
...
ssize_t bt_gatt_attr_read(struct bt_conn *conn, const struct bt_gatt_attr *attr,
   ...
   * Set event flags to avoid comflicts when other test cases need to add
   * reference codes.
   */
...
ssize_t bt_gatt_attr_read_service(struct bt_conn *conn,
  ...
  return bt_gatt_attr_read(conn, attr, buf, len, offset, BT_UUID_128(uuid)->val,
                           16);
}
...
static u8_t gatt_foreach_iter(const struct bt_gatt_attr *attr,
                              ...
                              uint16_t *num_matches, bt_gatt_attr_func_t func,
                              void *user_data) {
  u8_t result;
...
static void foreach_attr_type_dyndb(u16_t start_handle, u16_t end_handle,
  ...
  SYS_SLIST_FOR_EACH_CONTAINER(&db, svc, node) {
    ...
    for (i = 0; i < svc->attr_count; i++) {
      ...
      if (gatt_foreach_iter(attr, start_handle, end_handle, uuid, attr_data,
                            &num_matches, func,
...
static void gatt_indicate_rsp(struct bt_conn *conn, u8_t err, const void *pdu,
...
int bt_gatt_notify_cb(struct bt_conn *conn,
  ...
  if (params->uuid) {
    ...
    bt_gatt_foreach_attr_type(handle, 0xffff, params->uuid, NULL, 1, match_uuid,
                              &attr);
    if (!attr) {
      return -ENOENT;
    }
...
#if defined(CONFIG_BT_GATT_CLIENT)
void bt_gatt_notification(struct bt_conn *conn, u16_t handle, const void *data,
                          u16_t length) {
...
static void gatt_find_type_rsp(struct bt_conn *conn, u8_t err, const void *pdu,
                               u16_t length, void *user_data) {
...
static int gatt_find_type(struct bt_conn *conn,
  ...
  BT_DBG("uuid %s start_handle 0x%04x end_handle 0x%04x",
         bt_uuid_str(params->uuid), params->start_handle, params->end_handle);
...
static u16_t parse_characteristic(struct bt_conn *conn, const void *pdu,
  ...
  for (length--, pdu = rsp->data; length >= rsp->len;
    ...
    attr = (&(struct bt_gatt_attr)BT_GATT_CHRC(&u.uuid, chrc->value_handle,
                                               chrc->properties));
    attr->handle = handle;
...
static void gatt_read_type_rsp(struct bt_conn *conn, u8_t err, const void *pdu,
                               u16_t length, void *user_data) {
  struct bt_gatt_discover_params *params = user_data;
  u16_t handle;
...
static void gatt_read_group_rsp(struct bt_conn *conn, u8_t err, const void *pdu,
                                u16_t length, void *user_data) {
  struct bt_gatt_discover_params *params = user_data;
  u16_t handle;
...
static void gatt_find_info_rsp(struct bt_conn *conn, u8_t err, const void *pdu,
                               u16_t length, void *user_data) {
  ...
  for (i = length / len, pdu = rsp->info; i != 0;
    ...
    attr = (&(struct bt_gatt_attr)BT_GATT_DESCRIPTOR(&u.uuid, 0, NULL, NULL,
                                                     NULL));
    attr->handle = handle;
...

{
  ...
  buf =
      bt_att_create_pdu(conn, BT_ATT_OP_PREPARE_WRITE_REQ, sizeof(*req) + len);
  if (!buf) {
    return -ENOMEM;
  }
...
static void gatt_write_ccc_rsp(struct bt_conn *conn, u8_t err, const void *pdu,
                               u16_t length, void *user_data) {
...
static struct bt_gatt_ccc_cfg *ccc_find_cfg(struct _bt_gatt_ccc *ccc,
                                            const bt_addr_le_t *addr, u8_t id) {
  for (size_t i = 0; i < ARRAY_SIZE(ccc->cfg); i++) {
...
int bt_gatt_store_ccc(u8_t id, const bt_addr_le_t *addr) {
  ...
  if (id) {
    ...
    bt_settings_encode_key(key, sizeof(key), "ccc", (bt_addr_le_t *)addr,
                           id_str);
  } else {
...
static int bt_gatt_clear_cf(u8_t id, const bt_addr_le_t *addr) {
  ...
  if (id) {
    ...
    u8_to_dec(id_str, sizeof(id_str), id);
    bt_settings_encode_key(key, sizeof(key), "cf", (bt_addr_le_t *)addr,
                           id_str);
  } else {
