...
static int hci_le_create_conn(const struct bt_conn *conn) {
  ...
  if (conn->le.own_adder_type == BT_ADDR_LE_PUBLIC ||
      conn->le.own_adder_type == BT_ADDR_LE_PUBLIC_ID) {
...
static void enh_conn_complete(struct bt_hci_evt_le_enh_conn_complete *evt) {
  ...
  BT_DBG("status 0x%02x handle %u role %u %s", evt->status, handle, evt->role,
...
static void le_data_len_change(struct net_buf *buf) {
  ...
  BT_DBG("max. tx: %u (%uus), max. rx: %u (%uus)", max_tx_octets, max_tx_time,
...
bool bt_le_conn_params_valid(const struct bt_le_conn_param *param) {
  ...
  if (param->timeout < 10 || param->timeout > 3200 ||
      ((param->timeout * 4U) <= ((1 + param->latency) * param->interval_max))) {
    return false;
  }
...
static void le_conn_update_complete(struct net_buf *buf) {
  ...
  if (!evt->status) {
    ...
    notify_le_param_updated(conn);
             ...
             !atomic_test_and_set_bit(conn->flags, BT_CONN_SLAVE_PARAM_L2CAP)) {
...
#if defined(CONFIG_BT_STACK_PTS)
static int start_le_scan_with_isrpa(u8_t scan_type, u16_t interval,
                                    u16_t window, u8_t addre_type) {
  struct bt_hci_cp_le_set_scan_param set_param;
  struct net_buf *buf;
  int err = 0;
...
static const struct event_handler meta_events[] = {
    ...
    EVENT_HANDLER(BT_HCI_EV_LE_REMOTE_FEAT_COMPLETE, le_remote_feat_complete,
...
static const struct event_handler normal_events[] = {
    ...
    EVENT_HANDLER(BT_HCI_EVT_INQUIRY_RESULT_WITH_RSSI, inquiry_result_with_rssi,
                  sizeof(struct bt_hci_evt_inquiry_result_with_rssi)),
    EVENT_HANDLER(BT_HCI_EVT_EXTENDED_INQUIRY_RESULT, extended_inquiry_result,
...
static int bt_init(void) {
  ...
  u8_t dbg_irk[16];
#endif
/*Make sure that freertos is running when set info into flash, because Semaphore
 * is used in ef_set_env*/
#if defined(BFLB_BLE_PATCH_SETTINGS_LOAD)
  ...
  BT_PTS("Local IRK %s public identity bdaddr %s", bt_hex(dbg_irk, 16),
...
#if defined(CONFIG_BT_HCI_ACL_FLOW_CONTROL)
  net_buf_init(&acl_in_pool, CONFIG_BT_ACL_RX_COUNT, ACL_IN_SIZE,
               report_completed_packet);
#endif // CONFIG_BT_HCI_ACL_FLOW_CONTROL
#endif // CONFIG_BT_CONN
#if defined(CONFIG_BT_DISCARDABLE_BUF_COUNT)
  net_buf_init(&discardable_pool, CONFIG_BT_DISCARDABLE_BUF_COUNT,
               BT_BUF_RX_SIZE, NULL);
#endif
#endif
...
#if defined(BFLB_BLE)
#if (!BFLB_BLE_CO_THREAD)
  k_thread_create(&tx_thread_data, "hci_tx_thread", CONFIG_BT_HCI_TX_STACK_SIZE,
                  hci_tx_thread, CONFIG_BT_HCI_TX_PRIO);
#endif
#else
  k_thread_create(&tx_thread_data, tx_thread_stack,
                  K_THREAD_STACK_SIZEOF(tx_thread_stack), hci_tx_thread, NULL,
                  NULL, NULL, K_PRIO_COOP(CONFIG_BT_HCI_TX_PRIO), 0, K_NO_WAIT);
...
#if defined(BFLB_BLE)
  k_thread_create(
      &rx_thread_data, "hci_rx_thread",
      CONFIG_BT_HCI_RX_STACK_SIZE /*K_THREAD_STACK_SIZEOF(rx_thread_stack)*/,
      (k_thread_entry_t)hci_rx_thread, CONFIG_BT_RX_PRIO);
#else
...
int bt_disable_action(void) {
  ...
  // delete queue, not delete
  // hci_cmd_pool.free/hci_rx_pool.free/acl_tx_pool.free which store released
  // buffers.
...
static bool valid_adv_param(const struct bt_le_adv_param *param, bool dir_adv) {
  ...
  if (is_wl_empty() && ((param->options & BT_LE_ADV_OPT_FILTER_SCAN_REQ) ||
...
int bt_le_adv_start_internal(const struct bt_le_adv_param *param,
  ...
  if (param->options & BT_LE_ADV_OPT_CONNECTABLE) {
    ...
    if (dir_adv) {
      ...
      if (IS_ENABLED(CONFIG_BT_SMP) && !IS_ENABLED(CONFIG_BT_PRIVACY) &&
...
#if defined(BFLB_HOST_ASSISTANT)
  if (!atomic_test_bit(bt_dev.flags, BT_DEV_ASSIST_RUN) && host_assist_cb &&
      host_assist_cb->le_adv_cb)
...
#if defined(CONFIG_BT_STACK_PTS)
int bt_le_pts_scan_start(const struct bt_le_scan_param *param,
                         bt_le_scan_cb_t cb, u8_t addre_type) {
  int err;
  ...
  err = start_le_scan_with_isrpa(param->type, param->interval, param->window,
                                 addre_type);
