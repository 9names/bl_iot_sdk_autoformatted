...
#define BT_DEV_PERSISTENT_FLAGS                                                \
  (BIT(BT_DEV_ENABLE) | BIT(BT_DEV_PRESET_ID) | BIT(BT_DEV_USER_ID_ADDR))
...
typedef enum __packed {
  dir_connect_req = 0x01, /*Send a direct connection require while the Lower
                             test enters direct mode .*/
  ...
  att_read_by_group_type_ind =
      0x0c, /* CASE : GATT/SR/GAD/BV-01-C. Indicate PTS sends a GATT discover
               all primary services request to iut */
  att_find_by_type_value_ind =
      0x0d, /* CASE : GATT/SR/GAD/BV-02-C. Indicate PTS sends a request to iut
               for discover it contains Primary Services by Service UUID */
  att_read_by_type_ind =
      0x0e, /* CASE : GATT/SR/GAD/BV-04-C. Indicate PTS sends a request to iut
               for discover all characteristics of a specified service.*/
...
struct blhast_cb {
  ...
  void (*le_adv_cb)(const struct bt_le_adv_param *param,
                    const struct bt_data *ad, size_t ad_len,
                    const struct bt_data *sd, size_t sd_len);
};
