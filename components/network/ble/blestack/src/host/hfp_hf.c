...
static struct bt_sdp_attribute hfp_attrs[] = {
    BT_SDP_NEW_SERVICE,
    BT_SDP_LIST(
        BT_SDP_ATTR_SVCLASS_ID_LIST, BT_SDP_TYPE_SIZE_VAR(BT_SDP_SEQ8, 10),
        ...
        BT_SDP_DATA_ELEM_LIST(
            ...
            {BT_SDP_TYPE_SIZE_VAR(BT_SDP_SEQ8, 5),
             BT_SDP_DATA_ELEM_LIST(
                 ...
                 {BT_SDP_TYPE_SIZE(BT_SDP_UINT8), BT_SDP_ARRAY_16(0x01)})}, )),
    BT_SDP_LIST(
...
int hfp_hf_send_cmd(struct bt_hfp_hf *hf, at_resp_cb_t resp,
  ...
  // //zephyr
  ret =
      vsnprintf((char *)buf->data, (net_buf_tailroom(buf) - 1), format, vargs);
  if (ret < 0) {
...
static void hfp_hf_init(void) {
  ...
  net_buf_init(&hf_pool, CONFIG_BT_MAX_CONN + 1,
               BT_RFCOMM_BUF_SIZE(BT_HF_CLIENT_MAX_PDU), NULL);
#endif
