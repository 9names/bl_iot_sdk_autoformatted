...
struct bt_keys *bt_keys_get_addr(u8_t id, const bt_addr_le_t *addr) {
  ...
  for (i = 0; i < ARRAY_SIZE(key_pool); i++) {
    ...
    if (first_free_slot == ARRAY_SIZE(key_pool) &&
        (!bt_addr_le_cmp(&keys->addr, BT_ADDR_LE_ANY) || !keys->enc_size)) {
      first_free_slot = i;
    }
  }
...
int bt_keys_store(struct bt_keys *keys) {
#if defined(BFLB_BLE)
  int err;
  err = bt_settings_set_bin(NV_KEY_POOL, (const u8_t *)&key_pool[0],
                            sizeof(key_pool));
  return err;
#else
  ...
  str = settings_str_from_bytes(keys->storage_start, BT_KEYS_STORAGE_LEN, val,
                                sizeof(val));
  if (!str) {
...
int bt_keys_load(void) {
  return bt_settings_get_bin(NV_KEY_POOL, (u8_t *)&key_pool[0],
                             sizeof(key_pool), NULL);
}
#endif
