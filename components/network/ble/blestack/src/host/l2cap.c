...
l2cap_chan_alloc_cid(struct bt_conn *conn, struct bt_l2cap_chan *chan) {
...
void bt_l2cap_chan_set_state_debug(struct bt_l2cap_chan *chan,
  ...
  BT_DBG("chan %p psm 0x%04x %s -> %s", chan, chan->psm,
         bt_l2cap_chan_state_str(chan->state), bt_l2cap_chan_state_str(state));
...
struct net_buf *bt_l2cap_create_pdu_timeout(struct net_buf_pool *pool,
  ...
  return bt_conn_create_pdu_timeout(pool, sizeof(struct bt_l2cap_hdr) + reserve,
                                    timeout);
}
...
static void le_conn_req(struct bt_l2cap *l2cap, u8_t ident,
  ...
  BT_DBG("psm 0x%02x scid 0x%04x mtu %u mps %u credits %u", psm, scid, mtu, mps,
         credits);
...
static int l2cap_chan_le_send(struct bt_l2cap_le_chan *ch, struct net_buf *buf,
  ...
  if ((buf == seg || !buf->len) && ch->chan.ops->sent) {
    bt_l2cap_send_cb(ch->chan.conn, ch->tx.cid, seg, l2cap_chan_sdu_sent,
                     &ch->chan);
  } else {
...
static int l2cap_recv(struct bt_l2cap_chan *chan, struct net_buf *buf) {
  ...
  default:
    ...
    l2cap_send_reject(chan->conn, hdr->ident, BT_L2CAP_REJ_NOT_UNDERSTOOD, NULL,
                      0);
    break;
  }
