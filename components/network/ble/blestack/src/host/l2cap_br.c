...
static void connect_optional_fixed_channels(struct bt_l2cap_br *l2cap) {
  ...
  if (l2cap->info_fixed_chan & BIT(BT_L2CAP_CID_BR_SMP)) {
    ...
    chan =
        bt_l2cap_br_lookup_rx_cid(l2cap->chan.chan.conn, BT_L2CAP_CID_BR_SMP);
    if (chan) {
...
static int l2cap_br_info_rsp(struct bt_l2cap_br *l2cap, uint8_t ident,
  ...
  case BT_L2CAP_INFO_FIXED_CHAN:
    ...
    BT_DBG("remote fixed channel mask 0x%02x", l2cap->info_fixed_chan);
...
static int l2cap_br_conn_req_reply(struct bt_l2cap_chan *chan,
                                   uint16_t result) {
...
static void l2cap_br_conf_rsp(struct bt_l2cap_br *l2cap, uint8_t ident,
  ...
  BT_DBG("scid 0x%04x flags 0x%02x result 0x%02x len %u", scid, flags, result,
         opt_len);
  ...
  case BT_L2CAP_CONF_SUCCESS:
    ...
    if (chan->state == BT_L2CAP_CONFIG &&
        atomic_test_bit(BR_CHAN(chan)->flags, L2CAP_FLAG_CONN_RCONF_DONE)) {
...
static void l2cap_br_conf_req(struct bt_l2cap_br *l2cap, uint8_t ident,
  ...
  if (!chan) {
    ...
    l2cap_br_send_reject(conn, ident, BT_L2CAP_REJ_INVALID_CID, &data,
                         sizeof(data));
    return;
  }
...
send_rsp:
  ...
  if (atomic_test_bit(BR_CHAN(chan)->flags, L2CAP_FLAG_CONN_LCONF_DONE) &&
    ...
    BT_DBG("scid 0x%04x rx MTU %u dcid 0x%04x tx MTU %u", BR_CHAN(chan)->rx.cid,
...
static void l2cap_br_conn_rsp(struct bt_l2cap_br *l2cap, uint8_t ident,
  ...
  BT_DBG("dcid 0x%04x scid 0x%04x result %u status %u", dcid, scid, result,
         status);
...
static int l2cap_br_recv(struct bt_l2cap_chan *chan, struct net_buf *buf) {
  ...
  default:
    ...
    l2cap_br_send_reject(chan->conn, hdr->ident, BT_L2CAP_REJ_NOT_UNDERSTOOD,
                         NULL, 0);
    break;
  }
...
void bt_l2cap_br_init(void) {
  ...
  net_buf_init(&br_sig_pool, CONFIG_BT_MAX_CONN,
               BT_L2CAP_BUF_SIZE(L2CAP_BR_MIN_MTU), NULL);
#endif
