...
int multi_adv_set_ad_data(uint8_t *ad_data, const struct bt_data *ad,
                          size_t ad_len) {
  int i, len;
...
void multi_adv_reorder(int inst_num, uint16_t inst_interval[],
                       uint8_t inst_order[]) {
  int i, j;
...
int calculate_offset(uint16_t interval[], uint16_t offset[], int num,
                     int duration) {
...
void multi_adv_schedule_table(int inst_num, uint16_t inst_interval[],
...
int multi_adv_start_adv_instant(struct multi_adv_instant *adv_instant) {
  ...
  ret = bt_le_adv_start_instant(&adv_instant->param, adv_instant->ad,
...
void multi_adv_schedule_timeslot(struct multi_adv_scheduler *adv_scheduler) {
  ...
  //    BT_DBG("multi_adv_schedule_timeslot, num = %d, match = %d", inst_num,
  //    match);
  if (match) {
    ...
    if (match - insts > 1) {
      adv_scheduler->next_slot_offset =
          adv_scheduler->slot_offset + offset_per_instant;
      adv_scheduler->next_slot_clock = adv_scheduler->slot_clock;

      if ((adv_scheduler->next_slot_offset >= (TIME_PRIOD_MS - 2)) &&
          (adv_scheduler->slot_offset <= (TIME_PRIOD_MS + 2))) {
  ...
  next_slot = (adv_scheduler->next_slot_clock - adv_scheduler->slot_clock) *
                  TIME_PRIOD_MS +
              (adv_scheduler->next_slot_offset - adv_scheduler->slot_offset);
...
void multi_adv_new_schedule(void) {
  ...
  for (i = 0; i < MAX_MULTI_ADV_INSTANT; i++) {
    ...
    if (adv_instant) {
      ...
      inst_interval[inst_num] = change_to_tick(adv_instant->param.interval_min,
                                               adv_instant->param.interval_max);
      inst_order[inst_num] = i;
      inst_num++;
    }
  }
  ...
  for (i = 0; i < inst_num; i++) {
    ...
    // adv_instant->instant_id, adv_instant->instant_interval,
    // adv_instant->instant_offset);
  }
