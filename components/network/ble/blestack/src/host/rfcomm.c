...
static void rfcomm_dlc_tx_give_credits(struct bt_rfcomm_dlc *dlc,
  ...
  BT_DBG("dlc %p updated credits %u", dlc, k_sem_count_get(&dlc->tx_credits));
}
...
static void rfcomm_dlc_disconnect(struct bt_rfcomm_dlc *dlc) {
  ...
  case BT_RFCOMM_STATE_CONNECTED:
     ...
     */
    net_buf_put(&dlc->tx_queue, net_buf_alloc(&dummy_pool, K_NO_WAIT));
...
static void rfcomm_dlc_connected(struct bt_rfcomm_dlc *dlc) {
  ...
  thread_dlc = dlc;
  k_thread_create(&dlc->tx_thread, "rfcomm_dlc", CONFIG_BT_RFCOMM_TX_STACK_SIZE,
                  rfcomm_dlc_tx_thread, CONFIG_BT_RFCOMM_TX_PRIO);
...
static void rfcomm_handle_disc(struct bt_rfcomm_session *session,
  ...
  if (dlci) {
    ...
    if (!session->dlcs) {
      ...
      k_delayed_work_submit(&dlc->session->rtx_work, RFCOMM_IDLE_TIMEOUT);
    }
  } else {
    /* Cancel idle timer */
    k_delayed_work_cancel(&session->rtx_work);
...
int bt_rfcomm_dlc_send(struct bt_rfcomm_dlc *dlc, struct net_buf *buf) {
  ...
  if (buf->len > BT_RFCOMM_MAX_LEN_8) {
    ...
    *len = BT_RFCOMM_SET_LEN_16(sys_cpu_to_le16(buf->len - sizeof(*hdr) - 1));
  } else {
  ...
  cr = BT_RFCOMM_UIH_CR(dlc->session->role);
  hdr->address = BT_RFCOMM_SET_ADDR(dlc->dlci, cr);
  hdr->control = BT_RFCOMM_SET_CTRL(BT_RFCOMM_UIH, BT_RFCOMM_PF_UIH_NO_CREDIT);
...
static int rfcomm_recv(struct bt_l2cap_chan *chan, struct net_buf *buf) {
  ...
  case BT_RFCOMM_DM:
    ...
    break;
  default:
    BT_WARN("Unknown/Unsupported RFCOMM Frame type 0x%02x", frame_type);
    break;
  }
...
static struct bt_rfcomm_session *rfcomm_session_new(bt_rfcomm_role_t role) {
  ...
  for (i = 0; i < ARRAY_SIZE(bt_rfcomm_pool); i++) {
    ...
    k_delayed_work_init(&session->rtx_work, rfcomm_session_rtx_timeout);
