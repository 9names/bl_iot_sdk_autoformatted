...
static uint16_t sdp_svc_att_req(struct bt_sdp *sdp, struct net_buf *buf,
  ...
  att_list_len =
      create_attr_list(sdp, record, filter, num_filters, max_att_len,
...
static int sdp_client_chan_connect(struct bt_sdp_client *session) {
  return bt_l2cap_br_chan_connect(session->chan.chan.conn, &session->chan.chan,
                                  SDP_PSM);
}
...
int bt_sdp_get_proto_param(const struct net_buf *buf, enum bt_sdp_proto proto,
  ...
  if (res < 0) {
    BT_WARN("Attribute 0x%04x not found, err %d", BT_SDP_ATTR_PROTO_DESC_LIST,
            res);
    return res;
  }
...
int bt_sdp_get_profile_version(const struct net_buf *buf, uint16_t profile,
  ...
  if (res < 0) {
    BT_WARN("Attribute 0x%04x not found, err %d", BT_SDP_ATTR_PROFILE_DESC_LIST,
            res);
    return res;
  }
