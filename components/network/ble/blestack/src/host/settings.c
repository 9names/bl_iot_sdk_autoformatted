...
static void save_id(struct k_work *work) {
  ...
  BT_INFO("Saving ID");
  err =
      settings_save_one("bt/id", &bt_dev.id_addr, ID_DATA_LEN(bt_dev.id_addr));
  if (err) {
...
int bt_settings_get_bin(const char *key, u8_t *value, size_t exp_len,
                        size_t *real_len) {
  char *str_value;
  ...
  if ((str_value_len % 2) != 0 ||
      (exp_len > 0 && str_value_len > exp_len * 2)) {
    return -1;
  }
...
void bt_settings_save_id(void) {
  ...
  if (bt_check_if_ef_ready())
    return;
  bt_settings_set_bin(NV_LOCAL_ID_ADDR, (const u8_t *)&bt_dev.id_addr[0],
                      sizeof(bt_addr_le_t) * CONFIG_BT_ID_MAX);
#if defined(CONFIG_BT_PRIVACY)
  bt_settings_set_bin(NV_LOCAL_IRK, (const u8_t *)&bt_dev.irk[0],
                      16 * CONFIG_BT_ID_MAX);
#endif // CONFIG_BT_PRIVACY
#endif // CONFIG_BT_SETTINGS
#else
...
void bt_local_info_load(void) {
  ...
  if (dev_name != NULL) {
    len = ((strlen(dev_name) + 1) < CONFIG_BT_DEVICE_NAME_MAX)
              ? (strlen(dev_name) + 1)
              : CONFIG_BT_DEVICE_NAME_MAX;
    memcpy(bt_dev.name, dev_name, len);
  }
#endif
  bt_settings_get_bin(NV_LOCAL_ID_ADDR, (u8_t *)&bt_dev.id_addr[0],
                      sizeof(bt_addr_le_t) * CONFIG_BT_ID_MAX, NULL);
#if defined(CONFIG_BT_PRIVACY)
  bt_settings_get_bin(NV_LOCAL_IRK, (u8_t *)&bt_dev.irk[0][0],
                      16 * CONFIG_BT_ID_MAX, NULL);
#endif
}
#endif // CONFIG_BT_SETTINGS
#endif
