...
int ble_get_smp_paramters(const struct bt_conn *conn,
                          struct smp_parameters *paras) {
  if (!paras) {
    return -1;
  }
...
static u8_t get_io_capa(void) {
  ...
  if (user_smp_paras.set) {
    ...
    if (atomic_test_bit(&smp_test_flag, SMP_IO_CAP_DISPLAY_ONLY) ||
        atomic_test_bit(&smp_test_flag,
                        SMP_AUTH_NO_BONDING_MITM_IO_DISPLAY_ONLY)) {
...
static int smp_f5(const u8_t *w, const u8_t *n1, const u8_t *n2,
                  ...
                  u8_t *ltk) {
  static const u8_t salt[16] = {0x6c, 0x88, 0x83, 0x91, 0xaa, 0xf5, 0xa5, 0x38,
...
static void smp_pairing_br_complete(struct bt_smp_br *smp, u8_t status) {
  ...
  if (status) {
    ...
    if (bt_auth && bt_auth->pairing_failed) {
      bt_auth->pairing_failed(smp->chan.chan.conn, auth_err_get(status));
    }
  } else {
...
static void smp_br_send(struct bt_smp_br *smp, struct net_buf *buf,
                        bt_conn_tx_cb_t cb) {
  bt_l2cap_send_cb(smp->chan.chan.conn, BT_L2CAP_CID_BR_SMP, buf, cb, NULL);
...
static void legacy_distribute_keys(struct bt_smp *smp) {
  ...
  if (smp->local_dist & BT_SMP_DIST_ENC_KEY) {
    ...
    buf = smp_create_pdu(smp, BT_SMP_CMD_MASTER_IDENT, sizeof(*ident));
    if (!buf) {
    ...
    if (atomic_test_bit(smp->flags, SMP_FLAG_BOND)) {
      ...
      memcpy(keys->slave_ltk.val, rand.key, sizeof(keys->slave_ltk.val));
      memcpy(keys->slave_ltk.rand, rand.rand, sizeof(keys->slave_ltk.rand));
      memcpy(keys->slave_ltk.ediv, rand.ediv, sizeof(keys->slave_ltk.ediv));
    }
  }
}
#endif /* !CONFIG_BT_SMP_SC_PAIR_ONLY */
...
static u8_t bt_smp_distribute_keys(struct bt_smp *smp) {
  ...
  if (smp->local_dist & BT_SMP_DIST_ID_KEY) {
    ...
    buf =
        smp_create_pdu(smp, BT_SMP_CMD_IDENT_ADDR_INFO, sizeof(*id_addr_info));
    if (!buf) {
      BT_ERR("Unable to allocate Ident Addr Info buffer");
      return BT_SMP_ERR_UNSPECIFIED;
    }
...
#if defined(CONFIG_BT_STACK_PTS)
void bt_set_mitm(bool enable) { mitm = enable; }

void bt_set_smpflag(smp_test_id id) { atomic_set_bit(&smp_test_flag, id); }

void bt_clear_smpflag(smp_test_id id) { atomic_clear_bit(&smp_test_flag, id); }
#endif

void bt_set_oob_data_flag(bool enable) { oobd_present = enable; }
...
static u8_t get_auth(struct bt_conn *conn, u8_t auth) {
  ...
  if (user_smp_paras.set)
    ...
    if ((get_io_capa() == BT_SMP_IO_NO_INPUT_OUTPUT) ||
        (!mitm && (conn->required_sec_level < BT_SECURITY_L3))) {
#else
  if ((get_io_capa() == BT_SMP_IO_NO_INPUT_OUTPUT) ||
...
bool bt_smp_request_ltk(struct bt_conn *conn, u64_t rand, u16_t ediv,
  ...
  if (!conn->le.keys) {
    conn->le.keys = bt_keys_find(BT_KEYS_LTK_P256, conn->id, &conn->le.dst);
    if (!conn->le.keys) {
      conn->le.keys = bt_keys_find(BT_KEYS_SLAVE_LTK, conn->id, &conn->le.dst);
    }
  }
...
static void le_sc_oob_config_set(struct bt_smp *smp,
  ...
  if (IS_ENABLED(CONFIG_BT_CENTRAL) &&
      smp->chan.chan.conn->role == BT_HCI_ROLE_MASTER) {
    oob_config =
        req_oob_present ? BT_CONN_OOB_REMOTE_ONLY : BT_CONN_OOB_NO_DATA;
...
#else
static inline int smp_self_test(void) { return 0; }
#endif
