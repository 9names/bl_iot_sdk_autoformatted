...
struct _bt_gatt_ccc {
  ...
  bool (*cfg_write)(struct bt_conn *conn, const struct bt_gatt_attr *attr,
                    u16_t value);
...
#define BT_GATT_CUD(_value, _perm)                                             \
  BT_GATT_DESCRIPTOR(BT_UUID_GATT_CUD, _perm, bt_gatt_attr_read_cud, NULL,     \
                     (void *)_value)
...
static inline int bt_gatt_write_without_response(struct bt_conn *conn,
  ...
  return bt_gatt_write_without_response_cb(conn, handle, data, length, sign,
                                           NULL, NULL);
}
