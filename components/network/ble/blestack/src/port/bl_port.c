...
void *k_queue_get(struct k_queue *queue, s32_t timeout) {
  ...
  ret = xQueueReceive(queue->hdl, &msg,
                      t == BL_WAIT_FOREVER ? portMAX_DELAY : ms2tick(t));
  if (ret == pdPASS) {
    return msg;
  } else {
    return NULL;
...
int k_sem_take(struct k_sem *sem, uint32_t timeout) {
  ...
  ret = xSemaphoreTake(sem->sem.hdl,
                       t == BL_WAIT_FOREVER ? portMAX_DELAY : ms2tick(t));
...
int k_thread_create(struct k_thread *new_thread, const char *name,
                    size_t stack_size, k_thread_entry_t entry, int prio) {
...
void k_timer_init(k_timer_t *timer, k_timer_handler_t handle, void *args) {
  ...
  timer->timer.hdl = xTimerCreate("Timer", pdMS_TO_TICKS(1000), 0, args,
                                  (TimerCallbackFunction_t)(timer->handler));
