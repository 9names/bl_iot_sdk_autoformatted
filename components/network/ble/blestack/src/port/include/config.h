...
#if defined(CONFIG_BT_STACK_PTS) &&                                            \
    (defined(PTS_GAP_SLAVER_CONFIG_WRITE_CHARC) ||                             \
     defined(PTS_TEST_CASE_INSUFFICIENT_KEY))
...
/*
 *  Take throuthput test into consideration, set att tx max the same with
 * lowstack tx buffer count. att semaphore determine the max numble packets can
 * send to lowsatck at once.
 */
...
/*
 *  Take throuthput test into consideration, set upperstack conn tx max the same
 * with lowstack tx buffer count.
 */
...
/*******************************Bouffalo Lab
 * Patch******************************/
/*Fix the issue that DHKEY_check_failed error happens in smp procedure if
...
/*Timer/Queue/Sem allocated during connection establishment is not released when
disconnection happens, which cause memory leak issue.*/
...
/*To avoid sevice changed indication sent at the very beginning, without any new
 * service added.*/
#define BFLB_BLE_PATCH_SET_SCRANGE_CHAGD_ONLY_IN_CONNECTED_STATE
#ifdef CONFIG_BT_SETTINGS
/*Semaphore is used during flash operation. Make sure that freertos has already
  run up when it intends to write information to flash.*/
