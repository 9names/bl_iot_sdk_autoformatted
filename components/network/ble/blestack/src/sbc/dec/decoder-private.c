...
OI_CHAR *const OI_Codec_Copyright = "Copyright 2002-2007 Open Interface North "
...
PRIVATE void OI_SBC_ReadScalefactors(OI_CODEC_SBC_COMMON_CONTEXT *common,
  ...
  if (common->frameInfo.nrof_subbands == 8 ||
     ...
     * we need to do is initialize the bitstream.
     */
    ...
    *scale_factor++ = LOW(f);
     ...
     * half-byte boundary, so it's necessary to use the bitstream abstraction to
     * read it, since OI_SBC_ReadSamples will need to pick up in mid-byte.
     */
    OI_BITSTREAM_ReadInit(bs, b);
