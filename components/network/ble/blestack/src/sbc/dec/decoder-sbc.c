...
static OI_STATUS DecodeBody(OI_CODEC_SBC_DECODER_CONTEXT *context,
                            const OI_BYTE *bodyData, OI_INT16 *pcmData,
                            OI_UINT32 *pcmBytes, OI_BOOL allowPartial) {
  OI_BITSTREAM bs;
   ...
   */
  if (*pcmBytes <
          (sizeof(OI_INT16) * frameSamples * context->common.pcmStride) &&
      !allowPartial) {
  ...
  } else if (*pcmBytes < sizeof(OI_INT16) *
                             context->common.frameInfo.nrof_subbands *
  ...
  if (allowPartial) {
    decode_block_count = *pcmBytes / sizeof(OI_INT16) /
                         context->common.pcmStride /
                         context->common.frameInfo.nrof_subbands;
...
OI_STATUS OI_CODEC_SBC_DecoderReset(OI_CODEC_SBC_DECODER_CONTEXT *context,
  ...
  return internal_DecoderReset(context, decoderData, decoderDataBytes,
                               maxChannels, pcmStride, enhanced, msbc_enable);
}
...
OI_STATUS OI_CODEC_SBC_DecodeFrame(OI_CODEC_SBC_DECODER_CONTEXT *context,
   ...
   * subbands. We need to ensure that the SBC parameters for this frame are
   * compatible with the restrictions imposed by the loaded overlays.
   */
  ...
  if (context->common.frameInfo.nrof_channels > context->common.maxChannels) {
    ERROR(("SBC parameters incompatible with number of channels specified "
           "during reset"));
    return OI_STATUS_INVALID_PARAMETERS;
  }
   ...
   * a false syncword, so the header data might be invalid. Make sure we have
   * enough bytes to read in the CRC-protected header, but don't require we have
   * the whole frame. That way, if it turns out that we're acting on bogus
   * header data, we don't stall the decoding process by waiting for data that
