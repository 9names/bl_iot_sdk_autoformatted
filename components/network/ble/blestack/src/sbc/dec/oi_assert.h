...
/** @file
  ...
  compiled into the program, providing for a runtime assertion failure check.
...
/** The macro OI_ASSERT takes a condition argument. If the asserted condition
   ...
   function, OI_AssertFail(), which reports the failure and generates a runtime
   error.
*/
...
/**
   ...
   field offsets, etc. An assertion failure results in compile time error C2118:
   negative subscript. Unfortunately, this elegant macro doesn't work with GCC,
   so it's all commented out for now. Perhaps later.....
*/
