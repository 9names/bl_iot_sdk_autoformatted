 ...
 */
typedef struct {
  OI_UINT8 addr[OI_BD_ADDR_BYTE_SIZE]; /**< Bluetooth device address represented
...
typedef struct {
  ...
  OI_UINT8 base[OI_BT_UUID128_SIZE -
                sizeof(OI_UINT32)]; /**< remainder of 128-bit UUID, array of
                                       8-bit values */
} OI_UUID128;
 ...
 */
typedef struct {
  OI_UINT8 key[OI_BT_LINK_KEY_SIZE]; /**< link key represented as an array of
                                        8-bit values */
} OI_LINK_KEY;
...
typedef struct {
  OI_UINT8
      value[OI_BT_OOB_NUM_BYTES]; /**< same struct used for C and R values */
} OI_OOB_DATA;
...
/** amount of space allocated for a PIN (personal indentification number) in
 * bytes */
#define OI_BT_PIN_CODE_SIZE 16

/** data type for a PIN (PINs are treated as strings, so endianness does not
 * apply.) */
typedef struct {
  OI_UINT8 pin[OI_BT_PIN_CODE_SIZE]; /**< PIN represented as an array of 8-bit
                                        values */
} OI_PIN_CODE;

/** maximum number of SCO connections per device, which is 3 as of
   version 2.0+EDR
...
#define OI_BT_DEV_CLASS_FORMAT_MASK                                            \
  0x000003 /**< Bits 0-1 contain format type. */
#define OI_BT_DEV_CLASS_MINOR_DEVICE_MASK                                      \
  0x0000FC /**< Bits 2-7 contain minor device class value. */
#define OI_BT_DEV_CLASS_MAJOR_DEVICE_MASK                                      \
  0x001F00 /**< Bits 8-12 contain major device class value. */
#define OI_BT_DEV_CLASS_MAJOR_SERVICE_MASK                                     \
  0xFFE000 /**< Bits 13-23 contain major service class value. */
