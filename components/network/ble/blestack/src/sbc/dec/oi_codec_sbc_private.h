...
OI_UINT computeBitneed(OI_CODEC_SBC_COMMON_CONTEXT *common, OI_UINT8 *bitneeds,
...
OI_INT adjustToFitBitpool(const OI_UINT bitpool, OI_UINT32 *bitneeds,
                          const OI_UINT subbands, OI_UINT bitcount,
                          OI_UINT *excess);

OI_INT allocAdjustedBits(OI_UINT8 *dest, OI_INT bits, OI_INT excess);

OI_INT allocExcessBits(OI_UINT8 *dest, OI_INT excess);
...
OI_STATUS internal_DecoderReset(OI_CODEC_SBC_DECODER_CONTEXT *context,
                                ...
                                OI_UINT32 decoderDataBytes, OI_BYTE maxChannels,
                                OI_BYTE pcmStride, OI_BOOL enhanced,
                                OI_BOOL msbc_enable);

OI_UINT16 OI_SBC_CalculateFrameAndHeaderlen(OI_CODEC_SBC_FRAME_INFO *frame,
                                            OI_UINT *headerLen_);
...
PRIVATE OI_UINT8 OI_SBC_CalculateChecksum(OI_CODEC_SBC_FRAME_INFO *frame,
                                          OI_BYTE const *data);
...
PRIVATE void cosineModulateSynth4(SBC_BUFFER_T *RESTRICT out,
                                  OI_INT32 const *RESTRICT in);
PRIVATE void SynthWindow40_int32_int32_symmetry_with_sum(
    OI_INT16 *pcm, SBC_BUFFER_T buffer[80], OI_UINT strideShift);
...
PRIVATE void analyze4_generated(SBC_BUFFER_T analysisBuffer[RESTRICT 40],
                                OI_INT16 *pcm, OI_UINT strideShift,
                                OI_INT32 subband[4]);
...
#ifdef SBC_ENHANCED
PRIVATE void
analyze8_enhanced_generated(SBC_BUFFER_T analysisBuffer[RESTRICT 112],
                            OI_INT16 *pcm, OI_UINT strideShift,
                            OI_INT32 subband[8]);
#endif
...
void OI_SBC_ReadHeader(OI_CODEC_SBC_COMMON_CONTEXT *common,
                       const OI_BYTE *data);
...
PRIVATE void OI_SBC_SynthFrame(OI_CODEC_SBC_DECODER_CONTEXT *context,
                               OI_INT16 *pcm, OI_UINT start_block,
                               OI_UINT nrof_blocks);
OI_INT32 OI_SBC_Dequant(OI_UINT32 raw, OI_UINT scale_factor, OI_UINT bits);
PRIVATE OI_BOOL OI_SBC_ExamineCommandPacket(
    OI_CODEC_SBC_DECODER_CONTEXT *context, const OI_BYTE *data, OI_UINT32 len);
PRIVATE void OI_SBC_GenerateTestSignal(OI_INT16 pcmData[][2],
                                       OI_UINT32 sampleCount);
