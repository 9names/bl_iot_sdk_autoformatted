...
/**
 * OI_STATUS must fit in 16 bits, so status codes can range from 0 to 66535,
 * inclusive.
 */
...
typedef enum {
  ...
  OI_HCI_DEVICE_NOT_INITIALIZED =
      627, /**< HCI: commands from upper layers disallowed until device is up
              and running */
  ...
  OI_SDP_NO_ATTRIBUTE_LIST_TO_REMOVE =
      841, /**< SDP: Attempt to remove a non-existent attribute list from a
              service record */
  OI_SDP_ATTRIBUTE_LIST_ALREADY_ADDED =
      842, /**< SDP: An attribute list has already been added to the service
              record */
  ...
  OI_DEVMGR_ENUM_UNEXPECTED_INQ_COMPLETE =
      1320, /**< DEVMGR: inquiry complete event in inappropriate enumeration
               state */
  OI_DEVMGR_ENUM_UNEXPECTED_INQ_RESULT =
      1321, /**< DEVMGR: inquiry result event in inappropriate enumeration state
             */
  OI_DEVMGR_ENUM_DATABASE_FULL = 1322, /**< DEVMGR: device enumeration, database
                                          is full, couldn't add a new device */
  OI_DEVMGR_ENUM_INQUIRIES_OVERLAP =
      1323, /**< DEVMGR: device enumeration, periodic inquiries occurring too
               close together */
  OI_DEVMGR_UNKNOWN_LINK_TYPE =
      1324, /**< DEVMGR: HCI connect request with unkown link type */
  OI_DEVMGR_PARAM_IO_ACTIVE =
      1325, /**< DEVMGR: request for parameter read/write while param read/write
               active */
  ...
  OI_DEVMGR_NOT_IN_MASTER_MODE =
      1332, /**< DEVMGR: OI_DEVMGR EndMasterMode without prior
...
in the queue; \n timeout configurable via @ref
OI_CONFIG_DEVMGR::connectQueueTimeoutSecs "connectQueueTimeoutSecs" */
  ...
  OI_SECMGR_UNKNOWN_ENCRYPT_VALUE =
      1411, /**< SECMGR: got EncryptionChange event, unknown encryption enable
               value */
  ...
  OI_SECMGR_UNIT_KEY_UNSUPPORTED = 1415, /**< SECMGR: authentication failed due
                                            to non-support of unit keys */
  ...
  OI_SECMGR_INSUFFICIENT_LINK_KEY =
      1419, /**< SECMGR: link key type is not sufficient to meet service
               requirements */
  ...
  OI_OBEX_FOLDER_BROWSING_NOT_ALLOWED =
      1753, /**< OBEX: Indicates that an FTP server does not allow folder
               browsing */
  ...
  OI_HANDSFREE_SERVICE_NOT_STARTED =
      1803, /**< HANDSFREE: Cannot connect to handsfree AG if handsfree service
               not started */
  ...
  OI_HANDSFREE_FEATURE_NOT_SUPPORTED =
      1808, /**< HANDSFREE: Local or remote feature not supported for requested
               command */

  OI_HEADSET_SERVICE_NOT_STARTED = 1901, /**< HEADSET: Cannot connect to headset
                                            AG if headset service not started */
  OI_HEADSET_AG_SERVICE_NOT_STARTED =
      1902, /**< HEADSET: Cannot connect to headset device if headset AG service
               not started */
  ...
  OI_HID_HOST_SERVICE_NOT_STARTED =
      2601, /**< HID: Cannot connect to a HID device unless HID host is started
             */
  ...
  OI_UNICODE_SOURCE_EXHAUSTED =
      3201, /**< Unicode Conversion: Incomplete Unicode character at end of
               source buffer. */
  OI_UNICODE_DESTINATION_EXHAUSTED =
      3202, /**< Unicode Conversion: Destination buffer not large enough to hold
  ...
  OI_AVRCP_RESPONSE_INVALID_PDU =
      3305, /**< AVRCP: Command rejected: target received a PDU that it did not
               understand. */
  OI_AVRCP_RESPONSE_INVALID_PARAMETER =
      3306, /**< AVRCP: Command rejected: target received a PDU with a parameter
               ID that it did not understand. */
   ...
   * by applications. However, because of differences between compilers, and
   * differences between 16-bit and 32-bit platforms custom status codes should
   * be in the 16-bit range, so status codes can range from 0 to 65534,
   * inclusive (65535 is reserved)
   */
  OI_STATUS_RESERVED_FOR_APPS = 10000,

  OI_STATUS_NONE =
      0xffff /**< Special status code to indicate that there is no status. (Only
                to be used for special cases involving OI_SLOG_ERROR() and
                OI_SLOG_WARNING().) */
