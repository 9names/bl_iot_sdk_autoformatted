...
/** @file
 *
 * This file provides time type definitions and interfaces to time-related
 * functions.
 *
...
OI_INT16 OI_Time_Compare(OI_TIME *T1, OI_TIME *T2);
 ...
 * @param Later a time value later than Sooner
 *
 * @note The result is an OI_UINT32 value so this function only works for time
 * intervals that are less than about 50 days.
 *
 * @return the time interval between the two times = (Later - Sooner)
 */

OI_UINT32 OI_Time_IntervalMsecs(OI_TIME *Sooner, OI_TIME *Later);

/**
 * This function answers the question, Have we reached or gone past the target
 * time?
 *
