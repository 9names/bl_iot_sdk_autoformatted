...
void SBC_Encoder(SBC_ENC_PARAMS *pstrEncParams) {
  ...
  do {
    ...
    pstrEncParams->ps16NextPcmBuffer +=
        s32Ch * s32NumOfBlocks; /* in case of multible sbc frame to encode
    ...
    if (pstrEncParams->s16ChannelMode == SBC_JOINT_STEREO) {
      ...
      for (s32Sb = 0; s32Sb < s32NumOfSubBands - 1; s32Sb++) {
        ...
        if ((*ps16ScfL + *(ps16ScfL + s32NumOfSubBands)) >
            (SINT16)(u32CountSum + u32CountDiff)) {
  ...
  pstrEncParams->u8NumPacketToEncode =
      1; /* default is one for retrocompatibility purpose */
}
...
void SBC_Encoder_Init(SBC_ENC_PARAMS *pstrEncParams) {
  ...
  if (pstrEncParams->sbc_mode != SBC_MODE_MSBC) {
    ...
    if ((pstrEncParams->s16ChannelMode == SBC_JOINT_STEREO) ||
      ...
      s16Bitpool =
          (SINT16)(((pstrEncParams->s16NumOfSubBands *
