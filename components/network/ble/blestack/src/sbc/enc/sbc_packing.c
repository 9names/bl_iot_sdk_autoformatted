...
void EncPacking(SBC_ENC_PARAMS *pstrEncParams) {
  ...
  *pu8PacketPtr = u8CRC;
  pstrEncParams->pu8NextPacket +=
      pstrEncParams
          ->u16PacketLength; /* move the pointer to the end in case there is
                                more than one frame to encode */
}
