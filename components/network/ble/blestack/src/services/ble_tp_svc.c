...
void indicate_rsp(struct bt_conn *conn, const struct bt_gatt_attr *attr,
                  u8_t err) {
  free(ind_params);
  printf("%s, receive comfirmation, err:%d\n", __func__, err);
}

static int bl_tp_send_indicate(struct bt_conn *conn,
...
*/
static void ble_tp_ind_ccc_changed(const struct bt_gatt_attr *attr,
                                   u16_t value) {
  int err = -1;
  char data[9] = {0x01, 0x02, 0x03, 0x04, 0x05, 0x06, 0x07, 0x08, 0x09};
  ...
  if (value == BT_GATT_CCC_INDICATE) {
    err = bl_tp_send_indicate(
        ble_tp_conn, get_attr(BT_CHAR_BLE_TP_IND_ATTR_VAL_INDEX), data, 9);
...
static void ble_tp_notify_task(void *pvParameters) {
  ...
  while (1) {
    err =
        bt_gatt_notify(ble_tp_conn, get_attr(BT_CHAR_BLE_TP_NOT_ATTR_VAL_INDEX),
                       data, (tx_mtu_size - 3));
...
*/
static void ble_tp_not_ccc_changed(const struct bt_gatt_attr *attr,
                                   u16_t value) {
  printf("ccc:value=[%d]\r\n", value);
  ...
  if (tp_start) {
    ...
    if (value == BT_GATT_CCC_NOTIFY) {
      if (xTaskCreate(ble_tp_notify_task, (char *)"bletp", 256, NULL, 15,
                      &ble_tp_task_h) == pdPASS) {
        created_tp_task = 1;
...
static struct bt_gatt_attr attrs[] = {
    ...
    BT_GATT_CHARACTERISTIC(BT_UUID_CHAR_BLE_TP_RD, BT_GATT_CHRC_READ,
                           BT_GATT_PERM_READ, ble_tp_recv_rd, NULL, NULL),

    BT_GATT_CHARACTERISTIC(BT_UUID_CHAR_BLE_TP_WR,
                           BT_GATT_CHRC_WRITE | BT_GATT_CHRC_WRITE_WITHOUT_RESP,
                           BT_GATT_PERM_WRITE | BT_GATT_PERM_PREPARE_WRITE,
                           NULL, ble_tp_recv_wr, NULL),

    BT_GATT_CHARACTERISTIC(BT_UUID_CHAR_BLE_TP_IND, BT_GATT_CHRC_INDICATE, NULL,
                           NULL, NULL, NULL),

    BT_GATT_CCC(ble_tp_ind_ccc_changed, BT_GATT_PERM_READ | BT_GATT_PERM_WRITE),

    BT_GATT_CHARACTERISTIC(BT_UUID_CHAR_BLE_TP_NOT, BT_GATT_CHRC_NOTIFY, NULL,
                           NULL, NULL, NULL),
