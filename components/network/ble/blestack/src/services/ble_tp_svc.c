...
static void ble_tp_notify_task(void *pvParameters) {
  ...
  while (1) {
    err =
        bt_gatt_notify(ble_tp_conn, get_attr(BT_CHAR_BLE_TP_TX_ATTR_VAL_INDEX),
                       data, (tx_mtu_size - 3));
...
*/
static void ble_tp_ccc_cfg_changed(const struct bt_gatt_attr *attr,
                                   u16_t value) {
...
NAME
    ble_tp_recv
*/
static int ble_tp_recv(struct bt_conn *conn, const struct bt_gatt_attr *attr,
...
static struct bt_gatt_attr attrs[] = {
    ...
    BT_GATT_CHARACTERISTIC(BT_UUID_CHAR_BLE_TP_TX, BT_GATT_CHRC_NOTIFY,
                           BT_GATT_PERM_READ, NULL, NULL, NULL),

    BT_GATT_CCC(ble_tp_ccc_cfg_changed, BT_GATT_PERM_READ | BT_GATT_PERM_WRITE),

    BT_GATT_CHARACTERISTIC(
        BT_UUID_CHAR_BLE_TP_RX, BT_GATT_CHRC_WRITE_WITHOUT_RESP,
        BT_GATT_PERM_READ | BT_GATT_PERM_WRITE, NULL, ble_tp_recv, NULL)};
