...
void oad_send_image_identity_to_servicer(struct bt_conn *conn, u8_t *data,
                                         u16_t len);
void oad_send_block_resp_to_servicer(struct bt_conn *conn, u8_t *data,
                                     u8_t len);
static void oad_start_identity(char *pcWriteBuffer, int xWriteBufferLen,
                               int argc, char **argv);
static void oad_discovery(char *pcWriteBuffer, int xWriteBufferLen, int argc,
                          char **argv);
static void oad_subscribe(char *pcWriteBuffer, int xWriteBufferLen, int argc,
                          char **argv);
static void oad_start_update(char *pcWriteBuffer, int xWriteBufferLen, int argc,
                             char **argv);
...
void oad_block_req_to_uart(struct oad_file_info *file_info,
                           uint32_t file_offset, uint8_t data_len) {
  struct oad_cmdproc_req_t req;
...
static u8_t oad_discover_func(struct bt_conn *conn,
  ...
  case BT_GATT_DISCOVER_INCLUDE:
    ...
    printf("Include %s found: attr handle %x, start %x, end %x\r\n", str,
           attr->handle, gatt_include->start_handle, gatt_include->end_handle);
    break;
  default:
...
void oad_client_notify_handler(void *buf, u16_t len) {
  ...
  case OAD_CMD_IMAG_IDENTITY:
    if (len == sizeof(struct oad_image_identity_t) + 1) {
      ...
      printf("Length (%d) sizeof (%d) errors\r\n", len,
             sizeof(struct oad_image_identity_t));
    }
    break;
  ...
  case OAD_CMD_IMAG_BLOCK_REQ:
    if (len == sizeof(struct oad_block_req_t) + 1) {
      ...
      oad_block_req_to_uart(&(req.file_info), req.file_offset,
                            TRANSPORT_FILE_SIZE);
    } else {
      printf("Length (%d) sizeof (%d) errors\r\n", len,
             sizeof(struct oad_block_req_t));
    }
    break;
...
void oad_cli_register(void) { return; }
