...
static bool check_data_valid(struct oad_file_info *file_info) {
  if (file_info->manu_code != oad_env.file_info.manu_code ||
      file_info->file_ver != oad_env.upgrd_file_ver)
    return false;
...
static void oad_notify_block_req(struct bt_conn *conn) {
  struct net_buf_simple *buf =
      NET_BUF_SIMPLE(sizeof(struct oad_block_req_t) + OAD_OPCODE_SIZE);
...
static void oad_notify_upgrd_end(struct bt_conn *conn, u8_t status) {
  struct net_buf_simple *buf =
      NET_BUF_SIMPLE(sizeof(struct oad_upgrd_end_t) + OAD_OPCODE_SIZE);
...
static void oad_notity_image_identity(struct bt_conn *conn) {
  struct net_buf_simple *buf =
      NET_BUF_SIMPLE(sizeof(struct oad_image_identity_t));
...
void oad_upgrade(struct k_work *work) {
  ...
  bt_settings_set_bin(NV_IMG_info, (uint8_t *)&ef_info,
                      sizeof(struct oad_ef_info));
...
static u8_t oad_write_flash(u8_t *data, u16_t len) {
  ...
  if (!oad_env.new_img_addr) {
    if (hal_boot2_partition_addr_inactive(
            "FW", (uint32_t *)&oad_env.new_img_addr, &size)) {
  ...
  if (oad_env.w_img_end_addr <= oad_env.new_img_addr &&
      !oad_env.w_img_end_addr) {
...
static u8_t oad_image_data_handler(struct bt_conn *conn, const u8_t *data,
                                   u16_t len) {
  u16_t left_size = 0;
  u16_t wlen = 0;
#if defined(CONFIG_BT_SETTINGS)
  ...
  if (wData.wdata_buf) {
    ...
    if (left_size >= len) {
      ...
      if (wData.index >= OTA_WRITE_FLASH_SIZE &&
          wData.index <= WBUF_SIZE(conn)) {
        wlen = wData.index - OTA_WRITE_FLASH_SIZE;
        oad_write_flash(wData.wdata_buf, OTA_WRITE_FLASH_SIZE);
#if defined(CONFIG_BT_SETTINGS)
        write_count += 1;
        struct oad_ef_info ef_info;
        memcpy(&ef_info.file_info, &oad_env.file_info,
               sizeof(struct oad_file_info));
        ...
        bt_settings_set_bin(NV_IMG_info, (uint8_t *)&ef_info,
                            sizeof(struct oad_ef_info));
        printf("ef_info: file ver(%d) manu code(0x%x) file offset(0x%x) "
               "last_adder (0x%x)\r\n",
               ef_info.file_info.file_ver, ef_info.file_info.manu_code,
               ef_info.file_offset, ef_info.last_wflash_addr);
#endif
        wData.index = 0;
        memcpy((wData.wdata_buf + wData.index),
               (wData.wdata_buf + OTA_WRITE_FLASH_SIZE), wlen);
        wData.index += wlen;
      }
...
static void oad_image_info_handler(struct bt_conn *conn, const u8_t *data,
                                   u16_t len) {
  oad_notify_image_info(conn);
}

static u8_t oad_image_block_resp_handler(struct bt_conn *conn, const u8_t *data,
                                         u16_t len) {
...
static void oad_image_identity_handler(struct bt_conn *conn, const u8_t *data,
                                       u16_t len) {
  struct oad_image_identity_t *identity = (struct oad_image_identity_t *)(data);

  printf("File size=[0x%x] [0x%x] [0x%x] [0x%x]\r\n", identity->file_size,
         identity->file_info.file_ver, identity->file_info.manu_code,
         identity->crc32);
#if defined(CONFIG_BT_SETTINGS)
  size_t llen = 0;
  struct oad_ef_info ef_info;
  ...
  memset(&ef_info, 0, sizeof(struct oad_ef_info));
  bt_settings_get_bin(NV_IMG_info, (uint8_t *)&ef_info,
                      sizeof(struct oad_ef_info), &llen);
  printf("ef_info: file ver(%d) manu code(0x%x) file offset(0x%x) last_adder "
         "(0x%x)\r\n",
         ef_info.file_info.file_ver, ef_info.file_info.manu_code,
         ef_info.file_offset, ef_info.last_wflash_addr);
#endif
  ...
  if (identity->file_info.manu_code == oad_env.file_info.manu_code &&
      (app_check_cb)(oad_env.file_info.file_ver,
                     identity->file_info.file_ver)) {
...
static void oad_recv_callback(struct bt_conn *conn, const u8_t *data,
                              u16_t len) {
  if (len) {
    if (*data == OAD_CMD_IMAG_IDENTITY &&
        ((len - 1) == sizeof(struct oad_image_identity_t))) {
