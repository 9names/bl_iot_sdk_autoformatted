...
static ssize_t read_data(struct bt_conn *conn, const struct bt_gatt_attr *attr,
  ...
  /*As we donot know what info remote wants to read currently, respond value 0
    by default. This part will be implemented according to specific
    requirements.*/
  u8_t value = 0x00;
...
static ssize_t write_data(struct bt_conn *conn, const struct bt_gatt_attr *attr,
  ...
  /*In future, segmented frame will be implemented, below if condition need to
    be changed if segemented frame have the same seq num*/
  if (remote_seq_num != WIFIPROV_INVALID_SEQ &&
      rcvd_frm.sequence_number <= remote_seq_num) {
  ...
  if (remote_need_ack(rcvd_frm.frame_control)) {
    ack_buf[0] = WIFIPROV_TYPE_CTRL_FRAME |
                 (SUB_TYPE_CTRL_ACK << WIFIPROV_FRAME_TYPE_OFFSET);
