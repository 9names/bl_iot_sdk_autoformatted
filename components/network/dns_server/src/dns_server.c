...
static void dns_server_send(struct dns_server_ctx *dns_ctx) {
  ...
  struct dns_table_entry dns_server_table = {.txid = DNS_SERVER_ID,
  ...
  if (rp != NULL) {
    ...
    pbuf_realloc(rp,
                 query_idx + SIZEOF_DNSANS_HDRQUE); // shrink to the real size
    udp_sendto(dns_ctx->upcb1, rp, dns_ctx->addr1, dns_ctx->port1);
    pbuf_free(rp);
  }
}

void get_dns_request(void *arg, struct udp_pcb *upcb, struct pbuf *p,
  ...
  if (p->tot_len < SIZEOF_DNSANS_HDR) {
    ...
    pbuf_copy_partial(p, dns_ctx->name,
                      len = p->tot_len > sizeof(dns_ctx->name) - 1
                                ? sizeof(dns_ctx->name) - 1
                                : p->tot_len,
                      SIZEOF_DNSANS_HDR);
