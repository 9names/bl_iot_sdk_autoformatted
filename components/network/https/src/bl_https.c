...
static int proc_http_head(const uint8_t *buf, int buf_len, int *status_code,
                          int *body_start_off) {
  ...
  if (!((memcmp(buf, "HTTP/1.0 ", 9) == 0) ||
        (0 == memcmp(buf, "HTTP/1.1 ", 9)))) {
    return BL_HTTPSC_RET_HTTP_ERR;
  }

  memcpy(status_code_buf, buf + 9, 3);
  if (!(isdigit(status_code_buf[0]) && isdigit(status_code_buf[1]) &&
        isdigit(status_code_buf[2]))) {
    return BL_HTTPSC_RET_HTTP_ERR;
  }
 ...
 * parm res_len(in, out): pass in addr of A, A stores capacity of buffer
 * response pass out https response body length(ie no http header) out: HTTP
 * status code or BL_HTTPSC error code
 */
int https_request(const char *server, uint16_t port, const uint8_t *request,
                  int req_len, uint8_t *response, int *res_len) {
  int count = 0;
      ...
      while (1) {
        ...
        if (rcv_ret > 0) {
          ...
          if (!http_head_proced) {
            ...
            int proc_head_r =
                proc_http_head(rcv_buf, rcv_ret, &status_code, &body_start_off);
...
void test_0(void) {
  ...
  resp_len = 16 * 1024;
  status_code =
      https_request(host, 443, send_buf, sizeof(send_buf) - 1, buf, &resp_len);
