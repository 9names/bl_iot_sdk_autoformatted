...
static void bl_debug(void *ctx, int level, const char *file, int line,
                     const char *str) {
  ((void)level);
...
int32_t blTcpSslConnect(const char *dst, uint16_t port) {
  ...
  if ((ret = mbedtls_ctr_drbg_seed(&bl_hsbuf->ctr_drbg, mbedtls_entropy_func,
                                   &bl_hsbuf->entropy, NULL, 0)) != 0) {
    return BL_TCP_CREATE_CONNECT_ERR;
  }

#if defined(BL_VERIFY)
  ret = mbedtls_x509_crt_parse(&bl_hsbuf->cacert,
                               (const unsigned char *)bl_test_cli_key_rsa,
                               bl_test_cas_pem_len);
  ...
  if ((ret = mbedtls_ssl_config_defaults(&bl_hsbuf->conf, MBEDTLS_SSL_IS_CLIENT,
  ...
  mbedtls_ssl_conf_rng(&bl_hsbuf->conf, mbedtls_ctr_drbg_random,
                       &bl_hsbuf->ctr_drbg);
  ...
  if (flags < 0 ||
      fcntl(bl_hsbuf->server_fd.fd, F_SETFL, flags | O_NONBLOCK) < 0) {
    printf("ssl fcntl: %s\r\n", strerror(errno));
  ...
  if (connect(bl_hsbuf->server_fd.fd, (struct sockaddr *)&servaddr,
  ...
  // todo
  // mbedtls_ssl_set_bio(&bl_hsbuf->ssl, &bl_hsbuf->server_fd, mbedtls_net_send,
  // mbedtls_net_recv, mbedtls_net_recv_timeout); //noblock
  mbedtls_ssl_set_bio(&bl_hsbuf->ssl, &bl_hsbuf->server_fd, mbedtls_net_send,
