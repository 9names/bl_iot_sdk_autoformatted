...
static void wifiprov_ccc_cfg_changed(const struct bt_gatt_attr *attr,
                                     u16_t value);
...
static void scan_complete_cb(void *param) {
  ...
  memcpy(&gp_index->ap_item[gp_index->w_ap_item], item,
         sizeof(blesync_wifi_item_t));
  gp_index->w_ap_item++;
}
...
static ssize_t read_data(struct bt_conn *conn, const struct bt_gatt_attr *attr,
  ...
  ret = bt_gatt_attr_read(conn, attr, buf, len, offset, gp_index->attr_read_buf,
                          gp_index->rbuf_len);
  gp_index->rbuf_len = 0;
  return ret;
}
...
static int __recv_event(void *p_drv, struct pro_event *p_event) {
  ...
  if (p_event->type == PROTOCOL_EVENT_DATA) {
    ...
    case CMD_WIFI_DATA_GET:
      ...
      sprintf(bssid, "%02X:%02X:%02X:%02X:%02X:%02X", p_item->bssid[0],
              p_item->bssid[1], p_item->bssid[2], p_item->bssid[3],
              p_item->bssid[4], p_item->bssid[5]);
      ...
      // 0: open; 1:wep; 2:WPA/WPA2 - PSK; 3: WPA/WPA2 - Enterprise; 0xFF:
      // unknown
      cJSON_AddNumberToObject(p_root, "auth", p_item->auth);
      json_str = cJSON_Print(p_root);

      pro_trans_layer_ack_read(gp_index->pro_handle, json_str,
                               strlen(json_str));
      cJSON_Delete(p_root);
      vPortFree(json_str);
    ...
    case CMD_WIFI_STATE_GET:
      if (gp_index->wifi_func->local_wifi_state_get) {
        ...
        sprintf(bssid, "%02X:%02X:%02X:%02X:%02X:%02X",
                gp_index->state.bssid[0], gp_index->state.bssid[1],
                gp_index->state.bssid[2], gp_index->state.bssid[3],
                gp_index->state.bssid[4], gp_index->state.bssid[5]);
        p_root = cJSON_CreateObject();
    ...
    case CMD_GET_VERSION:
      pro_trans_layer_ack_read(gp_index->pro_handle, BLSYNC_BLE_VERSION,
...
static void __bl_ble_sync_task(void *p_arg) {
  ...
  index->xQueue1 =
      xQueueCreateStatic(BLE_PROV_QUEUE_NUMS, sizeof(index->buf[0]),
                         (uint8_t *)index->buf, &index->xQueueBuffer);

  while (!index->stop_flag) {

    if (xQueueReceive(index->xQueue1, &recv_buf,
                      (TickType_t)PRO_CONFIG_TIMEOUT) == pdTRUE) {
      if (index->p_cur_conn) {
...
#endif
      if (pro_trans_read(index->pro_handle, recv_buf.buf, recv_buf.len,
                         protocol_mtu) != PRO_OK) {
...
static struct bt_gatt_attr attrs[] = {
    ...
    BT_GATT_CCC(wifiprov_ccc_cfg_changed,
...
int bl_ble_sync_start(bl_ble_sync_t *index, struct blesync_wifi_func *func,
                      pfn_complete_cb_t cb, void *cb_arg) {
...
#if defined(CONFIG_ZIGBEE_PROV)
int bl_blezb_sync_start(bl_ble_sync_t *index, struct blesync_zb_func *func,
                        pfn_complete_cb_t cb, void *cb_arg) {
  if (index == NULL || func == NULL) {
    blog_info("ble sync init failed\r\n");
    return -1;
...
static void vnd_sync_get(struct bt_mesh_model *model,
  ...
  if (gp_index->rbuf_len < BT_MESH_TX_VND_SDU_MAX_SHORT) {
    ...
    net_buf_simple_add_mem(&msg, gp_index->attr_read_buf,
                           BT_MESH_TX_VND_SDU_MAX_SHORT);
...
int bl_blemesh_sync_start(bl_ble_sync_t *index, struct blesync_wifi_func *func,
                          pfn_complete_cb_t cb, void *cb_arg) {
  if (index == NULL || func == NULL) {
    blog_info("ble sync init failed\r\n");
    return -1;
