...
static int __pack_encrypt_read(void *p_drv, uint8_t pack_type, uint8_t *dst_id,
                               uint8_t *dst_buf, uint8_t *dst_len,
                               uint8_t *src_buf, size_t src_lenght) {
  ...
  return payload_read(p_enc->pyld_handle, dst_buf, dst_id, dst_len, src_buf,
                      src_lenght);
}

static int __pack_encrypt_write(void *p_drv, uint8_t pack_type, uint8_t src_id,
                                uint8_t *dst_buf, uint8_t *dst_len,
  ...
  return payload_write(p_enc->pyld_handle, dst_buf, dst_len, src_id, src_buf,
                       src_len, remain_len);
}

const static struct pro_enc_func __g_enc_func = {__pack_encrypt_read,
