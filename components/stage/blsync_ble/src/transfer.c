...
static int __protocol_ack(pro_handle_t handle, struct general_head *p_gen,
                          uint8_t ack_code, uint8_t *ack_buf, uint8_t len);
...
static int __pack_trans(pro_handle_t handle, struct general_head *p_general,
  ...
  ret = encrypt_layer_read(handle->enc_handle, pack_type, &ev_id,
                           handle->pyld_buf + handle->tol_len_now, &dst_len,
                           src_buf, src_len);
...
__end:
  ...
  if (BIT_GET(p_general->ctrl, PRO_CTRL_ACK_BIT) == 0) {
    __protocol_ack(handle, p_general, ret, handle->ack_buf, handle->ack_len);
  }
  handle->ack_len = 0;
...
static int __frag_section(pro_handle_t handle, struct general_head *p_general,
                          uint8_t *p_buf, size_t bytes) {
...
static int __protocol_send(pro_handle_t handle, uint8_t seq, uint8_t ack,
                           uint8_t type, const void *p_data, uint16_t length) {
  ...
  while (!frag_end) {
    ...
    ret = encrypt_layer_write(handle->enc_handle, PRO_TYPE_DATA, type, dst_buf,
                              &dst_len, (uint8_t *)p_data + tol_len_now,
...
static int __protocol_ack(pro_handle_t handle, struct general_head *p_gen,
                          uint8_t ack_code, uint8_t *ack_data, uint8_t len) {
  return __protocol_send(handle, p_gen->seq, PRO_ACK_PACK, ack_code, ack_data,
                         len);
}

int pro_trans_write(pro_handle_t handle, const void *p_data, uint16_t length) {
  return __protocol_send(handle, 0, PRO_NOT_ACK_PACK, 0, p_data, length);
}

int pro_trans_read(pro_handle_t handle, const void *buf, size_t bytes,
                   uint16_t mtu) {
  struct general_head head;
  uint8_t *p_buf;
  int ret;
...
int pro_trans_layer_ack_read(pro_handle_t handle, const void *ack_buf,
                             size_t bytes) {
  if ((!handle) || (!ack_buf)) {
    return -PRO_ERROR;
  }
...
pro_handle_t pro_trans_init(struct pro_dev *p_dev,
                            const struct pro_func *p_func, void *p_drv) {
...
void pro_trans_reset(pro_handle_t handle) { __clear_dev(handle); }
