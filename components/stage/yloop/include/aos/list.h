...
static inline void dlist_init(dlist_t *node) { node->next = node->prev = node; }
...
static inline int slist_empty(const slist_t *head) { return !head->next; }

static inline void slist_init(slist_t *head) { head->next = 0; }
...
#define slist_for_each_entry(queue, node, type, member)                        \
  for (node = aos_container_of((queue)->next, type, member); &node->member;    \
       node = aos_container_of(node->member.next, type, member))
...
#define slist_for_each_entry_safe(queue, tmp, node, type, member)              \
  for (node = aos_container_of((queue)->next, type, member),                   \
      tmp = (queue)->next ? (queue)->next->next : NULL;                        \
       &node->member; node = aos_container_of(tmp, type, member),              \
