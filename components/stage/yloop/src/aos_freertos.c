...
int aos_get_hz(void) { return 100; }

const char *aos_version_get(void) { return "aos-linux-xxx"; }
...
int aos_task_new(const char *name, void (*fn)(void *), void *arg,
  ...
  xHandle = xTaskCreateStatic(dfl_entry, name, stack_size, targ, 10, stack,
                              &task->fTask);
  if (xHandle == NULL) {
    vPortFree(task);
    vPortFree(stack);
...
void aos_task_exit(int code) { vTaskDelete(NULL); }
...
void aos_mutex_free(aos_mutex_t *mutex) { vSemaphoreDelete(mutex->hdl); }

int aos_mutex_lock(aos_mutex_t *mutex, unsigned int ms) {
  if (mutex) {
    xSemaphoreTake(mutex->hdl,
                   ms == AOS_WAIT_FOREVER ? portMAX_DELAY : ms2tick(ms));
  }
  return 0;
}
...
int aos_mutex_is_valid(aos_mutex_t *mutex) { return mutex->hdl != NULL; }
...
int aos_sem_wait(aos_sem_t *sem, unsigned int ms) {
  ...
  int ret = xSemaphoreTake(sem->hdl, ms == AOS_WAIT_FOREVER ? portMAX_DELAY
                                                            : ms2tick(ms));
...
int aos_sem_is_valid(aos_sem_t *sem) { return sem && sem->hdl != NULL; }

void aos_sem_signal_all(aos_sem_t *sem) { aos_sem_signal(sem); }
...
void aos_queue_free(aos_queue_t *queue) { vQueueDelete(queue->hdl); }
...
int aos_queue_recv(aos_queue_t *queue, unsigned int ms, void *msg,
                   unsigned int *size) {
  return xQueueReceive(queue->hdl, msg,
                       ms == AOS_WAIT_FOREVER ? portMAX_DELAY : ms2tick(ms)) ==
                 pdPASS
             ? 0
             : -1;
...
void *aos_queue_buf_ptr(aos_queue_t *queue) { return NULL; }
...
int aos_timer_start(aos_timer_t *timer) { return -1; }

int aos_timer_stop(aos_timer_t *timer) { return -1; }

int aos_timer_change(aos_timer_t *timer, int ms) { return -1; }
...
void aos_work_destroy(aos_work_t *work) { vPortFree(work->hdl); }
...
int aos_work_cancel(aos_work_t *work) { return -1; }
...
void *aos_malloc(unsigned int size) { return pvPortMalloc(size); }
...
#if !defined(USE_STDLIB_MALLOC)
void free(void *mem) { vPortFree(mem); }
#endif

void aos_free(void *mem) { vPortFree(mem); }

void aos_msleep(int ms) { vTaskDelay(pdMS_TO_TICKS(ms)); }
