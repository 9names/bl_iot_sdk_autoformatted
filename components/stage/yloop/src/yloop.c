...
aos_loop_t aos_current_loop(void) { return get_context(); }
...
void aos_cancel_poll_read_fd(int sock, aos_poll_call_t action, void *param) {
  ...
  if (i != ctx->reader_count - 1) {
    memmove(&ctx->readers[i], &ctx->readers[i + 1],
            (ctx->reader_count - i - 1) * sizeof(yloop_sock_t));
  }
...
void aos_loop_run(void) {
  ...
  while (!ctx->terminate &&
    ...
    if (!dlist_empty(&ctx->timeouts)) {
      yloop_timeout_t *tmo =
          dlist_first_entry(&ctx->timeouts, yloop_timeout_t, next);
    ...
    for (i = 0; i < readers; i++) {
      if (ctx->pollfds[i].revents & POLLIN) {
        ctx->readers[i].cb(ctx->readers[i].sock, ctx->readers[i].private_data);
      }
    }
  }
