...
class MapfileConfig:
    ...
    def codesize(self, sort_name="flash", sort_type="afile"):
        ...
        f1 = lambda x: os.path.split(re.sub("\\(.*?\\)", "", x))[1]
        f2 = (
            lambda x: re.findall(r"(.*\.a)", x)[0]
            if re.findall(r"(.*\.a)", x)
            else float("nan")
        )
        f3 = (
        ...
        for ram_info in ram_config:
            ...
            df0[ram_info["name"]] = df0.apply(
                lambda x: 0
                if x["address"] < ram_info["start"] or x["address"] > ram_info["end"]
                else x["size"],
                axis=1,
            )
        ...
        print(
            f"======================= Sort {self.sort_type} on {self.sort_name}======================="
        )
        print(str(df))
